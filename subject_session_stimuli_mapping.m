...
for nsubject = 1:length(SUBJECTS)
    ...
    for nsession = 1:length(all_sessions_this_animal)
        ...
        %         experiment_info(nexperiment).ANALYSIS_DATAPATH = fullfile(EPHYS_DATAPATH,'/analysis');
        ...
        for nstimuli = 1:length(StimulusName)
            ...
            if length(probe_id) == 1
                ...
                folderName = findGFolder(EPHYS_DATAPATH,experiment_info(nexperiment).gFileNum(nstimuli));
                experiment_info(nexperiment).stimuli_type(nstimuli).probe(1).EPHYS_DATAPATH = fullfile(EPHYS_DATAPATH,folderName,[folderName,'_imec',num2str(probe_id)]);
                if ~exist(fullfile(EPHYS_DATAPATH,'kilosort'), 'dir')
                    KS_DATAPATH = fullfile(EPHYS_DATAPATH,['kilosort_probe_',num2str(probe_id+1)]); % e.g. imec0 is in kilosort_probe_1 folder

                else

                    KS_DATAPATH = fullfile(EPHYS_DATAPATH,'kilosort');
                end
                ...
                for nprobe = 1:length(probe_id)
                    ...
                    folderName = findGFolder(EPHYS_DATAPATH,experiment_info(nexperiment).gFileNum(nstimuli));
                    experiment_info(nexperiment).stimuli_type(nstimuli).probe(nprobe).EPHYS_DATAPATH = fullfile(EPHYS_DATAPATH,folderName,[folderName,'_imec',num2str(probe_id(nprobe))]);
                    KS_DATAPATH =  fullfile(EPHYS_DATAPATH,['kilosort_probe_',num2str(nprobe)]);
