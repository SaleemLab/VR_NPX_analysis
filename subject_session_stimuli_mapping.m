...
for nsubject = 1:length(SUBJECTS)
    ...
    elseif contains(options,'V1-MEC')
        ...
        dataCell = readcell('Z:\ibn-vision\USERS\Masa\recording_info\session_stimuli_table','Sheet',SUBJECTS{nsubject});
        % Convert the cell array to a table
...
%         EPHYS_DATAPATH_OpenField = fullfile(DATAPATH,SUBJECTS{nsubject},'ephys',[num2str(all_sessions_this_animal(nsession)),'_OpenField']);
        ...
        for nstimuli = 1:length(StimulusName)
            ...
            if contains(StimulusName{nstimuli},'StaticGratings')
                ...
                if ~isempty(dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH,['*',file_time,'*.bin'])))
