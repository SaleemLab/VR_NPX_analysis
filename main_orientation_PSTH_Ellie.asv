    ...
    for n = 1:length(session_info) % How many recording sessions for spatial tasks (PRE, RUN and POST)
        ...
        if (contains(Stimulus_type, 'OMIT') || strcmp(Stimulus_type, 'E_CD')) && contains(plot_choice, 'aggregate') && contains(plot_type, 'FR')
            for nprobe = 1:length(clusters)
                ...
                for stim_pos = 1:sequence_length  % go through each of the 4 stimulus positions
                    ...
                    [~, bins, ~, ~, ~, binned_red] = psthAndBA(all_spike_times, red_onsets, [-0.15 0.30], psthBinSize);
                    [~, ~, ~, ~, ~, binned_blue] = psthAndBA(all_spike_times, blue_onsets, [-0.15 0.30], psthBinSize);
                    ...
                    for idx = 1:length(curr_idx_list)
                        ...
                        % Get unique orientation and contrast for legend
                    ...
                    xlim([-0.15 0.30]);
                    ylabel('Z-scored FR');
                    title(sprintf('Stimulus %d', stim_pos));
                    set(gca, "TickDir", "out", 'box', 'off', 'Color', 'none', 'FontSize', 12);
        ...
        if strcmp(Stimulus_type, 'DCBA') && contains(plot_choice, 'aggregate') && contains(plot_type, 'FR')
            for nprobe = 1:length(clusters)
                ...
                for pos = 1:sequence_length
                    % Normal trials (blue)
                    [~, bins, ~, ~, ~, ba_normal] = psthAndBA(all_spike_times, normal_onsets_by_pos{pos}, [-0.15 0.30], psthBinSize);
                    z_normal = (mean(ba_normal,1) - mu) / sigma;

                    % Reversed trials (red)
                    [~, ~, ~, ~, ~, ba_reversed] = psthAndBA(all_spike_times, reversed_onsets_by_pos{pos}, [-0.15 0.30], psthBinSize);
                    ...
                    normal_ori = normal_sequence(pos);
                    reversed_ori = reversed_sequence(pos);

                    legend(...
                    sprintf('ABCD (%d°)', normal_ori), ...
                    sprintf('DCBA (%d°)', reversed_ori), 'Location', 'best');
                    title(sprintf('Stimulus %d', pos));    
