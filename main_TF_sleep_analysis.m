                    ...
                    for b = 1:nBins
                        ...
                        for ev = 1:nEvents
                            ...
                            if contains(event_types{e}, 'ripples')
                                ...
                                phase_diff_contra = exp(1i * (TF_phase_V1_contra(freq_idx, tIdx, ev_idx) - TF_phase_HPC_contra(freq_idx, tIdx, ev_idx)));
                            end
...
%             % === Significance masks ===
%             [~, ~, sig_mask] = permutation_TF_test(TF_amp_V1_ipsi(:,:,event_index), 1000);
%             curr_TF_stats.(['mean_V1_amp_sig_mask', suffix]){nsession}(1,:,:) = sig_mask;
% 
%             [~, ~, sig_mask] = permutation_TF_test(TF_amp_V1_contra(:,:,event_index), 1000);
%             curr_TF_stats.(['mean_V1_amp_sig_mask', suffix]){nsession}(2,:,:) = sig_mask;
% 
%             [~, ~, sig_mask] = permutation_TF_test(TF_amp_HPC_ipsi(:,:,event_index), 1000);
%             curr_TF_stats.(['mean_HPC_amp_sig_mask', suffix]){nsession}(1,:,:) = sig_mask;
% 
%             [~, ~, sig_mask] = permutation_TF_test(TF_amp_HPC_contra(:,:,event_index), 1000);
%             curr_TF_stats.(['mean_HPC_amp_sig_mask', suffix]){nsession}(2,:,:) = sig_mask;
% 
% 
% 
%             [~, ~, sig_mask] = permutation_TF_test(TF_phase_V1_ipsi(:,:,event_index), 100, ...
%                 'data2', TF_phase_V1_contra(:,:,event_index), 'shuffle_option', 2);
%             curr_TF_stats.(['mean_PLV_sig_mask_V1', suffix]){nsession} = sig_mask;
% 
%             [~, ~, sig_mask] = permutation_TF_test(TF_phase_HPC_ipsi(:,:,event_index), 1000, ...
%                 'data2', TF_phase_HPC_contra(:,:,event_index), 'shuffle_option', 2);
%             curr_TF_stats.(['mean_PLV_sig_mask_HPC', suffix]){nsession} = sig_mask;
% 
%             if contains(event_types{e}, 'ripples')
%                 [~, ~, sig_mask] = permutation_TF_test(TF_phase_HPC_ipsi(:,:,event_index), 1000, ...
%                     'data2', TF_phase_V1_ipsi(:,:,event_index), 'shuffle_option', 2);
%                 curr_TF_stats.(['mean_PLV_sig_mask_V1_HPC', suffix]){nsession}(1,:,:) = sig_mask;
% 
%                 [~, ~, sig_mask] = permutation_TF_test(TF_phase_HPC_ipsi(:,:,event_index), 1000, ...
%                     'data2', TF_phase_V1_contra(:,:,event_index), 'shuffle_option', 2);
%                 curr_TF_stats.(['mean_PLV_sig_mask_V1_HPC', suffix]){nsession}(2,:,:) = sig_mask;
%             else
%                 [~, ~, sig_mask] = permutation_TF_test(TF_phase_V1_ipsi(:,:,event_index), 1000, ...
%                     'data2', TF_phase_HPC_ipsi(:,:,event_index), 'shuffle_option', 2);
%                 curr_TF_stats.(['mean_PLV_sig_mask_V1_HPC', suffix]){nsession}(1,:,:) = sig_mask;
% 
%                 [~, ~, sig_mask] = permutation_TF_test(TF_phase_V1_ipsi(:,:,event_index), 1000, ...
%                     'data2', TF_phase_HPC_contra(:,:,event_index), 'shuffle_option', 2);
