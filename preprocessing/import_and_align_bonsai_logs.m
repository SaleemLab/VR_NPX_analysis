...
elseif strcmp(options.paradigm,'masa')
    ...
    counts = 1 ;
    %     histogram(photodiode.Photodiode_smoothed,1000)
    for itrial =1:nTrials
        ...
        %          temp_idx = find(smoothdata(photodiode.Photodiode(frameidx_end:end),'movmedian', 5)<pd_thresh_up,1,'first');
        temp_idx = find(photodiode.Photodiode_smoothed(frameidx_start:end)>pd_thresh_up,1,'first');
        if isempty(temp_idx)
            temp_idx = 0;
            counts = counts + 1;
        end
        ...
        frameidx_end = frameidx_end + ceil(temp_idx/2)-1; % Adding half of the delay of the previous quad change

        temp_idx = find(photodiode.Photodiode_smoothed(frameidx_end:end)<pd_thresh_down,1,'first');
        if isempty(temp_idx)
            temp_idx = 0;
        end
%         temp_idx = find(smoothdata(photodiode.Photodiode(frameidx_end:end),'movmedian', 5)<pd_thresh_down,1,'first');
    ...
    if counts > nTrials/2
        photodiodeData.photodiode_failure = 1;
    end
