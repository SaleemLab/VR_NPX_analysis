...
%         dataCell = readcell('C:\Users\masah\Documents\GitHub\VR_NPX_analysis\session_stimuli_table','Sheet',SUBJECTS{nsubject});
    ...
    elseif contains(options,'V1-HPC')
        ROOTPATH = 'V:\Ellie';
        %         stimuli_info = readtable('C:\Users\masah\Documents\GitHub\VR_NPX_analysis\session_stimuli_table','Sheet',SUBJECTS{nsubject});
        stimuli_info = readtable('V:\Ellie\DATA\Ellie_session_stimuli_table','Sheet',SUBJECTS{nsubject});
        % stimuli_info = readtable('C:\Users\masah\Documents\GitHub\VR_NPX_analysis\session_stimuli_table','Sheet',SUBJECTS{nsubject});
        dataCell = readcell('V:\Ellie\DATA\Ellie_session_stimuli_table','Sheet',SUBJECTS{nsubject}); % this can create missing object when session id is missing.
%         dataCell = readcell('C:\Users\masah\Documents\GitHub\VR_NPX_analysis\session_stimuli_table','Sheet',SUBJECTS{nsubject});
        % Convert the cell array to a table
        stimuli_info_table = cell2table(dataCell(2:end,:),"VariableNames",stimuli_info.Properties.VariableNames); % not the best way but works
    ...
    for nsession = 1:length(all_dates_this_animal)
        ...
        % Path to the date folder (as Ellie has saved bonsai data within recording subfolders e.g. 0_g0, rather than putting everything
        % together within on date folder)
        date_path = fullfile(DATAPATH, SUBJECTS{nsubject}, 'stimuli', num2str(all_dates_this_animal(nsession)));
        % Get list of g-folders (like 0_g0, 0_g1, etc.)
        g_folders = dir(fullfile(date_path, '*_g*'));
        % Build full paths to each g-folder
        BONSAI_DATAPATH = cell(1, numel(g_folders));
        for i = 1:numel(g_folders)
            BONSAI_DATAPATH{i} = fullfile(date_path, g_folders(i).name);
        end
        % Store in experiment_info
        experiment_info(nexperiment).BONSAI_DATAPATH = BONSAI_DATAPATH;
        ...
        for nstimuli = 1:length(StimulusName)
            ...
            if contains(StimulusName{nstimuli},'StaticGrating')
                ...
                bonsai_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',this_stimulus, '*.csv']));

            else
                this_stimulus = StimulusName{nstimuli};
                if contains(this_stimulus , 'Track')
                    bonsai_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*','Track', '_*.csv']));
                elseif contains(this_stimulus,'Masa2tracks')
                    bonsai_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*','Masa2tracks', '*.csv']));
                    %                 elseif contains(this_stimulus,'Sleep')
                    %                     bonsai_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*','Sleep', '*.csv']));
                else
                    bonsai_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',this_stimulus, '*.csv']));
                end
            end
            ...
            if ~contains(bonsai_files(stimulus_counter(nstimuli)).name,'OpenField')
                ...
                bonsai_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',file_time,'*.csv']));
            end

            if ~isempty(dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',this_stimulus,'*.bin'])))

                % Unfortunately... sometimes bin file time stamp can be 1 or 2
                % mins later than others....
                bin_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',this_stimulus,'*.bin']));
                ...
                bin_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',file_time1,'*.bin']));
            ...
            if ~isempty(dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',file_time,'*proc.mat'])))
                mat_files = dir(fullfile(experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli},['*',file_time,'*proc.mat']));
            ...
            if isnan(all_session_id_this_date{nstimuli})
                ...
                if ~exist(EPHYS_DATAPATH_temp)
                    EPHYS_DATAPATH_temp = fullfile(DATAPATH,SUBJECTS{nsubject},'ephys',num2str(all_dates_this_animal(nsession)),[SUBJECTS{nsubject},'_',num2str(all_dates_this_animal(nsession)),'_',all_session_id_this_date{nstimuli}]);
                end
            end
            ...
            for nprobe = 1:length(probe_id)
                ...
                %end
                [~, foldername] = fileparts(EPHYS_DATAPATH_temp);
                if length(foldername) > 8  % e.g., '20250203_01' has more than 8 characters
                    % It's a session subfolder like '20250203_01', move up one level
                    date_folder = fileparts(EPHYS_DATAPATH_temp);
                else
                    % It's already the date folder like '20250203', no need to move up
                    date_folder = EPHYS_DATAPATH_temp;
                end
                SORTER_DATAPATH = fullfile(date_folder, ['probe', num2str(probe_id(nprobe))]);               
                ...
                experiment_info(nexperiment).session(nstimuli).probe(nprobe).segment_frames =...
                    fullfile(SORTER_DATAPATH,'sorters',['probe', num2str(probe_id(nprobe)), 'segment_frames.csv']);
                

                experiment_info(nexperiment).session(nstimuli).probe(nprobe).BONSAI_DATAPATH = experiment_info(nexperiment).BONSAI_DATAPATH{nstimuli};
