function [behaviour,position] = import_and_align_Bonsai_OpenField(StimulusName,options)
...
behaviour.computer_time = computer_timevec(:,4)*60*60 + computer_timevec(:,5)*60 + computer_timevec(:,6);
behaviour.wheel_time =  behaviour.computer_time- behaviour.computer_time(1);
behaviour.Sync = table2array(this_table(:,4));
behaviour.mobility = table2array(this_table(:,5));
behaviour.X = table2array(this_table(:,1));
behaviour.Y = table2array(this_table(:,3));
...
peripherals.Time =  behaviour.computer_time*1000;
...
behaviour.sglxTime = peripherals.sglxTime;
