...
function [bonsai_data] = alignBonsaiToNidqSyncTimes(bonsai_data,syncTimes_ephys)
bonsai_idx = find(diff(bonsai_data.Sync)==1);
syncTimes_bonsai_on = bonsai_data.Time(bonsai_idx+1)./1000; % add 1 to idx to compensate for diff and convert from ms to s

bonsai_idx = find(diff(bonsai_data.Sync)==0);
syncTimes_bonsai_off = bonsai_data.Time(bonsai_idx+1)./1000; % add 1 to idx to compensate for diff and convert from ms to s
syncTimes_bonsai = sort([syncTimes_bonsai_on syncTimes_bonsai_off]);
