...
if ~isempty(td)
    load(fullfile(td(1).folder,td(1).name),'Nidq');
...
[unique_pd_tvec,ia,ic]= unique(photodiode.sglxTime);
unique_pd = photodiode.Photodiode_smoothed(ia);

pd_tvec_60 = interp1(unique_pd_tvec,unique_pd_tvec,peripherals.sglxTime(1):1/60:peripherals.sglxTime(end),'linear');
pd_60 = interp1(unique_pd_tvec,unique_pd,peripherals.sglxTime(1):1/60:peripherals.sglxTime(end),'linear');

quad_60(isnan(pd_60))=[];
pd_60(isnan(pd_60))=[];
[r, lags] = xcorr(normalize(quad_60), normalize(pd_60));
[~, joint_idx] = max(r);
best_lag = lags(joint_idx);
