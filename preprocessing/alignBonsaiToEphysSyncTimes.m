...
if sum(diff(syncTimes_ephys)<0.1)>0
    syncTimes_ephys(find(diff(syncTimes_ephys)<0.01)+1) = []; % remove sync pulse that happens less than 10 ms (possibly due to jitter)
end

if sum(diff(syncTimes_bonsai)<0.1)>0
    syncTimes_bonsai(find(diff(syncTimes_bonsai)<0.01)+1) = []; % remove sync pulse that happens less than 10 ms (possibly due to jitter)
end
