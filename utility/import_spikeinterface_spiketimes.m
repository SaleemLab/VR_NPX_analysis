...
if isfield(options,'sorter_type')
    if contains(options.sorter_type,'original')
        ...
        if isempty(dir(fullfile(SORTER_DATAPATH,'spikes.csv')))
            SORTER_DATAPATH = fullfile(options.SORTER_DATAPATH,'sorters',options.sorter_folder,'in_container_sorting');
        end
    end
else
    SORTER_DATAPATH = fullfile(options.SORTER_DATAPATH,'waveform',[options.sorter_folder,'_merged'],'sorting');
    if isempty(dir(fullfile(SORTER_DATAPATH,'spikes.csv')))
        SORTER_DATAPATH = fullfile(options.SORTER_DATAPATH,'sorters',[options.sorter_folder,'_merged'],'in_container_sorting');
    end
end
...
if isempty(DIR_segment)
    temp_DIR =  fullfile(options.EPHYS_DATAPATH,'..','..','..',sprintf('probe%isegment_frames.csv',options.probe_id));
    options.segment_frames = fullfile(options.EPHYS_DATAPATH,'..','..','..',sprintf('probe%isegment_frames.csv',options.probe_id));
end
...
if contains(options.sorter_folder,'kilosort')
    ...
    [file_to_use imecMeta chan_config sorted_config] = extract_NPX_channel_config(options,1);
     cluster_coordinates = readNPY(fullfile(options.SORTER_DATAPATH,'sorters',options.sorter_folder,'sorter_output','channel_positions.npy')); % load all good channels (not noise channels) coordinate used for spike sorting
%     original_cluster_index = readNPY(fullfile(options.SORTER_DATAPATH,'sorters',[options.sorter_folder,'_merged'],'properties','original_cluster_id.npy')); % load all good channels (not noise channels) coordinate used for spike sorting
    ...
    %      cluster_coordinates = readNPY(fullfile(options.SORTER_DATAPATH,'waveform',[options.sorter_folder,'_merged'],'extensions','unit_locations','unit_locations.npy')); % Information about template waveform (for postprocessed clusters);
    ...
    if contains(imecMeta.imDatPrb_pn,'NP2013') %
        ...
        for nchannel = 1:size( cluster_coordinates,1)
            if isempty(chan_config.Channel(chan_config.Ks_xcoord ==  cluster_coordinates(nchannel,1)+27 & chan_config.Ks_ycoord ==  cluster_coordinates(nchannel,2)+15))
        ...
        if error_count>0
            ...
            unique_x = unique( cluster_coordinates(:,1));
            temp =  cluster_coordinates(:,1);
            unique_x = reshape(unique_x,2,[]);
            for x = 1:size(unique_x,2)
                temp( cluster_coordinates(:,1)==unique_x(1,x)) = unique_x(2,x);
                temp( cluster_coordinates(:,1)==unique_x(2,x)) = unique_x(1,x);
            end

             cluster_coordinates(:,1) = temp;
        end
        ...
        for nchannel = 1:size( cluster_coordinates,1)
            ...
            all_good_channels(nchannel) = chan_config.Channel(chan_config.Ks_xcoord ==  cluster_coordinates(nchannel,1)+11 & chan_config.Ks_ycoord ==  cluster_coordinates(nchannel,2));
        end
    end
