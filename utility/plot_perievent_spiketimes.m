...
addParameter(p,'plot_option','by time',@isstr) % Select channels for analysis (default is all the channles or at least all channels loaded (e.g. only from one cloumn)
            ...
            if sum(event_id==2)>1
                if length(h)>2
                    legend([h(1:4)],{'Track 1 odd','Track 1 even','Track 2 odd','Track 2 even'},'Color','none')
                     ...
                     legend([h(1:2)],{'Track 1','Track 2'},'Color','none')
                end
            else
                legend([h(1:2)],{'Track 1','Track 2'},'Color','none')
            end
