...
addParameter(p,'speed_filtering',0,@isnumeric) % if shading running period
...
speed_filtering = p.Results.speed_filtering;
...
for iPlot = 1: ceil(no_cluster/(no_subplot))
    ...
    for iCluster = 1:no_subplot
        ...
        if speed_filtering == 1
            ...
            event2_rasterX = reshape(rasterX_reshaped(:,event2_raster_id),[1 sum(event2_raster_id)*3]);
            event2_rasterY = reshape(rasterY_reshaped(:,event2_raster_id),[1 sum(event2_raster_id)*3]);
            plot(event1_rasterX,event1_rasterY,'LineWidth',0.5,'Color','k')
            hold on;
            plot(event2_rasterX,event2_rasterY,'LineWidth', 0.5,'Color','k')
        end
...
%         xlabel('Time')
        ...
        for track_id = 1:2
            ...
            if speed_filtering==1
                if track_id == 1
                    binnedArray1(speed_this_event>5)=nan;
                else
                    binnedArray2(speed_this_event>5)=nan;
                end
            end
        end
        ...
        if size(binnedArray1,1)==1
            ...
            average_map_track1 = psth_track1;
        else
        ...
        if size(binnedArray2,1)==1
            ...
            average_map_track2 = psth_track2;
        else
