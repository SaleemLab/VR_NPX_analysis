...
%                 save(fullfile(options.ANALYSIS_DATAPATH,sprintf('extracted_LFP%s.mat',erase(stimulus_name{n},'Masa2tracks'))),'LFP');
            ...
            for nprobe = 1:length(session_info(n).probe)
                ...
                LFP_SR = 1/mean(diff(LFP(probe_no).tvec));
                tvec_edges = [LFP(probe_no).tvec(1)-1/(1/mean(diff(LFP(probe_no).tvec))*2) LFP(probe_no).tvec+1/(1/mean(diff(LFP(probe_no).tvec))*2)];
                CA1_spike_counts=[];
                V1_spike_counts=[];
                w = gausswin(0.05*1/mean(diff(LFP(probe_no).tvec)));
                w = w / sum(w);
       
                spike_times = V1_clusters(probe_no).spike_times;
