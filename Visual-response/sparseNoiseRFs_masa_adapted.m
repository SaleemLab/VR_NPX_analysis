...
% for SVD method, can directly SUA_data * reshape(permute(stim_matrix,[3 1 2]),size(stim_matrix,3),size(stim_matrix,1)*size(stim_matrix,2))
unit_id = 316;
SUA_data = squeeze(resps(unit_id,:,:));
stimLinear = reshape(permute(stim_matrix,[3 1 2]),size(stim_matrix,3),size(stim_matrix,1)*size(stim_matrix,2));
% also convert to stimBlack stimWhite stimContrast
respMatrixL = SUA_data * stimLinear;
[UL,SL,VL] = svd(respMatrixL);
stimWeightL = reshape(VL(:,1),8,16);
timeCourseL = UL(:,1);
stimBlack = stimLinear; stimBlack(stimLinear > 0) = 0;
respMatrixB = SUA_data * stimBlack;
[UB,SB,VB] = svd(respMatrixB);
stimWeightB = reshape(VB(:,1),8,16);
timeCourseB = UB(:,1);
stimWhite = stimLinear; stimWhite(stimLinear < 0) = 0;
respMatrixW = SUA_data * stimWhite;
[UW,SW,VW] = svd(respMatrixW);
stimWeightW = reshape(VW(:,1),8,16);
timeCourseW = UW(:,1);
stimContrast = stimLinear; stimBlack(stimLinear < 0) = 1;
respMatrixC = SUA_data * stimContrast;
[UC,SC,VC] = svd(respMatrixC);
stimWeightC = reshape(VC(:,1),8,16);
timeCourseC = UC(:,1);
figure; 
subplot(2,2,1); imagesc(flip(stimWeightL));colorbar;
subplot(2,2,2); imagesc(flip(stimWeightB));colorbar;
subplot(2,2,3); imagesc(flip(stimWeightW));colorbar;
subplot(2,2,4); imagesc(flip(stimWeightC));colorbar;
%% Use Ridge Regression with Cross-Validation to get a kernel version of RFs


stim_matrix = cat(3,stimData.stim_matrix{:}); % N rows x M cols x nFrames
lambdas = logspace(-5,-3,5);
crossFolds = 10;
method = 'shift';
[receptiveFields, explainedVariance, predictions] = ...
    ridgeReceptiveField(resps, stim_matrix, lambdas, crossFolds,method);
%save(fullfile(EPHYS_DATAPATH,'analysis','RidgeReceptiveFields.mat'),'receptiveFields','explainedVariance','predictions')
%load(fullfile(EPHYS_DATAPATH,'analysis','RidgeReceptiveFields.mat'));
RFs = reshape(receptiveFields,8,16,7,4,size(receptiveFields,2)); % height * width * frames * grid type * units, grid type: linear, black, white, contrast

%plot explained variance
meanFoldEVar = squeeze(nanmean(explainedVariance,2));
lambdasAll = repmat(lambdas,size(explainedVariance,1),1);
[~, bestStimLams] = max(mean(explainedVariance, 2), [], 3);
figure;
hold on
for i =1:length(lambdas)
    scatter(lambdasAll(:,i),meanFoldEVar(:,i))
end
ylim([-1, 1])
%plot RFs
figure;
imagesc(flip(RFs(:,:,6,4,316)));colorbar
%plot the temporal predictions
predictionsTemp = zscore(reshape(squeeze(predictions(:,:,316,1)),[size(predictions,1)*size(predictions,2) 1]));
filterStim = reshape(permute(stim_matrix,[3 1 2]),size(stim_matrix,3),size(stim_matrix,1)*size(stim_matrix,2))*reshape(squeeze(RFs(:,:,6,4,316)),8*16,1);
threshFiltStim = filterStim > 0.1 | filterStim <-0.1;
respsUnit = zscore(reshape(resps(316,:,:),1,7*9000));
tFrame = 1:7:6300;
figure; plot(1:6300,predictionsTemp(1:6300));hold on; xline(tFrame(threshFiltStim(1:900)),'Color','m');xline(tFrame(~threshFiltStim(1:900)),'Color','b');
hold on; plot(1:6300, respsUnit(:,1:6300));