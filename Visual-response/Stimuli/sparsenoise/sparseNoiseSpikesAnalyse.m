...
function [initMap, lfp_data,wheel_data,eye_data] = sparseNoiseSpikesAnalyse(thisAnimal,thisSession,thisFileNum,options);
...
if~isfield(options,'stim_duration')
...
options.spikesToppath = fullfile(options.serverPath,'DATA','SUBJECTS',upper(thisAnimal),'Ephys',upper(thisSession));
...
% % % ttltimestamps = union(OE.OEttl.upPhases, OE.OEttl.downPhases);
% For spikes...
loadSpikeData(thisAnimal,thisSession,thisFileNum,options)
