...
switch options.sync_input
    ...
    case 'adc'
        ...
        if options.photo_br ==1
            ...
            ttlind = find(ttlData>1.5);
            ttl_diff = diff(ttlind);
            ttlind2 = find(ttl_diff > 4500); % 150ms difference
            upPhases = [ttlind(1); ttlind(ttlind2+1)];
            
            %Black squares
            ttlind = find(ttlData<0.5);
            ttl_diff = diff(ttlind);
            ttlind2 = find(ttl_diff > 4500);
            ...
            upPhases = [ttlind(find(ttl_diff_ts > 0.1 &  ttl_diff_ts < 1.5)+1)]; % find sample points in range of phase reversals
            blockStarts = [firstCrossing; ttlind(find(ttl_diff_ts > 5)+1)]; % find sample points in range of block changes
            
            %I have noticed a problem with few recordings having high signal at
            %the begginning of the session, probably due to the black square
            %that was present before starting the stimulus.
            if length(blockStarts)>10
                %blockStarts = blockStarts(2:end);
                %upPhases = upPhases(upPhases>=blockStarts(1));
                warning('More than 10 blocks detected. Please check');
                blockStarts(diff(blockStarts)<6000000) = [];
            end
            
            upPhases = union(blockStarts,upPhases);
            ...
            c = find(diff(ttlind2)>10);
            downPhases = ttlind2(c(diff(c)>100)+1);
