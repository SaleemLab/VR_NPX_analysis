...
stim_frames=2*abs(ES.SN_sequence/max(ES.SN_sequence(:))-0.5); %Calculate the absolute stim contrast 0 to 1
...
%     [forfit_xval,forfit_yval]=meshgrid(1:8,1:8);
    ...
    if all(isnan(m.STC(:,:,icell)))
        ...
        [fitparams(icell,:), modelout(:,:,icell), options,R2(icell),initparamsBl] = ibn_fit2DGaussLsq_Speed(m.STC(:,:,icell),forfit_xval,forfit_yval,options);
