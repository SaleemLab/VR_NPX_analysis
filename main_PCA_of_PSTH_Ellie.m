    ...
    for n = 1:length(session_info) % How many recording sessions for spatial tasks (PRE, RUN and POST)
        ...
        if (contains(Stimulus_type, 'GAVNIK_ABCD')) || (contains(Stimulus_type, 'TRAIN'))
            for nprobe = 1:length(clusters)
                ...
                %%%% Filter out neurons which do not appear to be visually responsive (neither activated nor suppressed by the OP stimulus)
                ...
                % Plot signal correlation histogram (as percentage)
                h_signal = histogram(all_signal_corr_values, bin_edges, ...
                    'DisplayStyle', 'stairs', 'LineWidth', 2, 'EdgeColor', 'k', 'Normalization', 'probability');
                
                % Preallocate for legend handles
                h_noise = gobjects(1, length(ordered_oris));
                
                % Plot noise correlation histograms for each stimulus type (as percentage)
                for i = 1:length(ordered_oris)
                    corrmat = stored_corrmats{i};
                    off_diag = corrmat(~eye(size(corrmat)));  % exclude diagonals
                    h_noise(i) = histogram(off_diag, bin_edges, ...
                        'DisplayStyle', 'stairs', 'LineWidth', 1.5, ...
                        'EdgeColor', colors(i, :), 'Normalization', 'probability');  % Vary color slightly
                end
                ...
                correlation_summary.signal_values = all_signal_corr_values;
                ...
                % Store noise values per orientation
                correlation_summary.noise_values = cell(1, length(ordered_oris));
                for i = 1:length(ordered_oris)
                    corrmat = stored_corrmats{i};
                    off_diag = corrmat(~eye(size(corrmat)));
                    correlation_summary.noise_values{i} = off_diag;
                end
