...
selected_cells_L = unique([find(contains(place_fields_all_L(1).region,'HPC') & ...
    place_fields_all_L(1).ripple_modulation_percentile>0.95) find(contains(place_fields_all_L(1).region,'HPC') & ...
...
figure
subplot(2,2,1)
imagesc(normalize(place_fields_all_L(1).average_map(:,selected_cells_L),'range')');
xlabel('position')
subplot(2,2,2)
imagesc(normalize(place_fields_all_L(2).average_map(:,selected_cells_L),'range')');
xlabel('position')
% title('L ripple')
subplot(2,2,3)
imagesc(normalize(place_fields_all_R(1).average_map(:,selected_cells_R),'range')');
xlabel('position')
subplot(2,2,4)
imagesc(normalize(place_fields_all_R(2).average_map(:,selected_cells_R),'range')');
xlabel('position')
% title('R ripple')

figure
subplot(2,2,1)
imagesc(normalize(place_fields_all_L(1).ripple_PSTH(:,selected_cells_L),'range')');
xline(size(place_fields_all_L(1).ripple_PSTH,1)/2)
title('L ripple T1')

subplot(2,2,2)
imagesc(normalize(place_fields_all_L(2).ripple_PSTH(:,selected_cells_L),'range')');
xline(size(place_fields_all_L(1).ripple_PSTH,1)/2)
title('L ripple T2')

subplot(2,2,3)
imagesc(normalize(place_fields_all_R(1).ripple_PSTH(:,selected_cells_R),'range')');
xline(size(place_fields_all_L(1).ripple_PSTH,1)/2)
title('R ripple T1')

subplot(2,2,4)
imagesc(normalize(place_fields_all_R(2).ripple_PSTH(:,selected_cells_R),'range')');
xline(size(place_fields_all_L(1).ripple_PSTH,1)/2)
title('R ripple T2')
sgtitle('ripple PSTH')
...
selected_cells_R = unique([find(contains(place_fields_all_R(1).region,'V1') & ...
    place_fields_all_R(1).ripple_modulation_percentile>0.95) find(contains(place_fields_all_R(1).region,'V1') & ...
