...
depth_for_analysis = 'CA1'; % choose 'L4' or 'L2_3' or 'L5_6' 
...
z_score_period = 'none'; % 'none' = no z scoring or z score either over 'entire_session' or 'first30secs' (for every stimulus recording
...
sessions_to_plot = [15]; %4/4 row numbers of recording dates in "experiment_info" 15, 9
    ...
    for stim_idx = 1:length(Stimulus_types)
        ...
        for n = 1:length(session_info) 
            ...
            switch depth_for_analysis
                ...
                case 'L2_3'
                    depth_range = (max(L4_depth_range) : max(V1_depth_range));
                case 'L5_6'
                    depth_range = (min(V1_depth_range) : min(L4_depth_range));
                case 'V1'
