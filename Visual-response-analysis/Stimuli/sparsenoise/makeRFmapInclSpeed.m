...
%   runningSpeed: [default empty] - include speed in regression (only valid if
%               using fitlm method
% Outputs:
%   thisAvgMap - n x m x N receptive field map (average, fitlm)
%   thisVarMap - n x m x N receptive field map (variance, p value)
%   thisRunningSpeedBeta - scalar value representing the regression onto speed
%   thisRunningSpeedP - p-value for speed Beta
%
% History
% SGS 13th April 2020. Wrote makeRFmap.
% SGS 5th November 2021 Adapted from makeRFmap to include speed as a
%       regressor
function [thisAvgMap,thisVarMap,thisRunningSpeedBeta,thisRunningSpeedP] = makeRFmap(stimulusMatrix,responseVector,delaysInBins,method,runningSpeedVector)
if ~exist('runningSpeedVector','var')
    runningSpeedVector = [];   % If no speed specified, make an empty matrix
end
if ~exist('method','var')
...
% Default ouputs
thisAvgMap = NaN(size(stimulusMatrix,1),size(stimulusMatrix,2));
thisVarMap = thisAvgMap;
thisRunningSpeedBeta = NaN;
thisRunningSpeedP = NaN;

% Make sure response vector is a column vector
responseVector  = responseVector(:);

% Make sure speed vector is a column vector
runningSpeedVector  = runningSpeedVector(:);
...
for thisDelay = 1:length(delaysInBins)
    ...
    switch(method)
        case 'fitlm'
            ...
            switch(isempty(runningSpeedVector))
                case 1 % No running speed included in inputs, so just do standard
                ...
                case 0 % Running speed was included in inputs
                    td = fitlm([thisStimulusMatrix, runningSpeedVector],thisResponseVector);
                    theseResponseWeights = td.Coefficients.Estimate(2:end-1);
                    theseVariances = td.Coefficients.pValue(2:end-1);
                    allResponses(thisDelay,:) = theseResponseWeights;
                    allVariances(thisDelay,:) = theseVariances;
                    thisRunningSpeedBeta = td.Coefficients.Estimate(end);
                    thisRunningSpeedP = td.Coefficients.pValue(end);
