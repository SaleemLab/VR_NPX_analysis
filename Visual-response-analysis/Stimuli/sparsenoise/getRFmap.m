...
ProcessedFiles = dir([SubjectDir mouseName filesep 'Processed',filesep,'*SN_AC_Spikes_ks*']);

f_n = 1; f_n_i = 0;
while f_n<=length(ProcessedFiles)
    if strfind(ProcessedFiles(f_n).name,'SN')
        d_i1 = strfind(ProcessedFiles(f_n).name,['19' iexp(1:2)]);
        d_i2 = strfind(ProcessedFiles(f_n).name,['20' iexp(1:2)]);
        d_i=union(d_i1,d_i2);
        if isempty(d_i)
...
m = ReverseCorrelationSTC_Speed(ES,m,plotSingleUnits);
