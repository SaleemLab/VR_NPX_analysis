...
for nsession =1:length(experiment_info)
    ...
    for n = 1:length(session_info)
        for nprobe = 1:length(session_info(n).probe) % For each session, how many probes
            ...
            load(fullfile(options.ANALYSIS_DATAPATH,"extracted_behaviour_RUN1.mat"))
            load(fullfile(options.ANALYSIS_DATAPATH,"extracted_task_info_RUN1.mat"))


            bin_size = 1/1250;
            AnalysisTimeWindow = [-2 10];
            ...
            [resps] = perievent_LFP_response(start_time_all,AnalysisTimeWindow,options,'filter_freq',[300 625]);
            tvec = linspace(AnalysisTimeWindow(1),AnalysisTimeWindow(2),size(resps,2));
            start_time_all


            V1_channel_ids = find(V1_channel_ids==1);
            zscored_resp=[];
            spikes=[];
            for ntrack = 1:2
                lap_id = find(track_ID_all==ntrack);
                for nchannel = 1:length(V1_channel_ids)
                    this_channel_resp = abs(hilbert(squeeze(resps(V1_channel_ids(nchannel),:,lap_id))));
                    resp_1D = reshape(this_channel_resp,[],1);
                    resp_1D = (resp_1D-nanmean(resp_1D))./nanstd(resp_1D);
                    putative_spikes = resp_1D>5;
                    zscored_resp{ntrack}(nchannel,:,:) = reshape(resp_1D,size(this_channel_resp));
                    spikes{ntrack}(nchannel,:,:) = reshape(putative_spikes,size(this_channel_resp));
                end
            end

            plot(tvec,squeeze(nanmean(zscored_resp{1}(:,:,:),1)),'r');hold on;
            plot(tvec,squeeze(nanmean(zscored_resp{2}(:,:,:),1)),'b')
            ...
            for ntrack = 1:2
                for nchannel = 1:length(V1_channel_ids)
                    channel_spike_counts(nchannel,ntrack) = sum(sum(spikes{ntrack}(nchannel,tvec>0&tvec<0.1,:)));
                end
            end

            bar(channel_spike_counts(:,1)); hold on; bar(-channel_spike_counts(:,2))

            channel_spike_counts=[];
            for ntrack = 1:2
                for nchannel = 1:length(V1_channel_ids)
                    channel_spike_counts(nchannel,ntrack) = sum(sum(spikes{ntrack}(nchannel,tvec>-0.3&tvec<-0.02,:)));
                end
            end
...
for nprobe = 1:2
    ...
    for nshank = 1:4
        ...
        thisMap_s = nanmean(RF_map(:,:,V1_channel_ids),3);
