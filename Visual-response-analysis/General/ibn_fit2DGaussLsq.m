...
% Function to fit 2D gaussian to genric input data
%
...
if ~exist('options','var')
    sdInit = 2;
    rangeBg = 4; % eg = 2; ie from half to twice the minimum
...
% For Gaussian fit
initparamsBl(1) = forfit_xval(row,col);         % centre pos, x
initparamsBl(2) = forfit_yval(row,col);         % centre pos, y
...
options.lb = [min(forfit_xval(:))-initparamsBl(4), min(forfit_yval(:))-initparamsBl(4),...
    0, 1, initparamsBl(5)-initparamsBl(5)*rangeBg+0.01]; %
options.ub = [max(forfit_xval(:))+initparamsBl(4), max(forfit_yval(:))+initparamsBl(4),...
    max(forfit_f0(:))*10, 100, initparamsBl(5)+initparamsBl(5)*rangeBg+0.01]; % Just centre, no surround
