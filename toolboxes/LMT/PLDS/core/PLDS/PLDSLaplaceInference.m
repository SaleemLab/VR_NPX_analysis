...
if computeVarBound
    ...
    % Modified by Masahiro Takigawa 2025 to use sparse blkdiag, which handles long data with limited
    % memory.
    Ci   = sparse(double(params.model.C));   % yDim x xDim
    Wmax = kron(speye(Tmax), Ci);            % (yDim*Tmax) x (xDim*Tmax), sparse
