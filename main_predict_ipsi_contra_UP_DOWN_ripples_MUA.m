...
%%
%%%%%%%%%%%%% Effect of ripples on UP survival probability
...
% [C,index,ib] = intersect(non_overlap_idx,find(~isnan(ripples_info.ipsi_last_ripples_power_UP))');
index = intersect(find(~isnan(UP_DOWN_info.contra_ripple_counts_UP))',find(UP_DOWN_info.contra_ripple_counts_UP>0)');
data = UP_DOWN_info.contra_last_ripples_power_UP(index);
...
ripple_threshold2 = prctile(data,75);
event_id = find(data<ripple_threshold1);
timeToTransition_ripples = UP_DOWN_info.contra_time_from_last_ripples_UP(index);
