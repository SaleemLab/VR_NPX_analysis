...
for nsession =1:length(experiment_info)
    ...
    for n = 1:length(session_info) % How many recording sessions for spatial tasks (PRE, RUN and POST)
        ...
        for nprobe = 1:length(merged_clusters)
            ...
            plot_perievent_spiketimes(merged_clusters(nprobe).spike_times,merged_clusters(nprobe).merged_spike_id,Task_info,Behaviour,[5 1],[-0.5 1],0.02,...
                'unit_depth',merged_clusters(nprobe).peak_depth(ia),'unit_region',merged_clusters(nprobe).region(ia),'unit_id',C,'event_times',sorted_events,...
                'event_label',{'Track 1','Track 2'},'event_id',event_id);
