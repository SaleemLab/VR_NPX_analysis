...
# ==============================================================================
message("\nFitting Ripple Power Model...")

mdl_ripple <- bam(Event_Coherence_Post_GeoMean ~ 
...
mdl_spindle <- bam(Event_Coherence_Post_GeoMean ~ 
                    # 1. Surviving Main Power Effects
                    s(SpindlePower_Match_Z, k = 5) + 
                    s(SpindlePower_NonMatch_Z, k = 5) + 
...
mdl_SO <- bam(Event_Coherence_Post_GeoMean ~ 
                     # 1. Surviving Main Power Effects
                     s(JointState, k = 5) +
...
message("\n--- FINAL MODEL SUMMARY ---")
print(summary(mdl_spindle))

message("\nGenerating Final Publication Figures...")

# 1. Main Effect: Ripple Power
dev.new(noRStudioGD = TRUE)
p_ripple <- draw(mdl_ripple, select = "s(RipplePower_Z)", residuals = FALSE, rug = FALSE) + 
  theme_bw() + labs(title = "Main Effect: Ripple Power", x = "Ripple Power (Z)", y = "Partial Effect")

print(p_ripple)

# Calculate your scaling factors (replace with your actual data)
raw_breaks <- c(5, 10, 15)
z_breaks <- sapply(raw_breaks, function(val) {
  # Find the Z-score where raw RipplePower is closest to the target
  dat_clean$RipplePower_Z[which.min(abs(dat_clean$RipplePower - val))]
})

dev.new(noRStudioGD = TRUE)
p_ripple_raw <- draw(mdl_ripple, select = "s(RipplePower_Z)", residuals = FALSE, rug = FALSE) + 
  theme_bw() + 
  scale_x_continuous(
    breaks = z_breaks,    # Positions based on Z-score
    labels = raw_breaks   # Names based on Raw units
  ) +
  labs(
    title = "Main Effect: Ripple Power", 
    x = "Ripple Power (Raw Z)", 
    y = "Partial Effect"
  )
...
# ==============================================================================
# VISUALIZING THE TOTAL RIPPLE EFFECT (Main Effect + ti() Gate)
# ==============================================================================

# 1. Create a dense prediction grid 
# We vary Ripple and Phase, and hold everything else at baseline/dummy values
pred_grid <- expand.grid(
  RipplePower_Z = seq(-2.5, 2.5, length.out = 100),
  SOPhase_Match = seq(0, 2*pi, length.out = 100),
  
  # Hold other variables constant (at 0 / mean)
  SpindlePower_Match_Z = 0,
  SOPhase_NonMatch = pi,
  SessionID = dat_clean$SessionID[1] # Dummy session
)

# 2. Extract the individual terms using predict(type = "terms")
# This returns a matrix where each column is the isolated mathematical contribution of a model term
term_preds <- predict(mdl_final, newdata = pred_grid, type = "terms")

# 3. Add the Main Effect and the Interaction together
# (Check your exact column names if this throws an error, but usually mgcv formats them like this)
pred_grid$Total_Ripple_Effect <- term_preds[, "s(RipplePower_Z)"] + 
  term_preds[, "ti(RipplePower_Z,SOPhase_Match)"]

# 4. Plot the Combined Landscape
p_total_ripple <- ggplot(pred_grid, aes(x = SOPhase_Match, y = RipplePower_Z, fill = Total_Ripple_Effect)) +
  geom_raster(interpolate = TRUE) + 
  geom_contour(aes(z = Total_Ripple_Effect), color = "black", alpha = 0.3, linewidth = 0.3) +
  ...
  scale_x_continuous(breaks = c(0, pi, 2*pi), labels = c("0", expression(pi), expression(2*pi))) +
  theme_minimal() + 
  labs(title = "Total Effect of Ripple Power by SO Phase",
       subtitle = "Main Effect + Phase Gate Combined",
       x = "Match SO Phase (Rad)", 
       y = "Ripple Power (Z)")

dev.new(noRStudioGD = TRUE)
print(p_total_ripple)
