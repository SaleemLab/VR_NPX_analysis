...
cleanTbl.Event_Coherence_Pre_GeoMean = sign(cleanTbl.HPC_logodds_Z .* cleanTbl.V1_logodds_Z) .* sqrt(abs(cleanTbl.HPC_logodds_Z .* cleanTbl.V1_logodds_Z));
...
f = ['Event_Coherence_Post_GeoMean ~ RipplePower_Z + SpindlePower_Match_Z + ', ...
     'SpindlePower_NonMatch_Z + JointState + (1|AnimalID) + (1|SessionID)'];
% f = ['Event_Coherence_Post ~ RipplePower_Z + SpindlePower_Match_Z + ', ...
%      'SpindlePower_NonMatch_Z + JointState + (1|AnimalID) + (1|SessionID)'];
f = ['Event_Coherence_Post_GeoMean ~ RipplePower_Z + (1|AnimalID) + (1|SessionID)'];
...
% Set the number of rows based on your states
numRows = size(cleanTbl,1); 
% Create the simplified table
predTbl = table(zeros(numRows, 1), repmat(cleanTbl.AnimalID(1), numRows, 1), repmat(cleanTbl.SessionID(1), numRows, 1), ...
    'VariableNames', {'RipplePower_Z', 'AnimalID', 'SessionID'});
