...
base_folder = 'Z:\ibn-vision\DATA\SUBJECTS\M24065\analysis\';

ALLDIR = dir(base_folder);

for nsession = 3:17
    cd(ALLDIR(nsession).folder)
    DIR = dir(ALLDIR(nsession).name)

    cd(ALLDIR(nsession).name)
    for nfolder = 1:length(DIR)
        cd(DIR(nfolder).folder)
        if DIR(nfolder).isdir==1 & ~contains(DIR(nfolder).name,'.')
            cd(DIR(nfolder).name)
            TCAT_DIR = dir('*tcat*.lf.meta');
            T0_DIR =  dir('*t0*.lf.meta');
            if isempty(TCAT_DIR) & ~isempty(T0_DIR)

            elseif ~isempty(TCAT_DIR) & ~isempty(T0_DIR) %if both present
                if length(T0_DIR)==length(TCAT_DIR)
                    for nfile = 1:length(T0_DIR)
                        delete(T0_DIR(nfile).name)
                        disp(['delete file: ',T0_DIR(nfile).name])
                    end
                end
            end
...
base_folder = 'Z:\ibn-vision\DATA\SUBJECTS\M24018\ephys\20240723\20240723_9';
...
for nfolder = 1:length(DIR)
    cd(DIR(nfolder).folder)
    if DIR(nfolder).isdir==1 & ~contains(DIR(nfolder).name,'.')
        ephys_folders = dir(DIR(nfolder).name);W
        cd(DIR(nfolder).name)

        for n = 1:length(ephys_folders)
            cd(ephys_folders(n).folder)
            if ephys_folders(n).isdir==1 & ~contains(ephys_folders(n).name,'.')
                cd(ephys_folders(n).name)

                TCAT_DIR = dir('*tcat*.lf*');
                T0_DIR =  dir('*t0*.lf*');
                if isempty(TCAT_DIR) & ~isempty(T0_DIR)
