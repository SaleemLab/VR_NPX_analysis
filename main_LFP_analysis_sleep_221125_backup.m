...
for nsession =1:22
    ...
    if isfield(LFP(probe_no),'best_HPC') % if exist best HPC channels
        ...
        if length(LFP)==1
            lfp_HPC.data= single([LFP(probe_no).best_HPC(:,time_idx)']);
            %             lfp_V1.data= single([LFP(probe_no).best_V1(:,time_idx)']);
            lfp_V1.data= single([LFP(probe_no).average_V1(:,time_idx)']);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % phi_unwrapped = unwrap(lfp.SO_phase_LFP);
    % 
    % % Interpolate LFP phase to the exact spike times
    % spike_phases_unwrapped = interp1(tvec, lfp.SO_phase_LFP, session_clusters.spike_times, 'linear');
    % 
    ...
    for hemi = 1:2
        ...
        tidx = mod(loc, 2) == 1;
        SO_phase_distribution(hemi,:) = histcounts(lfp_V1.SO_phase_LFP(tidx,hemi),phase_bin_edges)./lfp_V1.samplingRate;
        ...
        tidx = mod(loc, 2) == 1;
        SO_phase_distribution_ripples(hemi,:) = histcounts(lfp_V1.SO_phase_LFP(tidx,hemi),phase_bin_edges)./lfp_V1.samplingRate;
    end
