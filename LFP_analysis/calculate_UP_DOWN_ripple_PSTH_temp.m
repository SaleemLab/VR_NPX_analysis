...
%         ripple_peaktimes = min(ripples_all(nprobe).SWR_peaktimes{sessions_to_process(nsession)}(ripples_all(nprobe).probe_hemisphere{sessions_to_process(nsession)} == nprobe,ripples_all(nprobe).SWS_index(ripples_all(nprobe).session_count == sessions_to_process(nsession))==1))';
        ...
        for mprobe = 1
            ...
            % V1_spike_counts{mprobe} = histcounts(spike_times_sleep,tvec_edges);

            spike_times_sleep = [slow_waves_all(1).HPC_MUA_spiketimes{nsession}; slow_waves_all(2).HPC_MUA_spiketimes{nsession}];

            HPC_spike_counts{mprobe} = filtfilt(w,1,histcounts(spike_times_sleep,tvec_edges));
            % HPC_spike_counts{mprobe} = histcounts(spike_times_sleep,tvec_edges);

            if contains(option,'MUA')

                %%%%% HPC MUA
                % HPC MUA UP
                [psth, bins, ~, ~, ~, temp] = psthAndBA([slow_waves_all(1).HPC_MUA_spiketimes{nsession}; slow_waves_all(2).HPC_MUA_spiketimes{nsession}], UP_ints(:,1), time_windows, timebin_size);
                ...
                % HPC MUA DOWN
                [psth, bins, ~, ~, ~, temp] = psthAndBA([slow_waves_all(1).HPC_MUA_spiketimes{nsession}; slow_waves_all(2).HPC_MUA_spiketimes{nsession}], DOWN_ints(:,1), time_windows, timebin_size);
                temp = (temp-mean(HPC_spike_counts{mprobe}(status)))./std(HPC_spike_counts{mprobe}(status));% zscore relative to spike count during sleep
                % temp = filtfilt(w,1,temp);
                ...
                % HPC MUA ripple
                [psth, bins, ~, ~, ~, temp] = psthAndBA([slow_waves_all(1).HPC_MUA_spiketimes{nsession}; slow_waves_all(2).HPC_MUA_spiketimes{nsession}], ripple_peaktimes, time_windows, timebin_size);
                temp = (temp-mean(HPC_spike_counts{mprobe}(status)))./std(HPC_spike_counts{mprobe}(status));% zscore relative to spike count during sleep
                % temp = filtfilt(w,1,temp);
                ...
                if ~isempty(spindle_onset)
                    [psth, bins, ~, ~, ~, temp] = psthAndBA([slow_waves_all(1).HPC_MUA_spiketimes{nsession}; slow_waves_all(2).HPC_MUA_spiketimes{nsession}], spindle_onset, time_windows, timebin_size);
                    temp = (temp-mean(HPC_spike_counts{mprobe}(status)))./std(HPC_spike_counts{mprobe}(status));% zscore relative to spike count during sleep
                % temp = filtfilt(w,1,temp);
...
%     PSTH(nprobe).R_V1_spindles = binnedArraySpindlesV1{2};
    ...
    PSTH(nprobe).HPC_UP = binnedArrayUPHPC{1};
    PSTH(nprobe).HPC_DOWN = binnedArrayDOWNHPC{1};
    PSTH(nprobe).HPC_ripples = binnedArrayRipplesHPC{1};
    PSTH(nprobe).HPC_spindles = binnedArraySpindlesHPC{1};
    ...
    PSTH(nprobe).HPC_UP_bootstrap = tempUP;
    PSTH(nprobe).HPC_DOWN_bootstrap = tempDOWN;
    PSTH(nprobe).HPC_ripples_bootstrap = tempRipples;
    PSTH(nprobe).HPC_spindles_bootstrap = tempSpindles;
    ...
    if contains(shuffle_option,'time_circular_shift')
        ...
        PSTH(nprobe).HPC_UP_shuffled = tempUP;
        PSTH(nprobe).HPC_DOWN_shuffled = tempDOWN;
        PSTH(nprobe).HPC_ripples_shuffled = tempRipples;
        PSTH(nprobe).HPC_spindles_shuffled = tempSpindles;
    end
