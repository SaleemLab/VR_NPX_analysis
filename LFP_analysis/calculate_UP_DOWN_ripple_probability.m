...
% If merging_threshold is 0.05s then left and right ripple that occred
% within 50ms would be treated as one and the earliest ripple would taken
merging_threshold = p.Results.merging_threshold;
...
if isempty(merging_threshold) % if treating L and R ripple separate
    ...
    for nprobe = 1:length(slow_waves_all)
        ...
        for nsession = 1:length(sessions_to_process)
            ...
            ripple_onset = [ripples_all(1).onset(ripples_all(1).session_count == nsession&ripples_all(1).SWS_index==1); ripples_all(2).onset(ripples_all(2).session_count == nsession&ripples_all(2).SWS_index==1)];
            ripple_offset = [ripples_all(1).offset(ripples_all(1).session_count == nsession&ripples_all(1).SWS_index==1); ripples_all(2).offset(ripples_all(2).session_count == nsession&ripples_all(2).SWS_index==1)];
            ripple_peaktimes = [ripples_all(1).peaktimes(ripples_all(1).session_count == nsession&ripples_all(1).SWS_index==1); ripples_all(2).peaktimes(ripples_all(2).session_count == nsession&ripples_all(2).SWS_index==1)];
            ...
            [event_ids_first,event_ids_second] = merge_bilateral_ripple_events(event_id,ripple_onset,0.05);

            if contains(time_option,'peaktimes')
                ripple_times= ripple_peaktimes(event_ids_first);
            else
                ripple_times = [ripple_onset(event_ids_first) ripple_offset(event_ids_first)];
            end

            if contains(option,'normalised')
                % Ripple probability during normalised UP duration
                tic
                [probability(nprobe).ripples_DOWN_session(nsession,:),event_index,normalized_duration,temp] = calculate_relative_event_probability(DOWN_ints,ripple_times,num_bins,0);
                toc
            else
                [probability(nprobe).ripples_DOWN_session(nsession,:),temp,event_index] = calculate_event_probability(ripple_times,DOWN_ints(:,1),time_windows(1):time_bin:time_windows(end),0);
            ...
            if contains(option,'normalised')
                [probability(nprobe).ripples_UP_session(nsession,:),event_index,normalized_duration,temp] = calculate_relative_event_probability(UP_ints,ripple_times,num_bins,0);
            else
                [probability(nprobe).ripples_UP_session(nsession,:),temp,event_index] = calculate_event_probability(ripple_times,UP_ints(:,1),time_windows(1):time_bin:time_windows(end),0);
        ...
        probability(nprobe).ripples_DOWN = binnedArrayDOWN;
        probability(nprobe).ripples_UP = binnedArrayUP;
        probability(nprobe).ripples_DOWN_index = ripple_index_DOWN_all;
        probability(nprobe).ripples_UP_index = ripple_index_UP_all;

        all_ripple_no = sum(ripples_all(1).SWS_index == 1);
        probability(nprobe).ripple_no = all_ripple_no;
        all_UP_no = length(UP_index_all);
        all_DOWN_no = length(DOWN_index_all);
        ...
        probability(nprobe).ripples_DOWN_bootstrap = tempDOWN;
        probability(nprobe).ripples_UP_bootstrap = tempUP;
        ...
        if contains(shuffle_option,'time_circular_shift')
            ...
            probability(nprobe).ripples_DOWN_shuffled = tempDOWN;
            probability(nprobe).ripples_UP_shuffled = tempUP;
        end
    end
