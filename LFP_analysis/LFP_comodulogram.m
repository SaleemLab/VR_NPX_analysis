function [ comod ] = LFP_comodulogram(lfp,specparms,varargin)
...
%Parameters
parms = inputParser;
addParameter(parms,'frange',[1 200],@isnumeric);
addParameter(parms,'nfreqs',200,@isnumeric);
...
addParameter(parms,'noverlap',0.5,@isnumeric);
addParameter(parms,'plot_option',true,@islogical);
% addParameter(parms,'ints',[],@isnumeric);
...
specparms.noverlap = parms.Results.noverlap;
plot_option = parms.Results.plot_option;
% ints = parms.Results.ints;
...
% 
%     for i = 1:length(ints)
%         tindx = FindInInterval(timestamps,[ints(i,1) ints(i,2)],previous);
%         timestamps =  [timestamps  lfp.timestamps(tindx(1):tindx(2))};
% 
%     timestamps = find(timestamps < ints);
%     end
    samplingRate = 1/mean(diff(lfp.timestamps))
...
if plot_option==1  %This whole figure thing can be better.
