function LFP_extraction_and_event_detection_pipeline_ripples(session_info,stimulus_name,best_channels)
...
for nprobe = 1:2
    ...
    channels_selected= channels_selected(ycoord_temp < 4500)
    ...
    if ~isempty(behavioural_state_merged.SWS)
        ...
        % 
        % figure;plot( power{nprobe}(IB,6)*10000)
        ...
        % LFP(probe_no).best_HPC_channel(nshank) = LFP(nprobe).average_V1_channel(channel_id);
        % LFP(probe_no).best_HPC_depth(nshank) = LFP(nprobe).average_V1_depth(channel_id);
        % LFP(probe_no).best_HPC_xcoord(nshank) = LFP(nprobe).average_V1_xcoord(channel_id);
        ...
        [ripples_temp(nprobe)] = FindRipples_masa(LFP(nprobe).best_HPC(2,:),LFP(probe_no).tvec','behaviour',Behaviour,'minDuration',30,'durations',[30 200],'frequency',mean(1./diff(LFP(nprobe).tvec)),...
...
ripples1 = ripples;%backup
clear ripples
ripples = ripples_temp;
