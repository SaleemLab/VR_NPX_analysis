function [ SlowWaves,VerboseOut ] = DetectSlowWaves_masa(varargin)
...
% gammaLFP.timestamps = time;
% gammaLFP.samplingRate = SR;
...
%         CCGvec = [DELTApeaks,ones(size(DELTApeaks));...
%             allspikes,2.*ones(size(allspikes))];
%         [Delta_spike_CCG,t_CCG] = CCG(CCGvec(:,1),CCGvec(:,2),'binSize',0.02,'norm','rate');
...
%     xlim([-0.8 0.8])
%     set(gca,'xticklabel',[]);
%     ylabel('Slow wave peak Rate')
%     xlabel('Relative to spiking')
...
subplot(4,2,1)
    ...
    set(gca,'xticklabel',[]);
    ylabel('Spike Rate');
    xlabel('Relative to Down state (Slow wave Peak)')
...
patch([DOWNints(:,1) DOWNints(:,2) DOWNints(:,2) DOWNints(:,1)]',...
    repmat([y(1) y(1) y(2) y(2)],length(DOWNints(:,1)),1)',...
    'g','FaceAlpha',0.2,'EdgeColor','none');
% plot(deltaLFP.timestamps,gammaLFP.normamp,'g','Linewidth',1)
% hold on
% plot(deltaLFP.timestamps,deltaLFP.normamp,'b','Linewidth',1)
...
function [thresholds,threshfigs] = DetermineThresholds(deltaLFP,gammaLFP,spikes,NREMInts,ratethresh,basePath)
    ...
    if sum(DELTAbox(:))== 0
        ...
        DELTAbox=bwboundaries(DELTAbox);
        DELTAbox = DELTAbox{1};
%         
%         for n = 1:length(DELTAbox)
%             cluster_size(n) = length(DELTAbox{n});
%         end
%         [~,index]   = max(cluster_size);
% 
%         DELTAbox = DELTAbox{index};
...
xwin = [-0.5 0.5];
exampledelta = [1,7,13,20,25,30];
figure
for ee = 1:length(exampledelta)
    subplot(5,3,ee+length(exampledelta))
