...
if contains(stimulus_name,'Masa2tracks')
    ...
    load(fullfile(options.ANALYSIS_DATAPATH,sprintf('extracted_clusters_ks3%s.mat',erase(stimulus_name,'Masa2tracks'))));
...
for nprobe = 1:length(session_info.probe)
    ...
    if ~isempty(behavioural_state_merged.SWS)
        ...
        temp = DetectSlowWaves_masa('time',tvec,'lfp',LFP(probe_no).best_V1(best_channel,:),'spikes',V1_clusters,'NREMInts',behavioural_state_merged.SWS);
