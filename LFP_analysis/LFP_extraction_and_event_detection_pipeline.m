...
%             temp = DetectSlowWaves_masa('time',tvec,'lfp',raw_LFP(good_V1_channels(best_V1_sleep_channel),:),'spikes',V1_clusters,'NREMInts',SWS);
            temp_V1_channels(nprobe).deltaspikecorr = deltaspikecorr;
            temp_V1_channels(nprobe).gammaspikecorr = gammaspikecorr;
            temp_V1_channels(nprobe).deltagammacorr = deltagammacorr;
            temp_V1_channels(nprobe).channel = unique_selected_channels(good_V1_channels);
...
for nprobe = 1:length(session_info.probe)
    ...
    elseif isfield(LFP(nprobe),'best_V1_high_freq')
        ...
        [~,best_channel] = max(LFP(nprobe).L5_power(:,7));% use high freq power channel (more clusters)

        if probe_no == 1 % Left hemisphere (shank 1 most anterior)
            if LFP(probe_no).L5_power(best_channel,7) < 2*LFP(probe_no).L5_power(1,7) % if anterior not bad, use anterior channel
                best_channel = 1;
            end

        elseif probe_no ==2 % Right hemisphere (shank 1 most posterior)
            if LFP(probe_no).L5_power(best_channel,7) < 2*LFP(probe_no).L5_power(end,7) % if anterior not bad, use anterior channel
                best_channel = length(LFP(probe_no).L5_shank_id);
            end
        end
