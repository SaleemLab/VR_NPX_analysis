function [normalised_raw_matrix PPvector shuffled_globalRemap_PPvector shuffled_rateRemap_PPvector] = plot_place_cell_map_correlation_DT(place_fields_all,good_cell_index,Task_info,Behaviour,options)
...
if isempty(good_cell_index)
    ...
    for nplot = 1:4
        ...
        xticks([30 50 70 90 110 130 140]/mean(diff(place_fields_all(1).x_bin_centres)))
        xticklabels([30 50 70 90 110 130 140])
        xline(120/mean(diff(place_fields_all(1).x_bin_centres)),'r')
        xline(135/mean(diff(place_fields_all(1).x_bin_centres)),'b')
