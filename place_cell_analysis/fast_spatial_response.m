function spatial_response = fast_spatial_response(spike_id,cluster_id,spike_times,tvec,position,speed,track_ID_all,start_time_all,end_time_all)
no_cluster = length(cluster_id);
spatial_response = cell(no_cluster,2);
t_bin = mean(diff(tvec));
position_edges = 0:1:140;
...
event_position = zeros(size(start_time_all));
    w = gausswin(9);
    w = w / sum(w);
position_bin_time = zeros(no_lap,140);
for iLap = 1:no_lap
...
for track_id = 1:max(track_ID_all)
    ...
    parfor iCluster = 1:no_cluster
        ...
        [~,~,binnedArray] = spatial_psth(spike_position(cluster_spike_id),temp_event_position, [0,140], 1,position_bin_time(track_ID_all==track_id,:));
        binnedArray(isnan(binnedArray)) = 0;
        binnedArray(isinf(binnedArray)) = 0;
        spatial_response{iCluster,track_id} = filtfilt(w,1,binnedArray')';
    end
end
