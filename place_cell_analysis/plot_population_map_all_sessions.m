function [normalised_raw_matrix] = plot_population_map_all_sessions(place_fields_all,good_cell_index,options)
% function [normalised_raw_matrix PPvector shuffled_globalRemap_PPvector shuffled_rateRemap_PPvector] = plot_population_map_all_sessions(place_fields_all,good_cell_index,options)
...
if isempty(good_cell_index)
    ...
    for type = 1:3
        ...
        for nsession = unique(place_fields_all(1).session_id)
            this_session_good_cell = good_cell_index(place_fields_all(1).session_id(good_cell_index) == nsession);
            
            if isempty(this_session_good_cell)
                continue
            end
            ...
            for track_id = 1:2
                ratemap_matrix{track_id} = [place_fields_all(track_id).raw{this_session_good_cell};];
                ratemap_matrix{track_id} = reshape(ratemap_matrix{track_id},size(place_fields_all(track_id).raw{this_session_good_cell(1)},1),[],length(this_session_good_cell));%laps X position bins X cells
            end
            ...
            average_maps_all{1} = [average_maps_all{1} temp_map(1:size(average_maps{1},1),:)];
           average_maps_all{2}= [average_maps_all{2} temp_map(1+size(average_maps{1},1):size(average_maps{1},1)+size(average_maps{2},1),:)];

        end
        average_maps_all{1}(isnan(average_maps{1}))=0;
        average_maps_all{2}(isnan(average_maps{2}))=0;

        for track_id = 1:2
            normalised_raw_matrix{type}{track_id} = average_maps_all{track_id}';
    ...
    fig = figure
    fig.Position = [500 70 670 500];


    sgtitle(sprintf('%s cell remapping',region))
    fig.Name = sprintf('%s cell remapping',region);
    ...
    fig = figure
    fig.Position = [500 70 670 500];
    sgtitle(sprintf('%s Even vs Odd laps %s cell maps',region))
    fig.Name = sprintf('%s Even vs Odd laps %s cell maps',region);
