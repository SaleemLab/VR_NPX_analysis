function place_fields = calculate_FR_immobility_masa_NPX(x_bins_width,position,clusters,option)
% INPUTS:
...
for track_id = 1:length(position.linear)
    ...
    elseif isfield(option.track(track_id),'lap_id') % if lap id specified
        lap_id = option.track(track_id).lap_id; 
        count = 1;
...
if ~isempty(allclusters_waveform) | ~isempty(clusters.cell_type)
