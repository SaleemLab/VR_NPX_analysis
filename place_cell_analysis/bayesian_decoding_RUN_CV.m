function [decoded_Bayesian_RUN] = bayesian_decoding_RUN_CV(clusters,place_fields_all,Behaviour,Task_info,options,varargin)

% Default values
p = inputParser;
addParameter(p,'time_bin',0.1,@isnumeric)
...
good_cell_index = 1:length(place_fields_BAYESIAN(1).cluster_id);
...
if isempty(good_cell_index)
    ...
    clear decoded_Bayesian_RUN

    decoded_Bayesian_RUN.x_bin_centres = place_fields_BAYESIAN(1).x_bin_centres;
    decoded_Bayesian_RUN.track_id = [];
    decoded_Bayesian_RUN.lap_id = [];
    decoded_Bayesian_RUN.position_index = [];
    decoded_Bayesian_RUN.matrix = [];
    decoded_Bayesian_RUN.speed = [];
    decoded_Bayesian_RUN.tvec = [];
    decoded_Bayesian_RUN.position_log_odds = [];
    decoded_Bayesian_RUN.log_odds = [];
...
%         tic
        ...
        for track_id = 1:length(place_fields_all)
            ...
            bayesian_spike_count_RUN = create_spike_count_masa(place_fields_BAYESIAN,clusters,...
                Task_info.start_time_all(track_laps{track_id}(cv_groups{track_id}{groupIndex}))',Task_info.end_time_all(track_laps{track_id}(cv_groups{track_id}{groupIndex})),[],time_bin);
            ...
            actual_position = [temp_estimated_position(track_id).run_actual_position];
            x = place_fields_BAYESIAN(1).x_bin_centres;
            actual_position = (actual_position - min(x)) / (x(2) - x(1)) + 1;
            actual_position = round(actual_position);   % ensure integer

            actual_speed = temp_estimated_position(track_id).run_speed; 
%             speed_mask = actual_speed > 1;

            RUN_matrix = [temp_estimated_position(1).run; temp_estimated_position(2).run];

            RUN_bias = [];
            for nbin = 1:length(actual_position)
                if  isnan(actual_position(nbin))
                    RUN_bias(1,nbin) = nan;
                    RUN_bias(2,nbin) = nan;
                else
                    RUN_bias(1,nbin) = temp_estimated_position(1).run(actual_position(nbin),nbin);
                    RUN_bias(2,nbin) = temp_estimated_position(2).run(actual_position(nbin),nbin);
                end
            end
            decoded_Bayesian_RUN.position_log_odds = [decoded_Bayesian_RUN.position_log_odds log(RUN_bias(1,:)./RUN_bias(2,:))];

            decoded_Bayesian_RUN.log_odds = [decoded_Bayesian_RUN.log_odds log(sum(temp_estimated_position(1).run)./sum(temp_estimated_position(2).run))];

            actual_track_id = [track_id*ones(1,length(actual_position))];
            decoded_Bayesian_RUN.track_id = [decoded_Bayesian_RUN.track_id, actual_track_id];


            selected_laps = track_laps{track_id}(cv_groups{track_id}{groupIndex})';
             bayesian_spike_count_RUN.lap_indices(end)=[];
            temp = nan(size( bayesian_spike_count_RUN.lap_indices));
            valid = ~isnan( bayesian_spike_count_RUN.lap_indices);
            temp(valid) = selected_laps( bayesian_spike_count_RUN.lap_indices(valid));

            decoded_Bayesian_RUN.lap_id = [decoded_Bayesian_RUN.lap_id, temp];

            decoded_Bayesian_RUN.position_index = [decoded_Bayesian_RUN.position_index actual_position];
            decoded_Bayesian_RUN.matrix = [decoded_Bayesian_RUN.matrix RUN_matrix];
            decoded_Bayesian_RUN.speed = [decoded_Bayesian_RUN.speed actual_speed];

            decoded_Bayesian_RUN.tvec = [decoded_Bayesian_RUN.tvec, bayesian_spike_count_RUN.run_time_centered];
            % first track is track to decode and second track is place field template track
            %             estimated_position_lap_CV(track_id).lap(cv_groups{track_id}{groupIndex}(nlap)).track = temp_estimated_position;
