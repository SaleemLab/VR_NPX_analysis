function decoding_performance = plot_within_session_decoded_error_HPC_V1(decoded_position_lap_id,VR_speed,actual_position,estimated_position_lap_CV_V1,decoded_position_V1,decoded_position_HPC,decoded_position_HPC_combined,decoded_position_V1_combined,...
    decoded_error_V1,decoded_error_HPC,decoded_error_HPC_combined,decoded_error_V1_combined,place_fields,options,nsession)
...
if length(options.probe)>1
    ...
    decoding_performance.confusion_matrix.HPC_combined = confusion_matrix;
    ...
    for track_id = 1:length(place_fields)
        for nbin =1:length(position_bin)
            true_pos = actual_position{nsession}{track_id};
            confusion_matrix{track_id}(:,nbin) = histcounts(decoded_position_HPC{probe_hemisphere}{nsession}{track_id}(true_pos == position_bin(nbin)...
                & VR_speed{nsession}{track_id}>5),length(position_bin_across_tracks));
...
for nprobe = 1:length(options.probe)
    ...
    for track_id = 1:length(place_fields)
        for nbin =1:length(position_bin)
            true_pos = actual_position{nsession}{track_id};
            confusion_matrix{probe_hemisphere}{track_id}(:,nbin) = histcounts(decoded_position_HPC{probe_hemisphere}{nsession}{track_id}(true_pos == position_bin(nbin)...
                & VR_speed{nsession}{track_id}>5),length(position_bin_across_tracks));
    ...
    for track_id = 1:length(place_fields)
        for temp_track = 1:length(place_fields)
            for lap_id = 1:length(estimated_position_lap_CV_V1(nprobe).track(track_id).lap)
                ...
                median_lap_decoding_error{track_id}{temp_track}(lap_id) = nanmedian(decoded_error_HPC{probe_hemisphere}{nsession}{track_id}{temp_track}...
                    (decoded_position_lap_id{nsession}{track_id} == lap_id & VR_speed{nsession}{track_id}>5));
            end
            %                 scatter(median_lap_decoding_error{track_id}{temp_track})
        end
