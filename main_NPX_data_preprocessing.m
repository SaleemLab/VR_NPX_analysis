...
%% import and align and store Bonsai and spike data
...
for nsession =1:length(experiment_info)
    ...
    for n = 1:length(session_info) % just in case there might be multiple recording for the same stimulus type (e.g. PRE RUN POST Masa2tracks)
        ...
        if contains(Stimulus_type,'Masa2tracks')
            ...
            if exist((fullfile(ROOTPATH,'DATA','SUBJECTS',options.SUBJECT,'analysis',options.SESSION,Stimulus_type))) == 0
                mkdir(fullfile(ROOTPATH,'DATA','SUBJECTS',options.SUBJECT,'analysis',options.SESSION,Stimulus_type))
            end

            save(fullfile(ROOTPATH,'DATA','SUBJECTS',options.SUBJECT,'analysis',options.SESSION,Stimulus_type,...
            ...
            save(sprintf('extracted_behaviour%s.mat','Behaviour'))
            save(sprintf('extracted_Task_info%s.mat','Task_info'))
            save(sprintf('extracted_peripherals%s.mat','Peripherals'))
        end
