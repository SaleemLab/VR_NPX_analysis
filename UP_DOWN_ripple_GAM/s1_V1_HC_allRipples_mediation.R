...
my_folder <- "C:/Users/masah/Documents/GitHub/VR_NPX_analysis/UP_DOWN_ripple_GAM/V1_HC_allRipples_mediation"
...
# 1. MEDIATOR MODEL: Pre-V1 -> HC bias
model_m <- bam(HC_post_z ~ 
                 s(V1_pre_z, k = 5) + 
                 s(AnimalID, bs = "re") + 
                 s(SessionID, bs = "re"),
               data = dat_clean, 
...
# 2. OUTCOME MODEL: Pre-V1 + HC bias -> Post-V1
model_y <- bam(V1_post_z ~ 
                 s(HC_post_z, k = 5) + 
                 s(V1_pre_z, k = 5) + 
                 s(AnimalID, bs = "re") + 
                 s(SessionID, bs = "re"),
...
# 3. SET CONTRAST POINTS (25th vs 75th percentile of Pre-V1)
v1_low  <- quantile(dat_clean$V1_pre_z, 0.25, na.rm = TRUE)
v1_high <- quantile(dat_clean$V1_pre_z, 0.75, na.rm = TRUE)
# Tells mgcv to use 4 CPU cores for processing openMP loops
options(mc.cores = 4)

# 4. RUN MEDIATION (Starts simulations using your GAM smooths)
med_output <- mediate(model_m, model_y, 
                      treat = "V1_pre_z", 
                      mediator = "HC_post_z",
                      control.value = v1_low, 
                      treat.value = v1_high,
                      boot = TRUE, 
                      sims = 10) # Start with 500 to check speed; bump to 1000+ later

# View the final mediation metrics (ACME, ADE, Total Effect)
summary(med_output)
plot(med_output)

library(broom)

# Turn the summary into a clean data frame (coefficients, p-values, etc.)
model_table <- tidy(med_output)

# Save as a CSV file you can open in Excel
write.csv(model_table, "mediation_model_results.csv", row.names = FALSE)

# Open a file connection
sink("model_summary.txt")

# Print the summary (this will go into the text file, not the console)
summary(med_output)

# Crucial step: Close the connection so R stops writing to the file
sink()
