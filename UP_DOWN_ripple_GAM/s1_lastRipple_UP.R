...
plot_data <- raw_iterations_clean %>%
  ...
  mutate(
    ...
    Term = case_when(
      Term == "s(lastRippleHPC_z):DOWN_no"  ~ "Near DOWN: No (>100ms away)",
      Term == "s(lastRippleHPC_z):DOWN_yes" ~ "Near DOWN: Yes (<100ms away)",
      TRUE ~ as.character(Term)
    ),
    Term = factor(Term, levels = c(
      "Near DOWN: Yes (<100ms away)", "Near DOWN: No (>100ms away)"
    ))
  )
