...
# --- 3. Clean Data ---
z_thresh <- 3.50  # include <99.9th centiles of data
...
dat_clean$is_near_DOWN <- ifelse(dat_clean$TimefromLastRipple <= 0.101, "yes", "no")
...
p_HC_raw <- draw(mdl_lastRipple, select = "s(lastRippleHPC_z):is_near_DOWNno", residuals = FALSE, rug = FALSE) + 
  ...
  # coord_cartesian(xlim = c(-2.5,2.5),ylim = c(-0.16,0.16)) +
  coord_cartesian(xlim = c(-3.8,3.8), ylim = c(-0.21, 0.21),expand = FALSE) +
  scale_x_continuous(breaks = z_breaks, labels = raw_breaks) +
  
  labs(
    title = "HC ripple bias predicting post V1 track bias (last ripple >100ms away from DOWN)", 
    x = "HC bias", 
    y = "Partial Effect"
  )
...
p_V1PRE_raw <- draw(mdl_lastRipple, select = "s(lastRippleV1PRE_z):is_near_DOWNno", residuals = FALSE, rug = FALSE) + 
  theme_bw(base_family = "Arial") + 
  theme(aspect.ratio = 1) +
  scale_x_continuous(breaks = z_breaks, labels = raw_breaks) +
  coord_cartesian(xlim = c(-3.8,3.8), ylim = c(-0.21, 0.21), expand = FALSE) +
  labs(
    title = "V1 PRE ripple bias predicting post V1 track bias (last ripple >100ms away from DOWN)", 
    x = "V1 PRE bias", 
    y = "Partial Effect"
  )
cairo_pdf("lastRippleV1PRE_away_from_DOWN.pdf", width = 4.3, height = 4.3)
print(p_V1PRE_raw)
dev.off()

# ---------------------------------------------------------
# Plot 4: V1PRE near DOWN
# ---------------------------------------------------------
raw_breaks <- c(-2,-1,0,1,2)
z_breaks <- sapply(raw_breaks, function(val) {
  dat_clean$lastRippleV1PRE_z[which.min(abs(dat_clean$lastRippleV1PRE - val))]
...
p_V1PRE_raw <- draw(mdl_lastRipple, select = "s(lastRippleV1PRE_z):is_near_DOWNyes", residuals = FALSE, rug = FALSE) + 
  theme_bw(base_family = "Arial") + 
  theme(aspect.ratio = 1) +
  scale_x_continuous(breaks = z_breaks, labels = raw_breaks) +
  coord_cartesian(xlim = c(-3.8,3.8), ylim = c(-0.21, 0.21), expand = FALSE) +
...
cairo_pdf("lastRippleV1PRE_close_to_DOWN.pdf", width = 4.3, height = 4.3)
print(p_V1PRE_raw)
dev.off()
...
term_preds <- predict(mdl_lastRipple, newdata = pred_grid_total, type = "terms")
...
cairo_pdf("lastRippleV1PRE_and_HPC_combined_UP_close_to_DOWN.pdf", width = 4.3, height = 4.3)
print(p_combined_yes)
dev.off()
...
message("\nCalculating 4 Effect Size Metrics for mdl_lastRipple (This may take a minute)...")
...
run_one_bootstrap <- function(rep_id, original_data, base_terms, formula_terms, smooth_labels) {
  ...
  for (j in seq_along(smooth_labels)) {
    ...
    r_form <- as.formula(paste("lastRippleV1_z ~", paste(act_terms, collapse = " + ")))
    mdl_r <- tryCatch({
...
write.csv(raw_iterations_clean, "LastRippleV1_GAM_model_raw_bootstrap_iterations.csv", row.names = FALSE)
...
cairo_pdf("LastRippleV1_Model_Effect_Sizes_With_CI.pdf", width = 10, height = 6.5)
print(p_bars_with_ci)
dev.off()
...
write.csv(combined_results, "LastRippleV1_GAM_model_CI_output.csv", row.names = FALSE)
message("\nAnalysis Pipeline Complete! Saved 'LastRippleV1_GAM_model_CI_output.csv' and figures.")
