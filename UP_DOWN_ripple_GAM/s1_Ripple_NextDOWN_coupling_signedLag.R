...
# library(ggplot2)
# 
# ggplot(dat_clean1, aes(x = nextDOWNlag)) +
#   geom_histogram(fill = "steelblue", color = "white", bins = 30) +
#   theme_minimal() +
#   labs(title = "Histogram of nextDOWNlagSigned",
#        x = "nextDOWNlagSigned",
#        y = "Count")

###
### Next DOWN lag
mdl_final <- bam(nextDOWNlagSigned ~ 
...
p_lag_raw <- draw(mdl_final, select = "s(lastRipplePower_z)", residuals = FALSE, rug = FALSE) + 
  ...
  labs(
    title = "Ripple power effect on nextUP V1 SO lag", 
    x = "Ripple power", 
    y = "Partial Effect"
  )
...
p_lag_raw <- draw(mdl_final, select = "s(TimefromLastRipple_z)", residuals = FALSE, rug = FALSE) + 
  ...
  labs(
    title = "TimefromLastRipple on next DOWN lag (only when both in UP)", 
    x = "TimefromLastRipple", 
    y = "Partial Effect"
  )
dev.new(noRStudioGD = TRUE)
# cairo_pdf("TimefromLastRipple_predicts_DownSOLag_SignedLeading.pdf", width = 4.3, height = 4.3)
print(p_lag_raw)
dev.off()
...
# cairo_pdf("TimefromLastRipple_and_lastRipplePower_ti_predicts_DownLag_SignedLeading.pdf", width = 4.3, height = 4.3)
...
p_interaction_raw_scale <- ggplot(pred_grid_reconstruct, aes(x = TimefromLastRipple, y = lastRipplePower , fill = Reconstructed_effect)) +
  ...
  theme(aspect.ratio = 1) +
  labs(
    title = "Total effect last ripple power and time from last ripple -> next DOWN lag (Lagging)",
    # subtitle = "",
    y = "lastRipplePower_z (Raw)", 
    x = "TimefromLastRipple_z (Raw)"
  )
dev.new(noRStudioGD = TRUE)
# cairo_pdf("TimefromLastRipple_and_lastRipplePower_total_effect_predicts_DownLag_SignedLagging.pdf", width = 4.3, height = 4.3)
print(p_interaction_raw_scale)
# dev.off()
...
p_interaction_raw_scale <- ggplot(pred_grid_reconstruct, aes(x = TimefromLastRipple, y = lastRipplePower , fill = Reconstructed_effect)) +
  ...
  theme(aspect.ratio = 1) +
  labs(
    title = "Total effect last ripple power and time from last ripple -> next DOWN lag (Leading)",
    # subtitle = "",
    y = "lastRipplePower_z (Raw)", 
    x = "TimefromLastRipple_z (Raw)"
  )
dev.new(noRStudioGD = TRUE)
# cairo_pdf("TimefromLastRipple_and_lastRipplePower_total_effect_predicts_DownLag_SignedLeading.pdf", width = 4.3, height = 4.3)
print(p_interaction_raw_scale)
