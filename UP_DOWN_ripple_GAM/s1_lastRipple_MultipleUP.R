...
my_folder <- "C:/Users/masah/Documents/GitHub/VR_NPX_analysis/UP_DOWN_ripple_GAM/MultipleUP_lastRipple"
...
###### Previous
mdl_multipleUP <- bam(curr_lastRippleV1_z ~
                        # prev_is_near_DOWN +
...
cairo_pdf("currentUP_last_ripple_nearDOWN.pdf", width = 4.3, height = 4.3)
p_HC_raw <- draw(mdl_multipleUP, select = "s(curr_lastRippleHPC_z):curr_is_near_DOWNyes", residuals = FALSE, rug = FALSE) + 
  ...
  coord_cartesian(ylim = c(-0.5, 0.5)) + # Forces uniform y-axis scaling
  labs(
    title = "last ripple near DOWN -> last ripple V1", 
    x = "HC bias", 
    y = "Partial Effect"
  )
...
cairo_pdf("currentUP_last_ripple_awayDOWN.pdf", width = 4.3, height = 4.3)
p_HC_raw <- draw(mdl_multipleUP, select = "s(curr_lastRippleHPC_z):curr_is_near_DOWNno", residuals = FALSE, rug = FALSE) + 
  ...
  labs(
    title = "last ripple not near DOWN -> last ripple V1", 
    x = "HC bias", 
    y = "Partial Effect"
  )
# dev.new(noRStudioGD = TRUE)
...
# This mirrors the quantile strategy to allow isolated single-level drops
...
# Parametric baseline controls kept in every single model refit
BASE_TERMS <- c(
  "prev_is_near_DOWN",
  "curr_is_near_DOWN"
)

# Macro components tested/dropped EXACTLY one at a time via negative indexing [-j]
formula_terms <- c(
...
run_one_bootstrap <- function(rep_id, original_data, base_terms, formula_terms, smooth_labels) {
  ...
  for (j in seq_along(smooth_labels)) {
    ...
    # A. Corrected Partial Deviance: Drop ONLY the single targeted level j (Quantile Strategy)
    ...
    # Parse target components out dynamically
    is_prev  <- grepl("prev_lastRippleHPC_z", target_term)
    v_smooth <- if(is_prev) "prev_lastRippleHPC_z" else "curr_lastRippleHPC_z"
    v_dummy  <- if(is_prev) {
...
plot_data <- raw_iterations_clean %>%
  ...
  mutate(
    ...
    # Clean up labels for presentation graph
    Term = case_when(
      Term == "s(prev_lastRippleHPC_z):prev_DOWN_no"  ~ "Prev Smooth (Near DOWN: No)",
      Term == "s(prev_lastRippleHPC_z):prev_DOWN_yes" ~ "Prev Smooth (Near DOWN: Yes)",
      Term == "s(curr_lastRippleHPC_z):curr_DOWN_no"  ~ "Curr Smooth (Near DOWN: No)",
      Term == "s(curr_lastRippleHPC_z):curr_DOWN_yes" ~ "Curr Smooth (Near DOWN: Yes)",
      TRUE ~ as.character(Term)
    ),
    Term = factor(Term, levels = c(
      "Curr Smooth (Near DOWN: Yes)", "Curr Smooth (Near DOWN: No)",
      "Prev Smooth (Near DOWN: Yes)", "Prev Smooth (Near DOWN: No)"
    ))
  )

# Build Horizontal Bar Plot Dashboard
p_bars_with_ci <- ggplot(plot_data, aes(x = Term, y = Val, fill = Term)) +
  ...
  theme_bw() +
  labs(
    title = "mdl_multipleUP Effect Size Metrics (Isolated Dummy Matrix Fix)",
...
# Export Summary Spreadsheet Matching Core Model Outputs
...
# Clean up baseline summary keys before linking matrices together
...
message("\nAnalysis Pipeline Complete! Saved 'MultipleUP_GAM_model_CI_output.csv'.")
