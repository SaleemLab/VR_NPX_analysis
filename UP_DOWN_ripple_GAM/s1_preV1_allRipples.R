...
my_folder <- "C:/Users/masah/Documents/GitHub/VR_NPX_analysis/UP_DOWN_ripple_GAM/preV1_HC_allRipples"
...
print(summary(mdl_preV1))

# ---------------------------------------------------------
# Plot 1: Main Effect of V1_pre_z
# ---------------------------------------------------------
...
cairo_pdf("pre V1 predicts HC bias.pdf", width = 4.3, height = 4.3)
...
p_preV1_raw <- draw(mdl_preV1, select = "s(V1_pre_z)", residuals = FALSE, rug = FALSE) + 
  theme_bw(base_family = "Arial") + 
  theme(aspect.ratio = 1) +
  scale_x_continuous(breaks = z_breaks, labels = raw_breaks) +
  labs(
    title = "V1 pre-ripple bias predicting HC bias", 
...
model_stats <- as.data.frame(summary(mdl_preV1)$s.table) %>%
