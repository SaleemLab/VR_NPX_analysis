...
########
######## first ripple HC content predicts V1 content
########
...
dat_clean$is_near_DOWN <- ifelse(dat_clean$TimefromLastRipple < 0.178, "yes", "no")
...
dat_clean$log_TimefromLastRipple_z <- scale(dat_clean$log_TimefromLastRipple)
...
mdl_lastRipple <- bam(lateUPV1_z ~ 
                        ...
                        # s(lateUPHPC_z, k = 5) +
                        # is_near_DOWN +
                        # s(lateUPHPC_z, by = is_near_DOWN)+

                        ripple_quartile +
                        s(lateUPHPC_z, by = ripple_quartile)+
                        # 4. Control Term
                        s(AnimalID, bs = "re") +
                        s(SessionID, bs = "re"), 
...
mdl_lastRipple <- bam(lastRippleV1_z ~ 
                  ...
                  ripple_quartile +
                  s(lastRippleHPC_z, by = ripple_quartile)+

                  # s(lastRippleHPC_z, bs = "tp",k = 5) +  
                  # ti(lastRippleHPC_z,log_TimefromLastRipple_z,k=5,bs = c("tp", "tp"))+
                  # 
