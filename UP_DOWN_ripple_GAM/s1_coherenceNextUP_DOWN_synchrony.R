...
# z_cols <- c(
#   "firstRipplePower_z","lastRipplePower_z"
# )
z_thresh = 3.5
z_cols <- c(
  ...
  "TimefromFirstRipple_z","firstRipplePower_z",
  "TimefromLastRipple_z","lastRipplePower_z"
)
...
### last ripple power -> last ripple - next UP coherence
...
p_lag_raw <- draw(mdl_final, select = "s(lastRipplePower_z)", residuals = FALSE, rug = FALSE) + 
  ...
  labs(
    title = "Ripple power on nextUP coherence", 
    x = "Ripple power", 
    y = "Partial Effect"
  )
...
#   theme(aspect.ratio = 1) +
#   scale_x_continuous(breaks = z_breaks, labels = raw_breaks) +
#   coord_cartesian(xlim = c(-1.3,2.6),ylim=c(-0.12,0.14),expand = FALSE) + 
#   labs(
...
pred_grid_ti <- expand.grid(
  ...
  TimefromLastRipple_z = 0,   # Constant mean for other model variables
