...
# --- 3. Clean Data ---
my_folder <- "C:/Users/masah/Documents/GitHub/VR_NPX_analysis/UP_DOWN_ripple_GAM/coherenceNextUP"
...
p_lag_raw <- draw(mdl_final, select = "s(lastRippleMUArate_z)", residuals = FALSE, rug = FALSE) +
  ...
  # coord_cartesian(xlim = c(-1.3,2),ylim=c(-0.1,0.06),expand = FALSE) +
  labs(
    title = "Ripple HC MUA rate on last ripple to nextUP coherence",
    x = "Ripple HC MUA",
    y = "Partial Effect"
  )
# dev.new(noRStudioGD = TRUE)
...
#   theme(aspect.ratio = 1) +
#   scale_x_continuous(breaks = z_breaks, labels = raw_breaks) +
#   coord_cartesian(xlim = c(-0.911, 0.553),ylim = c(-0.1, 0.14),expand = FALSE) + 
#   labs(
#     title = "TimefromLastRipple on nextUP V1", 
#     x = "TimefromLastRipple", 
#     y = "Partial Effect"
#   )
# cairo_pdf("TimefromLastRipple_lastRippleNextUPCoherence.pdf", width = 4.3, height = 4.3)
# # dev.new(noRStudioGD = TRUE)
# print(p_lag_raw)
# dev.off()
# print(p_lag_raw)
...
# # Mean and SD of your un-standardized log variable
# mean_log <- mean(dat_clean1$log_TimefromLastRipple, na.rm = TRUE)
# sd_log   <- sd(dat_clean1$log_TimefromLastRipple, na.rm = TRUE)
# 
# # If your log variable is natural log (ln), convert raw_breaks to ln first:
# raw_breaks_ln <- log(10^raw_breaks) # -6.90, -4.60, -2.30, 0.00
# 
# # Calculate exact theoretical z-scores for those breaks
# z_breaks <- (raw_breaks_ln - mean_log) / sd_log
...
p_lag_raw <- draw(mdl_final, select = "s(log_TimefromLastRipple_z)", residuals = FALSE, rug = FALSE) + 
  ...
  coord_cartesian(xlim = c(-2.8, 1.4),ylim = c(-0.1, 0.19),expand = FALSE) +
...
# 
# # 1. Define sequence ranges using your actual RAW data minimums and maximums
# raw_range_so   <- seq(1, 3.5, length.out = 100)
...
# 
# # ==============================================================================
# # --- 2D Ti of next DOWN lag and last ripple  ---
# # ==============================================================================
# 
# # 1. Define sequence ranges using your actual RAW data minimums and maximums
# raw_range_lag  <- seq(0, 0.5, length.out = 100)
...
#   lastRipplePower        =raw_range_ripplePower,
#   TimefromLastRipple_z           = 0,  # other held at 0
#   nextDOWNSOPower           = 0,  # other held at 0
# 
#   AnimalID           = dat_clean1$AnimalID[1],  # Constant random effect
#   SessionID          = dat_clean1$SessionID[1]   # Constant random effect
# )
...
# # --- CRITICAL FIX HERE ---
# # mgcv drops the spaces and the 'k=5' syntax in the term name matrix column!
# pred_grid_te$Interaction_Effect <- term_preds[, "ti(lastRipplePower_z,nextDOWNlag_z)"]
# 
# # 5. Plot Total Tensor Surface using RAW scales for X and Y axes
# p_interaction_raw_scale <- ggplot(pred_grid_te, aes(x = nextDOWNlag, y = lastRipplePower, fill = Interaction_Effect)) +
#   geom_tile() +
#   geom_contour(aes(z = Interaction_Effect), color = "black", alpha = 0.2) +
#   scale_fill_gradient2(low = "dodgerblue", mid = "white", high = "firebrick", midpoint = 0, name = "Effect") +
#   theme_minimal() +
