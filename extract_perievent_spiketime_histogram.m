function PSTH = extract_perievent_spiketime_histogram(all_spike_data,events,varargin)
...
% Default values
p = inputParser;
addParameter(p,'group','by cell zscore',@isstr) % 
    ...
    switch group
        ...
        case 'by cell zscore'
            ...
            for n = 1:size(all_spike_data,2)
                ...
                mean_FR(peak_FR<1) = [];
                peak_latency(peak_FR<1) = [];
                cell_id(peak_FR<1) = [];
                cell_psth(peak_FR<1,:) = [];
                cell_spike_counts(peak_FR<1,:,:) = [];
                ...
                switch group
                    ...
                    otherwise
                        PSTH(n).MUA_psth = psth;
                        PSTH(n).cell_psth = cell_psth;
                        PSTH(n).spike_count = cell_spike_counts;
                end
                PSTH(n).group = group;
                PSTH(n).group_name = group_name{n};
                PSTH(n).event_name = event_name;
                PSTH(n).peak_latency = peak_latency;
                PSTH(n).mean_FR = mean_FR;
