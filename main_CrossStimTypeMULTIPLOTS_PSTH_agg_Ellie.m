%%% ERB 2025 For analysis of unit spiking in response to GAVNIK visual stimuli
...
analysis_dir = fullfile(subjects_dir, SUBJECTS{1}, 'analysis');
cd(analysis_dir) % 3/6 files will be saved here in the cd
...
elseif contains(Stimulus_types{2}, 'GAVNIK_A_CD') || contains(Stimulus_types{2}, 'GAVNIK250_A_CD')
    stimulus_colors = [
        0.0,  0.0,  1.0;  % 1st group - blue - GAVNIK_ABCD
...
stim_onset_response_calc_begins  = 0.03; % calc peak onset response during 30-75ms after stimulus onset
stim_onset_response_calc_ends = 0.080; % calc peak onset response during 30-75ms after stimulus onset

if contains(Stimulus_types{1}, 'GAVNIK_') || contains(Stimulus_types{1}, 'GAVNIK ') 
...
%initiate struct to save peak firing rate following onset of element 2
FRomit_summary = struct([]);
entry_counter = 0;

for nsession = sessions_to_plot 
    for stim_idx = 1:length(Stimulus_types)
        Stimulus_type = Stimulus_types{stim_idx};
        stimulus = regexp(Stimulus_type, '(ABCD|A_CD|E_CD)$', 'match', 'once');
        ...
        for n = 1:length(session_info) 
            ...
            if contains(Stimulus_types{2}, 'GAVNIK DCBA') || contains(Stimulus_types{2}, 'GAVNIK250 DCBA')  % only make this plot if GAVNIK DCBA is present
                ...
                if contains(plot_type, 'FR')
                    ...
                    elseif (contains(Stimulus_types{stim_idx}, 'GAVNIK250_ABCD')) % to label the plot only once
                        xline(0, 'k', (sprintf('A %d%s or D onset', round(ordered_oris(1)), char(176))), 'LabelVerticalAlignment','top', 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off', 'FontSize', 24);
                        xline(0.25, 'k', (sprintf('B %d%s or C onset', round(ordered_oris(2)), char(176))), 'LabelVerticalAlignment','top', 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off', 'FontSize', 24);
                        xline(0.50, 'k', (sprintf('C %d%s or B onset', round(ordered_oris(3)), char(176))), 'LabelVerticalAlignment','top', 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off', 'FontSize', 24);
                        xline(0.75, 'k', (sprintf('D %d%s or A onset', round(ordered_oris(4)), char(176))), 'LabelVerticalAlignment','top', 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off', 'FontSize', 24);     
                    end
            ...
            if contains(Stimulus_types{2}, 'GAVNIK_A_CD') || contains(Stimulus_types{2}, 'GAVNIK250_A_CD') % only make this plot if GAVNIK_A_CD is present
                ...
                if contains(plot_type, 'FR')
                    for nprobe = 1:length(clusters)
                        ...
                        FRomit_summary(entry_counter).session_number = nsession;
                        FRomit_summary(entry_counter).day = experiment_info(nsession).date;
                        FRomit_summary(entry_counter).stimulus = stimulus;
                        ...
                        FRomit_summary(entry_counter).stim_onset_response_calc = ...
                    ...
                    elseif (contains(Stimulus_types{stim_idx}, 'GAVNIK250_ABCD')) % to label the plot only once
                        xline(0, 'k', (sprintf('A %d%s or E novel onset', round(ordered_oris(1)), char(176))), 'LabelVerticalAlignment','top', 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off', 'FontSize', 24);
                        xline(0.25, 'k', (sprintf('B %d%s or grey onset', round(ordered_oris(2)), char(176))), 'LabelVerticalAlignment','top', 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off', 'FontSize', 24);
...
%save struct
if contains(Stimulus_types{2}, 'GAVNIK_A_CD') || contains(Stimulus_types{2}, 'GAVNIK250_A_CD') 
    save(fullfile(subjects_dir, ...
        ['FRomit_summary_' depth_for_analysis '_' SUBJECTS{1} '_' Stimulus_types{1} '_' num2str(experiment_info(nsession).date) '.mat']), ...
        'FRomit_summary');
end    