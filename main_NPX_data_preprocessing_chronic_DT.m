...
for n = 1:length(all_SUBJECTS)
    ...
    stimuli_info = readtable('Z:\ibn-vision\USERS\Diao\ephys_recording_info\session_stimuli_table_DT','Sheet',all_SUBJECTS{n});
...
%             end
            ...
            meta_this_session = ReadMeta(fullfile(experiment_info(nsession).session(nstimuli).probe(1).EPHYS_DATAPATH,bin_DIR.name));
                ...
                bin_DIR = dir(fullfile(experiment_info(nsession).session(nstimuli).probe(1).EPHYS_DATAPATH,'*.ap.meta'));
                meta_this_session = ReadMeta(fullfile(experiment_info(nsession).session(nstimuli).probe(1).EPHYS_DATAPATH,bin_DIR.name));
            end
            ...
            if length(experiment_info(nsession).session(nstimuli).probe)==2
                ...
                meta_this_session = ReadMeta(fullfile(experiment_info(nsession).session(nstimuli).probe(2).EPHYS_DATAPATH,bin_DIR.name));
                    ...
                    bin_DIR = dir(fullfile(experiment_info(nsession).session(nstimuli).probe(2).EPHYS_DATAPATH,'*.ap.meta'));
                    meta_this_session = ReadMeta(fullfile(experiment_info(nsession).session(nstimuli).probe(2).EPHYS_DATAPATH,bin_DIR.name));
                end
    ...
    writetable(stimuli_info,'Z:\ibn-vision\USERS\Diao\ephys_recording_info\session_stimuli_table_DT.xlsx','Sheet',all_SUBJECTS{n});
...
load(fullfile(base_folder,SUBJECTS{1},'analysis','experiment_info.mat'))
% experiment_info = subject_session_stimuli_mapping(SUBJECTS,options);
for nsession =4
...
%     for nStim = 1:length(session_info)
%         bonsai_files_names = session_info(nStim).probe(1).bonsai_files_names; 
%         BONSAI_DATAPATH = session_info(nStim).probe(1).BONSAI_DATAPATH;
...
%         async_missing(nStim) = sum(peripherals.Sync>0) <=0;
%     end
%     if sum(async_missing) > 0
    extract_and_preprocess_NPX(session_info,Stimulus_type,1)
%     end
end
end
%% LFP preprocessing
