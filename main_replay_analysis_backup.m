...
%% Candidate event detection + HPC reactivation detection
clear all
addpath(genpath('Z:\ibn-vision\USERS\Masa\code'))
    ...
    for nsession =1:length(experiment_info)
        ...
        for n = 1:length(session_info) % How many recording sessions for spatial tasks (PRE, RUN and POST)
            ...
            for nprobe = 1:length(session_info(n).probe)
                ...
                %             speed_interp = interp1(Behaviour.sglxTime,Behaviour.speed,tvec','linear');
                %             speedTreshold = 1;
                %
                ...
                %                 [tvec' raw_LFP{nprobe}(find(sorted_config.Channel == cortex_channel),:)'],[tvec' raw_LFP{nprobe}(find(sorted_config.Channel == CA1_channel),:)'],...
            ...
            save(sprintf('extracted_ripples_events%s.mat',erase(stimulus_name{n},'Masa2tracks')),'ripples')
            ...
            if length(session_info(n).probe) == 1 % if one probe, but it can be probe 2
                ...
                if isempty(reactivations.probe(1).onset)
            ...
            timebin =[];
            [place_fields_BAYESIAN,decoded_replay_events,probability_ratio_original] = log_odds_reactivation_analysis(clusters,place_fields_BAYESIAN,replay,position,[],stimulus_name{n},timebin)
            [place_fields_BAYESIAN,decoded_replay_events_global_remapped,probability_ratio_global_remapped] = log_odds_reactivation_analysis(clusters,place_fields_BAYESIAN,replay,position,'global_remapped',stimulus_name{n},timebin)
            save(sprintf('probability_ratio_original%s.mat',erase(stimulus_name{n},'Masa2tracks')),'probability_ratio_original');
            save(sprintf('decoded_replay_events%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events');
            save(sprintf('probability_ratio_global_remapped%s.mat',erase(stimulus_name{n},'Masa2tracks')),'probability_ratio_global_remapped');
            save(sprintf('decoded_replay_events_global_remapped%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events_global_remapped');
            if length(session_info(n).probe)> 1
                HPC_place_fields_combined = place_fields_BAYESIAN;
                %         save('extracted_HPC_place_fields_combined','HPC_place_fields_combined');
                save(sprintf('extracted_HPC_place_fields_combined%s.mat',erase(stimulus_name{n},'Masa2tracks')),'HPC_place_fields_combined')
            else
                probe_no = session_info(n).probe(1).probe_id + 1;
                HPC_place_fields.probe(probe_no) = place_fields_BAYESIAN;
                save('extracted_HPC_place_fields','HPC_place_fields');
                save(sprintf('extracted_HPC_place_fields%s.mat',erase(stimulus_name{n},'Masa2tracks')),'HPC_place_fields')
            end
            toc
        end
...
for epoch = 2:length(Stimulus_types_all)
    Stimulus_type= Stimulus_types_all{epoch};
    ...
    for nsession =1:length(experiment_info)
        ...
        for n = 1:length(session_info) % How many recording sessions for spatial tasks (PRE, RUN and POST)
            ...
            timebin = 0.05;
            [place_fields_BAYESIAN,decoded_replay_events,probability_ratio_original,decoded_replay_events_shuffled] = log_odds_reactivation_analysis(clusters,place_fields_BAYESIAN,replay,position,[],stimulus_name{n},timebin);
            [place_fields_BAYESIAN,decoded_replay_events_global_remapped,probability_ratio_global_remapped,decoded_replay_events_global_remapped_shuffled] = log_odds_reactivation_analysis(clusters,place_fields_BAYESIAN,replay,position,'global_remapped',stimulus_name{n},timebin);

            save(sprintf('probability_ratio_original_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'probability_ratio_original');
            save(sprintf('decoded_replay_events_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events');
            save(sprintf('probability_ratio_global_remapped_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'probability_ratio_global_remapped');
            save(sprintf('decoded_replay_events_global_remapped_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events_global_remapped');
            save(sprintf('decoded_replay_events_shuffled_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events_shuffled');
            save(sprintf('decoded_replay_events_global_remapped_shuffled_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events_global_remapped_shuffled');
...
for epoch = 1:length(Stimulus_types_all)
    ...
    for nsession =1:length(experiment_info)
        ...
        for n = 1:length(session_info) % How many recording sessions for spatial tasks (PRE, RUN and POST)
            ...
            % Transform time windows for V1 reactivation analysis
            decoded_replay_events_shuffled_V1 = [];
            decoded_replay_events_V1 = [];
            timebin = 0.05;
            tic
            ...
            for nprobe = 1:length(session_info(n).probe)
                ...
                for event = 1:length(replay.onset)
                    ...
                    [place_fields_BAYESIAN,decoded_replay_events,probability_ratio_original,decoded_replay_events_shuffled] = log_odds_reactivation_analysis(V1_clusters.probe(probe_no),V1_place_fields.probe(probe_no),replay1,position,[],stimulus_name{n},timebin);
                    %                     [place_fields_BAYESIAN_V1.probe(probe_no),decoded_replay_events_global_remapped_V1.probe(probe_no),probability_ratio_global_remapped,decoded_replay_events_shuffled_global_remapped] = log_odds_reactivation_analysis(clusters,place_fields_BAYESIAN.probe(probe_no),replay1,position,'global_remapped',stimulus_name{n})

                    decoded_replay_events_V1.probe(probe_no).track(1).replay_events(event).replay = decoded_replay_events(1).replay_events.replay;
                    decoded_replay_events_V1.probe(probe_no).track(2).replay_events(event).replay = decoded_replay_events(2).replay_events.replay;

                    decoded_replay_events_V1.probe(probe_no).track(1).replay_events(event).summed_probability = sum(decoded_replay_events(1).replay_events.replay);
                    decoded_replay_events_V1.probe(probe_no).track(2).replay_events(event).summed_probability = sum(decoded_replay_events(2).replay_events.replay);

                    decoded_replay_events_V1.probe(probe_no).track(1).replay_events(event).spikes = decoded_replay_events(1).replay_events.spikes;
                    decoded_replay_events_V1.probe(probe_no).track(2).replay_events(event).spikes = decoded_replay_events(2).replay_events.spikes;

                    decoded_replay_events_V1.probe(probe_no).track(1).replay_events(event).timebins_edges = decoded_replay_events(1).replay_events.timebins_edges;
                    decoded_replay_events_V1.probe(probe_no).track(2).replay_events(event).timebins_edges = decoded_replay_events(2).replay_events.timebins_edges;

                    for nshuffle = 1:1000
                        decoded_replay_events_shuffled_V1.probe(probe_no).track(1).replay_events(event).summed_probability(nshuffle,:) = sum(decoded_replay_events_shuffled{nshuffle}(1).replay);
                        decoded_replay_events_shuffled_V1.probe(probe_no).track(2).replay_events(event).summed_probability(nshuffle,:) = sum(decoded_replay_events_shuffled{nshuffle}(2).replay);
                    end
...
%             save(sprintf('probability_ratio_original_V1%s.mat',erase(stimulus_name{n},'Masa2tracks')),'probability_ratio_original_V1');
            save(sprintf('decoded_replay_events_V1_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events_V1');
%             save(sprintf('probability_ratio_global_remapped_V1%s.mat',erase(stimulus_name{n},'Masa2tracks')),'probability_ratio_global_remapped_V1');
            save(sprintf('decoded_replay_events_shuffled_V1_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')),'decoded_replay_events_shuffled_V1');
...
%             load(sprintf('decoded_replay_events_shuffled_V1%s.mat',erase(stimulus_name{n},'Masa2tracks')))
            ...
            load(sprintf('decoded_replay_events_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')))
            load(sprintf('decoded_replay_events_shuffled_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')))
            load(sprintf('probability_ratio_original_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')))
            load(sprintf('probability_ratio_global_remapped_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')))
            load(sprintf('decoded_replay_events_V1_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')))
            load(sprintf('decoded_replay_events_shuffled_V1_50ms%s.mat',erase(stimulus_name{n},'Masa2tracks')))
...
%                                         spike_times = V1_clusters.probe(probe_no).spike_times;
            ...
            if length(session_info(n).probe) == 1 % if one probe, but it can be probe 2
                ...
                if sum(reactivations.probe(1).ripple_peak>=3) > sum(reactivations.probe(2).ripple_peak>=3)
                    replay = reactivations.probe(1);
                     events_probe_hemisphere = probe_hemisphere(1);
                else
                    replay = reactivations.probe(2);
                     events_probe_hemisphere = probe_hemisphere(2);
                end
            end
            ...
            for event = 1:length(decoded_replay_events(1).replay_events)
                ...
                log_odds.event_probe_hemisphere(c) = events_probe_hemisphere;
                log_odds.experiment(c) = nsession;
                log_odds.duration(c) = replay.duration(event);
                log_odds.onset(c) = replay.onset(event);
                log_odds.offset(c) = replay.offset(event);
                log_odds.speed(c) = replay.speed(event);

                log_odds.HPC_bayesian_bias(c) = HPC_bayesian_bias(event);
                log_odds.zscore(c) = z_log_odds(event);
                log_odds.zscore_global_remapped(c) = z_log_odds_global_remapped(event);
                log_odds.zscore_global_remapped_substracted(c) = z_log_odds_global_remapped(event)-mean(z_log_odds_global_remapped);
                log_odds.zscore_substracted(c) = z_log_odds(event)-mean(z_log_odds);
                log_odds.ripple_peak(c) = replay.ripple_peak(event);
                ...
                log_odds.V1_log_odds_L(c,:) = nan(1,6);
                log_odds.V1_log_odds_R(c,:) = nan(1,6);
                log_odds.V1_bayesian_bias_L(c,:) = nan(1,6);
                log_odds.V1_bayesian_bias_R(c,:) = nan(1,6);
               ...
               log_odds.V1_cell_spike_count_R{c} = []; 
                ...
                for nprobe = 1:length(probe_hemisphere)
                    ...
                    spike_count_this_event = squeeze(PSTH(1).spike_count);
                    cell_spike_count_this_event = [];

                    if size(spike_count_this_event,2) == 1
                        spike_count_this_event = spike_count_this_event';
                    end
...
%                     end
                    ...
                    if probe_hemisphere(nprobe) == 1
                        %                     mean(zscore(squeeze(PSTH(1).spike_count),0,2));
                        log_odds.V1_total_spike_count_L(c,:) = sum(spike_count_this_event,1);
                        log_odds.V1_spike_count_L(c,:) = nanmean(normalize(PSTH(1).cell_psth,2,'zscore'));
                        
                        log_odds.CA1_spike_count_L(c,:) = nanmean(normalize(PSTH(2).cell_psth,2,'zscore'));
                        log_odds.HPC_spike_count_L(c,:) = nanmean(normalize(PSTH(3).cell_psth,2,'zscore'));

                        log_odds.V1_log_odds_L(c,:) = V1_z_log_odds{probe_hemisphere(nprobe)}(event,:);
                        log_odds.V1_bayesian_bias_L(c,:) = V1_bayesian_bias{probe_hemisphere(nprobe)}(event,:);

                        spikes_this_event = find(all_spike_data{probe_hemisphere(nprobe)}{1}(:,2) >= log_odds.onset(c)-0.6 ...
                            & all_spike_data{probe_hemisphere(nprobe)}{1}(:,2) <= log_odds.onset(c)+0.6);
                        log_odds.V1_spike_id_L{c} = all_spike_data{probe_hemisphere(nprobe)}{1}(spikes_this_event,:);
                        
                    
                        time_window_c = 1;
                        for nwin = 1:20
                            cell_spike_count_this_event(:,nwin) = sum(spike_count_this_event(:,time_window_c:time_window_c+4),2);
                            time_window_c = time_window_c + 5;
                        end

                        log_odds.V1_cell_spike_count_L{c} = cell_spike_count_this_event;
%                          PSTH = plot_perievent_spiketime_histogram(all_spike_data{probe_hemisphere(nprobe)},replay.onset(event),'group','by cell zscore',...
%                              'group_name',group_name,'twin',[-1 1],'bin_size',0.1,'plot_option',0,'smooth_option',0);

                    if size(spike_count_this_event,1) > 2
                        time_window_c = 1;
                        for nwin = 1:10
                            participation_this_event(nwin) = sum(sum(spike_count_this_event(:,time_window_c:time_window_c+9),2)>0)/size(spike_count_this_event,1);
                            time_window_c = time_window_c + 10;
                        end

                        log_odds.V1_participation_L(c,:) = participation_this_event;
 
%                         log_odds.V1_spike_count_L(c,:) = mean(normalize(squeeze(PSTH(1).spike_count),2,'range'));
%                         log_odds.CA1_spike_count_L(c,:) = mean(normalize(squeeze(PSTH(2).spike_count),2,'range'));
%                         log_odds.HPC_spike_count_L(c,:) = mean(normalize(squeeze(PSTH(3).spike_count),2,'range'));
                    end

                    elseif probe_hemisphere(nprobe) == 2

                        log_odds.V1_spike_count_R(c,:) = nanmean(normalize(PSTH(1).cell_psth,2,'zscore'));
                        log_odds.V1_total_spike_count_R(c,:) = sum(spike_count_this_event,1);

                        log_odds.CA1_spike_count_R(c,:) = nanmean(normalize(PSTH(2).cell_psth,2,'zscore'));
                        log_odds.HPC_spike_count_R(c,:) = nanmean(normalize(PSTH(3).cell_psth,2,'zscore'));

                        log_odds.V1_log_odds_R(c,:) = V1_z_log_odds{probe_hemisphere(nprobe)}(event,:);
                        log_odds.V1_bayesian_bias_R(c,:) = V1_bayesian_bias{probe_hemisphere(nprobe)}(event,:);
                        
                        spikes_this_event = find(all_spike_data{probe_hemisphere(nprobe)}{1}(:,2) >= log_odds.onset(c)-0.6 ...
                            & all_spike_data{probe_hemisphere(nprobe)}{1}(:,2) <= log_odds.onset(c)+0.6);
                        log_odds.V1_spike_id_R{c} = all_spike_data{probe_hemisphere(nprobe)}{1}(spikes_this_event,:);
                        ...
                        log_odds.V1_cell_spike_count_R{c} = cell_spike_count_this_event;
                        ...
                        if size(spike_count_this_event,1) > 2
                            ...
                            log_odds.V1_participation_R(c,:) = participation_this_event;
                        end
%                         log_odds.V1_spike_count_R(c,:) = mean(normalize(squeeze(PSTH(1).spike_count),2,'range'));
%                         log_odds.CA1_spike_count_R(c,:) = mean(normalize(squeeze(PSTH(2).spike_count),2,'range'));
%                         log_odds.HPC_spike_count_R(c,:) = mean(normalize(squeeze(PSTH(3).spike_count),2,'range'));
                    end
                end
            ...
            %         ripple_thresholded = reactivations.probe(1).ripple_peak >= 2;
            %
            %         for nprobe = 1:length(session_info(n).probe)
            %             probe_no = session_info(n).probe(nprobe).probe_id + 1;
            %
            %             all_spike_data = [];
            %             %             all_spike_data{1} = [V1_clusters.probe(probe_no).spike_id V1_clusters.probe(probe_no).spike_times];
            %             %             all_spike_data{2} = [CA1_clusters.probe(probe_no).spike_id CA1_clusters.probe(probe_no).spike_times];
            %             %             all_spike_data{3} = [HPC_clusters.probe(probe_no).spike_id HPC_clusters.probe(probe_no).spike_times];
            %             [spike_times,spike_id]= select_spikes_subset(V1_clusters.probe(probe_no),...
            %                 V1_place_fields.probe(nprobe).good_place_cells_LIBERAL);
            %             all_spike_data{1} = [spike_id spike_times];
            %             [spike_times,spike_id]= select_spikes_subset(CA1_clusters.probe(probe_no),...
            %                 CA1_place_fields.probe(nprobe).good_place_cells_LIBERAL);
            %             all_spike_data{2} = [spike_id spike_times];
            %             [spike_times,spike_id]= select_spikes_subset(HPC_clusters.probe(probe_no),...
            %                 HPC_place_fields.probe(nprobe).good_place_cells_LIBERAL);
            %              all_spike_data{3} = [spike_id spike_times];
            %
            %             group_name = {'V1','CA1','HPC'};
            %
            %
            ...
            %
            %             % Ripple
            % %             events = ripples.probe(1).onset;
            %
            %             track_1_events = reactivations.probe(session_info(n).probe(1).probe_id + 1).onset(z_log_odds>0.5 & ripple_thresholded == 1);
            %             track_2_events = reactivations.probe(session_info(n).probe(1).probe_id + 1).onset(z_log_odds<-0.5 & ripple_thresholded == 1);
            %
            %             PSTH = plot_perievent_spiketime_histogram(all_spike_data,track_1_events,'group','by cell zscore','group_name',group_name,'event_name','T1 reactivation','twin',[-0.4 0.5])
            %             plot_perievent_spiketime_histogram(all_spike_data,track_1_events,'group','by region','group_name',group_name,'event_name','T1 reactivation','twin',[-1 1])
            %
            %             PSTH = plot_perievent_spiketime_histogram(all_spike_data,track_2_events,'group','by cell zscore','group_name',group_name,'event_name','T2 reactivation','twin',[-0.4 0.5])
            %             plot_perievent_spiketime_histogram(all_spike_data,track_2_events,'group','by region','group_name',group_name,'event_name','T2 reactivation','twin',[-1 1])
            %
            %
            %         end
        end
...
save('log_odds_50ms_ratio_threshold_0',"log_odds")
...
for nsession =[1 2 5 7 8 9 10]
    ...
    for n = 1
        ...
        c1 = 1;
        c2 = 1;
        V1_cell_spike_count_L = [];
        V1_cell_spike_count_R = [];
        indexR = [];
        indexL = [];
        for event = 1:length(index)

            %             log_odds.HPC_bayesian_bias(index(event))
            %                 z_log(event) =  log_odds.HPC_bayesian_bias(index(event));
            %                 HPC_bayesian_bias(event) =  log_odds.HPC_bayesian_bias(index(event))
            %             V1_cell_spike_count_R

            if ~isempty(log_odds.V1_cell_spike_count_R{index(event)})
                V1_cell_spike_count_R(c1,:,:) = log_odds.V1_cell_spike_count_R{index(event)};
                c1 = c1 + 1;
                indexR = [indexR index(event)];
            end


            if ~isempty(log_odds.V1_cell_spike_count_L{index(event)})
                V1_cell_spike_count_L(c2,:,:) = log_odds.V1_cell_spike_count_L{index(event)};
                c2 = c2 + 1;
                indexL = [indexL index(event)];
            end
        end


        if ~isempty(V1_cell_spike_count_L)

            [psth, bins, rasterX, rasterY, spikeCounts, binnedArray] = psthAndBA(spike_data(:,2), events, twin, bin_size);

            for ncell = 1:size(V1_cell_spike_count_L,2)


                spike_this_cell = squeeze(V1_cell_spike_count_L(:,ncell,:));
                spike_this_cell = spike_this_cell(:,1:2:end) + spike_this_cell(:,2:2:end);% 100ms to 200ms bin
                spike_this_cell = spike_this_cell(:,3:8);% only from -600 to 600ms

                for epoch = 1:4
                    [index index1] = intersect(indexL,index_session{nsession}{epoch});
                    cell_slope_L{nsession}{epoch}{ncell} = [];
                    if length(index) < 10 | sum(sum(spike_this_cell,2) == 0) > 0 % less than 10 events or not firing in more than half of the events (-600ms to 600ms)
                        continue
                    end
% length(index)/2
                    ...
                    for nwin = 1:6
                        ...
                        mdl = fitlm(log_odds.zscore(index)',spike_this_cell(index1,nwin)');
                        [pval,~,~] = coefTest(mdl);
                        ...
                        if pval <= 0.05
                            cell_slope_L{nsession}{epoch}{ncell} = [cell_slope_L{nsession}{epoch}{ncell};nwin b(2)];
                ...
                %
                %                 if (~isempty(cell_slope_L{nsession}{2}{ncell}) +  ~isempty(cell_slope_L{nsession}{3}{ncell}) +  ~isempty(cell_slope_L{nsession}{4}{ncell})) >= 1
                %                     for epoch = 2:4
                %                         % plot(nanmean(spike_this_cell),'k')
                %
                %                         [index index1] = intersect(indexL,index_session{nsession}{epoch});
                %                         if length(index) < 10 | sum(sum(spike_this_cell,2) == 0) > length(index)/2 % less than 10 events or not firing in more than half of the events (-600ms to 600ms)
                %                             continue
                %                         end
                %
                %                         fig = figure
                ...
                %                         for nwin = 1:6
                %                             subplot(2,3,nwin)
                %                             scatter(log_odds.zscore(index),spike_this_cell(index1,nwin),'MarkerEdgeColor','none','MarkerFaceColor','k','MarkerFaceAlpha','0.1')
                %                             hold on
                %                             mdl = fitlm(log_odds.zscore(index)',spike_this_cell(index1,nwin)');
                %                             [pval,~,~] = coefTest(mdl);
                %                             x =[min(log_odds.zscore(index)') max(log_odds.zscore(index)')];
                ...
                %                                 %                                 cell_slope{ncell} = [cell_slope{ncell} ;nwin b];
                %                             else
                %                                 plot(x,y_est,'k:')
                %                                 text(gca,0.5,0.2,['p = ' num2str(pval,2)],'Units','Normalized','FontName','Arial','Color','black');
                %                             end
                %                         end
                %                         sgtitle(sprintf('Left V1 session %i cell %i %s',nsession,cell_index_L(ncell),behavioural_epoches_text{epoch}))
                %                     end
                %
                %                 end
                %             end
            ...
            fig = figure
            fontsize(fig, 14, "points")
            fig.Position = [300 60 1200 900];
            fig.Name = sprintf('Left V1 cell SWR response %s %s',session_info(n).probe(1).SUBJECT,session_info(n).probe(1).SESSION);
            for ncell = 1:size(V1_cell_spike_count_L,2)
                subplot(6,6,c)

                [index index1] = intersect(indexL,find(log_odds.zscore > 0.5));
                %                 [index index2] = intersect(indexL,find(log_odds.zscore < 0));
                spike_this_cell = squeeze(V1_cell_spike_count_L(:,ncell,:));
                %                     [index index1] = intersect(indexL,find(log_odds.HPC_bayesian_bias > 0.6));
                if length(index) < 10 |sum(sum(spike_this_cell,2) == 0) > 2*length(index)/3  % less than 10 events or not firing in more than half of the events (-600ms to 600ms)
                    continue
                end
                %                     [index index2] = intersect(indexL,find(log_odds.HPC_bayesian_bias < 0.4));

                [index index2] = intersect(indexL,find(log_odds.zscore < -0.5));
                    if length(index) < 10 | sum(sum(spike_this_cell,2) == 0) > 2*length(index)/3 % less than 10 events or not firing in more than half of the events (-600ms to 600ms)
                        continue
                    end
                max_FR = max([mean(spike_this_cell(index1,:)) mean(spike_this_cell(index2,:))]);

                plot(-0.95:0.1:1,mean(spike_this_cell(index1,:))/max_FR,'r');hold on;
                plot(-0.95:0.1:1,mean(spike_this_cell(index2,:))/max_FR,'b')
                
                if sum(V1_all_cells{nsession}{1}(cell_index_L(ncell)) == V1_spatial_cells{nsession}{1}{2}) == 1
                    title(sprintf('Spatial cell %i',V1_all_cells{nsession}{1}(cell_index_L(ncell))),'Color','r')
                else
                    title(sprintf('cell %i',V1_all_cells{nsession}{1}(cell_index_L(ncell))),'Color','k')
                end
                c = c + 1;
                set(gca,"TickDir","out",'box', 'off','Color','none')
            end

            legend([p(1),p(2)],{'Log odds > 0.5','Log odds < -0.5'},'Color','none')
            sgtitle(sprintf('Left V1 SUA SWR activity %s %s',session_info(n).probe(1).SUBJECT,session_info(n).probe(1).SESSION))

        end



        if ~isempty(V1_cell_spike_count_R)

            for ncell = 1:size(V1_cell_spike_count_R,2)
                spike_this_cell = squeeze(V1_cell_spike_count_R(:,ncell,:));
                spike_this_cell = spike_this_cell(:,1:2:end) + spike_this_cell(:,2:2:end);% 100ms to 200ms bin
                spike_this_cell = spike_this_cell(:,3:8);% only from -600 to 600ms

                for epoch = 2:4
                    [index index1] = intersect(indexR,index_session{nsession}{epoch});
                    cell_slope_R{nsession}{epoch}{ncell} = [];
                    ...
                    for nwin = 1:6
                        ...
                        if pval <= 0.05
                            cell_slope_R{nsession}{epoch}{ncell} = [cell_slope_R{nsession}{epoch}{ncell} ;nwin b(2)];
                        end
                    end
                end
                
                % plot(nanmean(spike_this_cell),'k')
%                 if (~isempty(cell_slope_R{nsession}{2}{ncell}) +  ~isempty(cell_slope_R{nsession}{3}{ncell}) +  ~isempty(cell_slope_R{nsession}{4}{ncell})) >= 1
%                     for epoch = 2:4
% 
%                         [index index1] = intersect(indexR,index_session{nsession}{epoch});
%                         if length(index) < 10 | sum(sum(spike_this_cell,2) == 0) > length(index)/2 % less than 10 events or not firing in more than half of the events (-600ms to 600ms)
...
%                             end
%                         end
%                         sgtitle(sprintf('Right V1 session %i cell %i %s',nsession,cell_index_R(ncell),behavioural_epoches_text{epoch}))
%                     end
% 
%                 end
            ...
            fontsize(fig, 14, "points")
           fig.Position = [300 60 1200 900];
            fig.Name = sprintf('Right V1 cell SWR response %s %s',session_info(n).probe(1).SUBJECT,session_info(n).probe(1).SESSION);
            for ncell = 1:size(V1_cell_spike_count_R,2)
                
                subplot(6,6,c)

%                     [index index1] = intersect(indexR,find(log_odds.zscore > 0));
%                     [index index2] = intersect(indexR,find(log_odds.zscore < 0));
%                     [index index1] = intersect(indexR,find(log_odds.HPC_bayesian_bias > 0.6));
                     [index index1] = intersect(indexR,find(log_odds.zscore > 0.5));
                    spike_this_cell = squeeze(V1_cell_spike_count_R(:,ncell,:));

                    if length(index) < 10 |sum(sum(spike_this_cell,2) == 0) > 2*length(index)/3  % less than 10 events or not firing in more than half of the events (-600ms to 600ms)
                        continue
                    end
                     [index index2] = intersect(indexR,find(log_odds.zscore < -0.5));
%                     [index index2] = intersect(indexR,find(log_odds.HPC_bayesian_bias < 0.4));
                    ...
                    p(1) = plot(-0.95:0.1:1,mean(spike_this_cell(index1,:))/max_FR,'r');hold on;
                    p(2)  = plot(-0.95:0.1:1,mean(spike_this_cell(index2,:))/max_FR,'b')
                if sum(V1_all_cells{nsession}{2}(cell_index_R(ncell)) == V1_spatial_cells{nsession}{2}{1}) == 1
                    title(sprintf('Spatial cell %i',V1_all_cells{nsession}{2}(cell_index_R(ncell))),'Color','r')
                else
                    title(sprintf('cell %i',V1_all_cells{nsession}{2}(cell_index_R(ncell))),'Color','k')
                end
                    c = c + 1;
                            set(gca,"TickDir","out",'box', 'off','Color','none')
            end
...
for nsession =1:10
    ...
    for n = 1
        ...
        for event = 1:length(index)
            spike_data{1} = [spike_data{1}; log_odds.V1_spike_id_L{index(event)}];
            spike_data{2} = [spike_data{2}; log_odds.V1_spike_id_R{index(event)}];
        end
        ...
        for nprobe = 1:length(session_info(n).probe)
            ...
            for ncell = 1:length(cell_index{probe_hemisphere})
                ...
                spike_index = find(spike_data{probe_hemisphere}(:,1)==this_cell);
                ...
                if ischar(nprobe)
                    ...
                    fig.Name = sprintf('%s %s %s cell SWR response log odds probes %s (%i)',options.SUBJECT,options.SESSION,region,probe_hemisphere_text{probe_hemisphere},nfigure);
                end
...
%                 end
                ...
                if ~isempty(index1)
                    ...
                    for event = 1:length(event_index)
                        [psth, bins, rasterX, rasterY, spikeCounts, binnedArray] = psthAndBA(spike_data{probe_hemisphere}(spike_index,:),...
                            [zeros(1,event-1) event_index(event)], [-0.6 0.6], 0.001);
...
%                     xline(0,'r')
                    ...
                    subplot(6,4,pcount+1)
                    [psth, bins, rasterX, rasterY, spikeCounts, binnedArray] = psthAndBA(spike_data{probe_hemisphere}(spike_index,:),...
                        [log_odds.onset(index1)], [-0.6 0.6],0.05);
                    plot(mean(binnedArray)/0.04,'r');hold on
...
%                     xline(0,'r')
                    subplot(6,4,pcount+1)
                    [psth, bins, rasterX, rasterY, spikeCounts, binnedArray] = psthAndBA(spike_data{probe_hemisphere}(spike_index,:),...
                        [log_odds.onset(index2)], [-0.6 0.6], 0.05);
                    plot(mean(binnedArray)/0.04,'b');hold on
                end
                ...
                if sum(this_cell == V1_spatial_cells{nsession}{probe_hemisphere}{1}) == 1 &...
                    ...
                    title(sprintf('Spatial cell %i',this_cell),'Color','m')
                elseif sum(this_cell == V1_spatial_cells{nsession}{probe_hemisphere}{1}) == 1
                    title(sprintf('Spatial cell %i',this_cell),'Color','r')
                elseif sum(this_cell == V1_spatial_cells{nsession}{probe_hemisphere}{2}) == 1
                    title(sprintf('Spatial cell %i',this_cell),'Color','b')
                else
                    title(sprintf('cell %i',this_cell),'Color','k')
                end

                if ischar(nprobe)
                    sgtitle(sprintf('%s %s %s cell SWR response %s (%i)',options.SUBJECT,options.SESSION,region,nprobe,nfigure));
                else
                    sgtitle(sprintf('%s %s %s cell SWR response probes %s (%i)',options.SUBJECT,options.SESSION,region,probe_hemisphere_text{probe_hemisphere},nfigure));
                end
            end
...
%         index = find( log_odds.behavioural_state>0);
        ...
        scatter(log_odds.zscore_substracted(index),log_odds.V1_log_odds_R(index,nbin)','MarkerEdgeColor','none','MarkerFaceColor','k','MarkerFaceAlpha','0.1')
        %     mdl = fitlm(log_odds.zscore_substracted(index)',log_odds.V1_spike_count_R(index,time_bin)');
        mdl = fitlm(log_odds.zscore_substracted(index)',log_odds.V1_log_odds_R(index,nbin)');
        [pval,~,~] = coefTest(mdl);
        x =[min(log_odds.zscore_substracted') max(log_odds.zscore_substracted')];
        b = mdl.Coefficients.Estimate';
        y_est = polyval(fliplr(b),x);
        ...
        index2 = intersect(index,find(log_odds.zscore_substracted < -1));
        scatter(log_odds.zscore_substracted(index2),log_odds.V1_log_odds_R(index2,nbin),'MarkerEdgeColor','none','MarkerFaceColor','b','MarkerFaceAlpha','0.1')
        hold on
        index1 = intersect(index,find(log_odds.zscore_substracted > 1));
        scatter(log_odds.zscore_substracted(index1),log_odds.V1_log_odds_R(index1,nbin),'MarkerEdgeColor','none','MarkerFaceColor','r','MarkerFaceAlpha','0.1')
    ...
    for nbin =1:6
        ...
        scatter(log_odds.zscore_substracted(index),log_odds.V1_log_odds_L(index,nbin)','MarkerEdgeColor','none','MarkerFaceColor','k','MarkerFaceAlpha','0.1')
        %     mdl = fitlm(log_odds.zscore_substracted(index)',log_odds.V1_spike_count_R(index,time_bin)');
        mdl = fitlm(log_odds.zscore_substracted(index)',log_odds.V1_log_odds_L(index,nbin)');
        [pval,~,~] = coefTest(mdl);
        x =[min(log_odds.zscore_substracted') max(log_odds.zscore_substracted')];
        b = mdl.Coefficients.Estimate';
        y_est = polyval(fliplr(b),x);
        ...
        index2 = intersect(index,find(log_odds.zscore_substracted < -1));
        scatter(log_odds.zscore_substracted(index2),log_odds.V1_log_odds_L(index2,nbin)','MarkerEdgeColor','none','MarkerFaceColor','b','MarkerFaceAlpha','0.1')
        hold on
        index1 = intersect(index,find(log_odds.zscore_substracted > 1));
        scatter(log_odds.zscore_substracted(index1),log_odds.V1_log_odds_L(index1,nbin)','MarkerEdgeColor','none','MarkerFaceColor','r','MarkerFaceAlpha','0.1')
