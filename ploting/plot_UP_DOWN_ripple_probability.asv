function plot_UP_DOWN_ripple_probability()
...
%% plotting Probability of SWR relative to UP and DOWN onset
...
for nprobe = 1:2
    ...
    nexttile
    x = time_wondows(1)+time_bin/2:time_bin:time_wondows(end)-time_bin/2;
    y = cumsum(sum(probability(nprobe).DOWN_UP)./all_DOWN_no);
    %     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
    ...
    nexttile
    x = time_wondows(1)+time_bin/2:time_bin:time_wondows(end)-time_bin/2;
    y = sum(probability(nprobe).DOWN_UP)./all_DOWN_no;
    %     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
    ...
    nexttile
    x = time_wondows(1)+time_bin/2:time_bin:time_wondows(end)-time_bin/2;
    y = cumsum(sum(probability(nprobe).UP_DOWN)./all_UP_no);
    %     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
    ...
    nexttile
    x = time_wondows(1)+time_bin/2:time_bin:time_wondows(end)-time_bin/2;
    y = sum(probability(nprobe).UP_DOWN)./all_UP_no;
    %     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
