function plot_bilateral_synchronisation_clustering(slow_waves_all,ripples_all,spindles_all,behavioural_state_merged_all,UP_DOWN_ripple_PSTH_MUA,sessions_to_process)
...
for nprobe = 1:2
    ...
    for i = 1:15
        % parfor iBoot = 1:100
        rng(i); % Or any integer seed you like
        [~, ~, sumd] = kmeans(X, i, 'Replicates', 100);
        wss = sum(sumd);
        tss = sum(vecnorm(X - mean(X)).^2);
    ...
    % plot(eva.CriterionValues,'-o','Color','b')
    % xline(eva.OptimalK,'b')
    ...
    optimal_K = min(find(VE>0.85));
    rng(1); % Or any integer seed you like
    % Run k-means clustering
    [idx, C] = kmeans(X, optimal_K, 'Replicates', 100);

    % Compute mean corr value for each cluster
    meanCorrPerCluster = zeros(optimal_K, 1);
    for k = 1:optimal_K
        meanCorrPerCluster(k) = mean(corrs(idx == k));
    end

    % Sort clusters by mean corr (ascending)
    [~, sortOrder] = sort(meanCorrPerCluster);

    % Remap cluster labels based on sorted order
    newIdx = zeros(size(idx));
    for k = 1:optimal_K
        newIdx(idx == sortOrder(k)) = k;
    end
    idx = newIdx; % Final reordered cluster labels

    k_cluster(nprobe).DU_idx = idx;


    %%%%%%  Permutation Test for Silhouette Score
    eva = evalclusters(X, 'kmeans', 'silhouette', 'KList', optimal_K);
    num_perm = 1000;
    true_silhouette = eva.CriterionValues;

    null_silhouettes = zeros(num_perm, 1);
    parfor i = 1:num_perm
        tic
        X_perm=[];
        s = RandStream('mrg32k3a','Seed',i); % Set random seed for resampling
        X_perm(:,1) = X(randperm(s,size(X,1)), 1); % permute rows (destroy structure)

        s = RandStream('mrg32k3a','Seed',i+1000); % Set random seed for resampling
        X_perm(:,2) = X(randperm(s,size(X,1)), 2); % permute rows (destroy structure)

        s = RandStream('mrg32k3a','Seed',i+2000); % Set random seed for resampling
        X_perm(:,3) = X(randperm(s,size(X,1)), 3); % permute rows (destroy structure)

        eval_null = evalclusters(X_perm, 'kmeans', 'silhouette', 'KList', optimal_K); % fixed K=3
        null_silhouettes(i) = eval_null.CriterionValues;
        toc
    end

    % p-value: proportion of null scores >= observed
    p_sil = mean(null_silhouettes >= true_silhouette);
    k_cluster(nprobe).silhouette_DU = true_silhouette;
    k_cluster(nprobe).null_silhouette_DU = null_silhouettes;
    k_cluster(nprobe).p_silhouette_DU = p_sil;
    ...
    set(gca,"TickDir","out",'box', 'off','Color','none','FontSize',12)
    title(hemisphere_texts{nprobe})

    nexttile
    scatter3(ipsi_lag_DU{nprobe},ipsi_corr_DU{nprobe},ipsi_plv_DU{nprobe},5,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(1,:),'MarkerFaceAlpha',0.05)
    xlabel('ipsi lag')
    ylabel('ipsi corr')
    zlabel('ipsi plv')
    xlim([-0.14 0.14])
    ...
    nexttile
    scatter3(contra_lag_DU{nprobe},contra_corr_DU{nprobe},contra_plv_DU{nprobe},5,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(2,:),'MarkerFaceAlpha',0.05)
    xlabel('contra lag')
    ylabel('contra corr')
    zlabel('contra plv')
    xlim([-0.14 0.14])
...
k_cluster(nprobe).UD_idx = [];

fig = figure('Color','w');
fig.Position = [30 60 1880 900];
fig.Name = 'Left and Right UP-DOWN transition lag vs corr vs plv distribution';
...
for nprobe = 1:2
    ...
    % p-value: proportion of null scores >= observed
    p_sil = mean(null_silhouettes >= true_silhouette);
    k_cluster(nprobe).silhouette_UD = true_silhouette;
    k_cluster(nprobe).null_silhouette_UD = null_silhouettes;
    k_cluster(nprobe).p_silhouette_UD = p_sil;
    fprintf('Permutation p-value for silhouette score: %.4f\n', p_sil);
    ...
    nexttile
    scatter3(ipsi_lag_UD{nprobe},ipsi_corr_UD{nprobe},ipsi_plv_UD{nprobe},5,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(1,:),'MarkerFaceAlpha',0.05)
    xlabel('ipsi lag')
    ylabel('ipsi corr')
    zlabel('ipsi plv')
    view([-20 60]); % <-- Add this line for better 3D perspective
    ...
    nexttile
    scatter3(contra_lag_UD{nprobe},contra_corr_UD{nprobe},contra_plv_UD{nprobe},5,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(2,:),'MarkerFaceAlpha',0.05)
    xlabel('contra lag')
    ylabel('contra corr')
    zlabel('contra plv')
    view([-20 60]); % <-- Add this line for better 3D perspective
...
fig = figure('Color','w');
fig.Position = [30 60 1880 900];
fig.Name = 'Left and Right ripples lag vs corr vs plv distribution';
hemisphere_texts = {'Left ripples','Right ripples'}
for nprobe = 1:2
    k_cluster(nprobe).ripples_idx = [];
    ...
    % optimal_K = min(find(VE>0.85));
    optimal_K = 4;
    if ~isempty(optimal_K)
        rng(1); % Or any integer seed you like
        % Run k-means clustering
        [idx, C] = kmeans(X, optimal_K, 'Replicates', 10);
    ...
    k_cluster(nprobe).ripples_idx = idx;
    ...
    % scatter3( ...
    %     X(:, 1), X(:, 2), X(:, 3), ...
    %     3, 'MarkerFaceColor','k','MarkerEdgeColor','none','MarkerFaceAlpha',0.05 ...
    %     );
    ...
    nexttile
    scatter3(ipsi_lag_ripples{nprobe},ipsi_corr_ripples{nprobe},ipsi_plv_ripples{nprobe},3,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(1,:),'MarkerFaceAlpha',0.05)
    xlabel('ipsi lag')
    ylabel('ipsi corr')
    zlabel('ipsi plv')
    view([-20 60]); % <-- Add this line for better 3D perspective
    ...
    nexttile
    scatter3(contra_lag_ripples{nprobe},contra_corr_ripples{nprobe},contra_plv_ripples{nprobe},3,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(2,:),'MarkerFaceAlpha',0.05)
    xlabel('contra lag')
    ylabel('contra corr')
    zlabel('contra plv')
    view([-20 60]); % <-- Add this line for better 3D perspective
...
fig = figure('Color','w');
fig.Position = [30 60 1880 900];
fig.Name = 'Left and Right spindles lag vs corr vs plv distribution';
hemisphere_texts = {'Left spindles','Right spindles'}
for nprobe = 1:2
    k_cluster(nprobe).spindles_idx = [];
    ...
    k_cluster(nprobe).spindles_idx = idx;
    % Z = linkage(X, 'ward'); % 'ward', 'average', or 'single'
    % dendrogram(Z);
    ...
    % p-value: proportion of null scores >= observed
    p_sil = mean(null_silhouettes >= true_silhouette);
    k_cluster(nprobe).silhouette_spindles = true_silhouette;
    k_cluster(nprobe).null_silhouette_spindles = null_silhouettes;
    k_cluster(nprobe).p_silhouette_spindles = p_sil;
    fprintf('Permutation p-value for silhouette score: %.4f\n', p_sil);
    ...
    % scatter3( ...
    %     X(:, 1), X(:, 2), X(:, 3), ...
    %     20, 'MarkerFaceColor','k','MarkerEdgeColor','none','MarkerFaceAlpha',0.1 ...
    %     );
    ...
    nexttile
    scatter3(ipsi_lag_spindles{nprobe},ipsi_corr_spindles{nprobe},ipsi_plv_spindles{nprobe},20,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(1,:),'MarkerFaceAlpha',0.05)
    xlabel('ipsi lag')
    ylabel('ipsi corr')
    zlabel('ipsi plv')
    view([-20 60]); % <-- Add this line for better 3D perspective
    ...
    nexttile
    scatter3(contra_lag_spindles{nprobe},contra_corr_spindles{nprobe},contra_plv_spindles{nprobe},20,'MarkerEdgeColor','none','MarkerFaceColor',colour_lines(2,:),'MarkerFaceAlpha',0.05)
    xlabel('contra lag')
    ylabel('contra corr')
    zlabel('contra plv')
    view([-20 60]); % <-- Add this line for better 3D perspective
...
% cluster 1 -3 seems to be mainly assigned based on ipsi-contra corr diff 
...
for nprobe = 1:2
    ...
    mu2 = lags(idx == 2&lags'>=-0.21&lags'<=0.2);

    threshold_low  = mean([(mean(mu3)+mean(mu2))/2 (mean(mu1)+mean(mu2))/2]);

    mu1 = lags(idx == 1&lags'>0&lags');
    mu3 = lags(idx == 3&lags'>0&lags');

    threshold_high = mean([(mean(mu3)+mean(mu2))/2 (mean(mu1)+mean(mu2))/2]);

    event_info(nprobe).UP_lag_threshold_low = threshold_low;
    event_info(nprobe).UP_lag_threshold_high = threshold_high;

    event_info(nprobe).UP_lags_diff = lags;
    event_info(nprobe).UP_corrs_diff = corrs ;
    event_info(nprobe).UP_plvs_diff = plvs;

    lag_index{nprobe} = zeros(1,length(event_info(nprobe).UP_lags_diff));
    lag_index{nprobe}(event_info(nprobe).UP_lags_diff< event_info(nprobe).UP_lag_threshold_low) = -1;
    lag_index{nprobe}(event_info(nprobe).UP_lags_diff> event_info(nprobe).UP_lag_threshold_high) = 1;
end
...
lags = [event_info(1).UP_lags_diff event_info(2).UP_lags_diff];
corrs= [event_info(1).UP_corrs_diff event_info(2).UP_corrs_diff];
plvs= [event_info(1).UP_plvs_diff event_info(2).UP_plvs_diff];
...
merged_event_info.UP_corrs_diff = corrs';
merged_event_info.UP_plvs_diff = plvs';
...
for nprobe = 1:2
    ...
    event_info(nprobe).DOWN_lags_diff = lags;
    event_info(nprobe).DOWN_corrs_diff = corrs ;
    event_info(nprobe).DOWN_plvs_diff = plvs;

    lag_index{nprobe} = zeros(1,length(event_info(nprobe).DOWN_lags_diff));
    lag_index{nprobe}(event_info(nprobe).DOWN_lags_diff< event_info(nprobe).DOWN_lag_threshold_low) = -1;
    lag_index{nprobe}(event_info(nprobe).DOWN_lags_diff> event_info(nprobe).DOWN_lag_threshold_high) = 1;
...
% 0.05s apart
hemisphere_id = [ones(length(lag_index{1}),1); 2*ones(length(lag_index{2}),1)];
lags = [event_info(1).DOWN_lags_diff event_info(2).DOWN_lags_diff];
lag_index = [lag_index{1} lag_index{2}];
corrs =  [event_info(1).DOWN_corrs_diff event_info(2).DOWN_corrs_diff];
plvs =   [event_info(1).DOWN_plvs_diff event_info(2).DOWN_plvs_diff];
...
merged_event_info.DOWN_corrs_diff = corrs';
merged_event_info.DOWN_plvs_diff = plvs';
...
for nprobe = 1:2
    ...
    event_info(nprobe).ripples_lags_diff = lags;
    event_info(nprobe).ripples_corrs_diff = corrs ;
    event_info(nprobe).ripples_plvs_diff = plvs;

    lag_index{nprobe} = zeros(1,length(event_info(nprobe).ripples_lags_diff));
    lag_index{nprobe}(event_info(nprobe).ripples_lags_diff< event_info(nprobe).ripples_lag_threshold_low) = -1;
    lag_index{nprobe}(event_info(nprobe).ripples_lags_diff> event_info(nprobe).ripples_lag_threshold_high) = 1;
...
hemisphere_id = [ones(length(lag_index{1}),1); 2*ones(length(lag_index{2}),1)];
lags = [event_info(1).ripples_lags_diff event_info(2).ripples_lags_diff];
lag_index = [lag_index{1} lag_index{2}];
corrs =  [event_info(1).ripples_corrs_diff event_info(2).ripples_corrs_diff];
plvs =   [event_info(1).ripples_plvs_diff event_info(2).ripples_plvs_diff];
...
merged_event_info.ripples_lags_diff = lags';
merged_event_info.ripples_corrs_diff = corrs';
merged_event_info.ripples_plvs_diff = plvs';
...
for nprobe = 1:2
    ...
    event_info(nprobe).spindles_lags_diff = lags;
    event_info(nprobe).spindles_corrs_diff = corrs ;
    event_info(nprobe).spindles_plvs_diff = plvs;

    lag_index{nprobe} = zeros(1,length(event_info(nprobe).spindles_lags_diff));
    lag_index{nprobe}(event_info(nprobe).spindles_lags_diff< event_info(nprobe).spindles_boundary_low) = -1;
    lag_index{nprobe}(event_info(nprobe).spindles_lags_diff> event_info(nprobe).spindles_boundary_high) = 1;
...
hemisphere_id = [ones(length(lag_index{1}),1); 2*ones(length(lag_index{2}),1)];
lags = [event_info(1).spindles_lags_diff event_info(2).spindles_lags_diff];
lag_index = [lag_index{1} lag_index{2}];
corrs =  [event_info(1).spindles_corrs_diff event_info(2).spindles_corrs_diff];
plvs =   [event_info(1).spindles_plvs_diff event_info(2).spindles_plvs_diff];
...
merged_event_info.spindles_lags_diff = lags';
merged_event_info.spindles_corrs_diff = corrs';
merged_event_info.spidnels_plvs_diff = plvs';
