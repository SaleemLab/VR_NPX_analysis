...
for nsession = 1:max(ripples_all(1).session_count)
    for probe_no = 1:2
        ...
        HPC_ref_shank(nsession,probe_no) = ripples_all(probe_no).best_channel(nsession);
...
for nsession = 1:max(ripples_all(1).session_count)
    ...
    UP_phase_L(nsession,:) = histcounts(slow_waves_all(1).mean_phase_UP{nsession}(cortex_ref_shank(nsession,1),:),angleEdges);
    DOWN_phase_L(nsession,:) = histcounts(slow_waves_all(1).mean_phase_DOWN{nsession}(cortex_ref_shank(nsession,1),:),angleEdges);
    UP_phase_R(nsession,:) = histcounts(slow_waves_all(2).mean_phase_UP{nsession}(cortex_ref_shank(nsession,2),:),angleEdges);
    DOWN_phase_R(nsession,:) = histcounts(slow_waves_all(2).mean_phase_DOWN{nsession}(cortex_ref_shank(nsession,2),:),angleEdges);
    ...
    delta_speed_L(nsession,:)  = histcounts(2*pi*slow_waves_all(1).cortex_speed_UD(1,slow_waves_all(1).DOWN_session_count == nsession),-100:2:100);
    delta_speed_R(nsession,:)  = histcounts(2*pi*slow_waves_all(2).cortex_speed_UD(2,slow_waves_all(2).DOWN_session_count == nsession),-100:2:100);

    % delta_speed_L(nsession,:) = histcounts(mean(diff(0.00025*slow_waves_all(1).shank_id{nsession}(slow_waves_all(1).probe_hemisphere{nsession} == 1))'./diff(slow_waves_all(1).DOWN_peaktimes{nsession}(slow_waves_all(1).probe_hemisphere{nsession}==1,:))),-0.1:0.001:0.1);
    % delta_speed_R(nsession,:)  = histcounts(-1*mean(diff(0.00025*slow_waves_all(2).shank_id{nsession}(slow_waves_all(2).probe_hemisphere{nsession} == 2))'./diff(slow_waves_all(2).DOWN_peaktimes{nsession}(slow_waves_all(2).probe_hemisphere{nsession}==2,:))),-0.1:0.001:0.1);
...
nexttile
x = binCentre;
y = mean(UP_duration_L);
%     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
LCI = y - std(UP_duration_L)
UCI = y + std(UP_duration_L)
...
y = mean(UP_duration_R);
%     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
LCI = y - std(UP_duration_R)
UCI = y + std(UP_duration_R)
...
nexttile
x = binCentre;
y = mean(DOWN_duration_L);
%     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
LCI = y - std(DOWN_duration_L)
UCI = y + std(DOWN_duration_L)

PLOT = plot(x,y,'Color',colour_lines(1,:),'LineWidth',2);hold on;
ERROR_SHADE(1) = patch([x fliplr(x)],[UCI fliplr(LCI)],colour_lines(1,:),'FaceAlpha','0.3','LineStyle','none');


y = mean(DOWN_duration_R);
%     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
LCI = y - std(DOWN_duration_R)
UCI = y + std(DOWN_duration_R)
...
y = mean(spindle_duration_L);
%     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
LCI = y - std(spindle_duration_L)
UCI = y + std(spindle_duration_L)

PLOT = plot(x,y,'Color',colour_lines(1,:),'LineWidth',2);hold on;
ERROR_SHADE(1) = patch([x fliplr(x)],[UCI fliplr(LCI)],colour_lines(1,:),'FaceAlpha','0.3','LineStyle','none');


y = mean(spindle_duration_R);
%     y = mean(cumsum(probability(nprobe).L_ripples_DOWN_bootstrap,2));
LCI = y - std(spindle_duration_R)
UCI = y + std(spindle_duration_R)
