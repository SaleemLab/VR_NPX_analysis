function output = predict_ripples_by_DOWN_UP_synchrony(ipsi_V1_MUA,contra_V1_MUA,ipsi_HPC_MUA,contra_HPC_MUA,ipsi_ripples,contra_ripples,ripples_info,varargin)
% SETTINGS
p = inputParser;
...
%     key_window_idx2= find(timeVec_prob < 0 & timeVec_prob >= -0.05);
    ...
    ipsi_ripple_power(ipsiRipples_cat) = ripples_info.ipsi_last_ripples_power_UP(DOWN_UP_index(ipsiRipples_cat))';
    contra_ripple_power(contraRipples_cat) = ripples_info.contra_last_ripples_power_UP(DOWN_UP_index(contraRipples_cat))';
    ipsi_ripple_lag(ipsiRipples_cat) = abs(ripples_info.ipsi_last_ripples_lag_UP(DOWN_UP_index(ipsiRipples_cat))');
    contra_ripple_lag(contraRipples_cat) = abs(ripples_info.contra_last_ripples_lag_UP(DOWN_UP_index(contraRipples_cat))');
    ipsi_time_from_last_ripple(ipsiRipples_cat) = abs(ripples_info.ipsi_time_from_last_ripples_UP(DOWN_UP_index(ipsiRipples_cat))');
    contra_time_from_last_ripple(contraRipples_cat) = abs(ripples_info.contra_time_from_last_ripples_UP(DOWN_UP_index(contraRipples_cat))');

    ipsi_Delta_peaks_zscore_DU = abs(ripples_info.ipsi_Delta_peaks_zscore_DU(DOWN_UP_index)');
    contra_Delta_peaks_zscore_DU = abs(ripples_info.contra_Delta_peaks_zscore_DU(DOWN_UP_index)');
...
%     clear output
    ...
    parfor iBoot = 1:nBoot
        ...
        tbl = table(normalize(ipsi_time_from_last_ripple(index)),normalize(contra_time_from_last_ripple(index)),normalize(ipsi_ripple_lag(index)), normalize(contra_ripple_lag(index)), ...
            ...
            'ipsiV1','contraV1','ipsiDelta','contraDelta','DOWN_UP_lag','subjectID'});

        % Model list
        modelList = {
            'ipsiPower ~ ipsiV1 + contraV1 + ipsiV1*contraV1 + (1|subjectID)';
            'contraPower ~ ipsiV1 + contraV1 + ipsiV1*contraV1 + (1|subjectID)';

            'ipsiHPC ~ ipsiV1 + contraV1 + ipsiV1*contraV1 + (1|subjectID)';
            'contraHPC ~ ipsiV1 + contraV1 + ipsiV1*contraV1 + (1|subjectID)';

            'ipsiHPC ~ DOWN_UP_lag + (1|subjectID)';
            'contraHPC ~ DOWN_UP_lag + (1|subjectID)';

            'ipsiPower ~ DOWN_UP_lag + ipsiDelta + contraDelta + (1|subjectID)';
...
%         index = intersect(index,ripples_index)
        ...
        modelList = {
            'ipsiOccurance ~ ipsiV1  + (1|subjectID)';
            'contraOccurance ~ ipsiV1  + (1|subjectID)';

            'ipsiOccurance ~ DOWN_UP_lag  + (1|subjectID)';
            'contraOccurance ~ DOWN_UP_lag  + (1|subjectID)';

            'ipsiOccurance ~ ipsiDelta*contraDelta  + (1|subjectID)';
            'contraOccurance ~ ipsiDelta*contraDelta   + (1|subjectID)';

            'ipsiOccurance ~ DOWN_UP_lag*ipsiDelta*contraDelta  + (1|subjectID)';
            'contraOccurance ~ DOWN_UP_lag*ipsiDelta*contraDelta   + (1|subjectID)';
