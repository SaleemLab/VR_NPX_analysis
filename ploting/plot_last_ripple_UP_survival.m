...
%% 2. Bootstrapped Multivariable Cox Regression
...
% --- Physical Dimensions Configuration ---
px_per_bar     = 30;   
left_margin    = 85;    
right_margin   = 45;
bottom_margin  = 90;  
...
for f = 1:nFeat
    ...
    text(ax, x_pos(f), y_text, sprintf('p = %.2e', p50(f)), ...
        ...
        'FontSize', 8.5); % Slightly smaller font to match tighter spacing
end
...
%% 4. Plot Kaplan-Meier Survival Curves per Feature
...
for f = 1:nFeat
    ...
    P = [];
    for n = 1:2 % 1 = Low (<=25th pct), 2 = High (>=75th pct)
        if n == 1
