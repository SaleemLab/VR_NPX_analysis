...
%     key_window_idx2= find(timeVec_prob < 0 & timeVec_prob >= -0.05);
    ...
    ipsi_Delta_peaks_zscore_DU = abs(UP_DOWN_info.ipsi_Delta_peaks_zscore_DU(DOWN_UP_index)');
    contra_Delta_peaks_zscore_DU = abs(UP_DOWN_info.contra_Delta_peaks_zscore_DU(DOWN_UP_index)');
    DOWN_duration = UP_DOWN_info.previous_DOWN_duration(DOWN_UP_index);
    ipsi_spindles = double(sum(UP_DOWN_info.ipsi_spindles_UP(DOWN_UP_index,:),'omitnan')>0);
    contra_spindles = double(sum(UP_DOWN_info.contra_spindles_UP(DOWN_UP_index,:),'omitnan')>0);
