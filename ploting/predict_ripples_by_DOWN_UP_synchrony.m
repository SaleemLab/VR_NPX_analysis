...
if plot_option == 1
    ...
    % Simple linear regression for line of best fit
    coeffs = polyfit(DOWN_UP_lag, contraHPC_cat, 1);
    x_fit = linspace(min(DOWN_UP_lag), max(DOWN_UP_lag), 100);
    y_fit = polyval(coeffs, x_fit);

    % Median R² and p-value
    medianR2_contra = prctile(R2_contra, 50);
    medianP_contra = prctile(pval_contra, 50);

    % Plot line + annotate
    if medianP_contra < 0.05
        plot(x_fit, y_fit, 'Color', 'r', 'LineWidth', 2)
        text(0.2*max(x_fit), 0.5*max(contraHPC_cat), ...
    ...
    coeffs = polyfit(ipsiHPC_cat, contraHPC_cat, 1);
    x_fit = linspace(min(ipsiHPC_cat), max(ipsiHPC_cat), 100);
    y_fit = polyval(coeffs, x_fit);
    ...
    pval_cross = lme.Coefficients.pValue(2);

    % Display
    if pval_cross < 0.05
        text(0.2*max(x_fit), 0.5*max(contraHPC_cat), ...
            sprintf('R^2 = %.3f\np = %.2e', R2_cross, pval_cross), ...
            'FontSize', 10, 'Color', 'r')
    else
        text(0.2*max(x_fit), 0.5*max(contraHPC_cat), ...
            sprintf('R^2 = %.3f\np = %.2e', R2_cross, pval_cross), ...
    ...
    nexttile
    scatter(DOWN_UP_lag,contra_ripple_power,'filled','MarkerFaceColor',customColors(2,:),'MarkerFaceAlpha',0.2)
    ylim([5 25])
    xlabel('DOWN UP transition ipsi-contra lags')
    ...
    % Simple linear regression for line of best fit
    validIdx = ~isnan(contra_ripple_power) & ~isnan(contra_ripple_power);  % logical index of valid data
    coeffs = polyfit(DOWN_UP_lag(validIdx), contra_ripple_power(validIdx), 1);
    x_fit = linspace(min(DOWN_UP_lag(validIdx)), max(DOWN_UP_lag(validIdx)), 100);
    y_fit = polyval(coeffs, x_fit);
    ...
    if medianP_contra < 0.05
        plot(x_fit, y_fit, 'Color', 'r', 'LineWidth', 2)
        text(0.2*max(x_fit), 0.5*max(contra_ripple_power), ...
            sprintf('R^2 = %.3f\np = %.2e', medianR2_contra, medianP_contra), ...
            'FontSize', 10, 'Color', 'r')
    else
        plot(x_fit, y_fit, 'Color', 'k', 'LineWidth', 2)
    ...
    nexttile
    histogram(DOWN_UP_lag(contraRipples_cat==1),0:0.005:0.15,'Normalization','probability');hold on
    histogram(DOWN_UP_lag(contraRipples_cat==0),0:0.005:0.15,'Normalization','probability')
    [H, pValue, KSstatistic] = kstest2(DOWN_UP_lag(contraRipples_cat==1),DOWN_UP_lag(contraRipples_cat==0));
    text(0.1, 0.1, ...
        sprintf('p = %.2e', pValue), ...
        'FontSize', 10, 'Color', 'k')
