...
%% =================== FAST trainer: no explicit inverses, batched J ===================
function [fit, train_diag] = train_epgplvm_decoupled(Y, tvec, Xinit, theta0, nIter)
% Faster drop-in for the decoupled Laplace E-PGPLVM training.
% - No explicit inverses (Cholesky solves / operator PCG)
% - Per-neuron Laplace in parallel (if PCT available) or batched
% - Vectorized, batched Jacobian wrt X
% - Theta refinement only every few iters
%
% Inputs/Outputs: same as original.
...
% ---- knobs (tweak safely) ----
MAX_LAPLACE_IT = 20;         % Newton steps per neuron
PCG_TOL        = 1e-6;       % PCG tolerance for Laplace linear solve
PCG_MAXIT      = 200;
REFINE_EVERY   = 3;          % refine theta every k iters
BATCH_N        = 64;         % neurons per batch for Jacobian/H diag
EPS_STEP_STOP  = 1e-5;       % early stop on mean|step|

hasPCT = license('test','Distrib_Computing_Toolbox')==1;

for it = 1:nIter
    % ------------------------------------------------------------------
    % (A) Build tuning kernel once per iter (shared across neurons)
    % ------------------------------------------------------------------
    delta2 = max(theta.delta,1e-6)^2;
    % Kx(X,X)
    D2 = pdist2(X,X,'euclidean').^2;        % [T x T]
    Kx = theta.rho * exp(-0.5 * D2 / delta2);
    % jittered chol
    Lx = chol_fac(Kx);

    % ------------------------------------------------------------------
    % (B) Per-neuron Laplace (Poisson) at fixed X, {rho,delta}
    %     Parallel if possible; no dense inverses; PCG on (K^{-1}+diag(mu))
    % ------------------------------------------------------------------
    f_all     = zeros(T,N,'double');
    alpha_all = zeros(T,N,'double');
    mu_all    = zeros(T,N,'double');   % store mu for H diag later

    if hasPCT
        parfor n = 1:N
            [f,alpha,mu] = laplace_one_neuron(Y(:,n), Lx, MAX_LAPLACE_IT, PCG_TOL, PCG_MAXIT);
            f_all(:,n)     = f;
            alpha_all(:,n) = alpha;
            mu_all(:,n)    = mu;
        end
    ...
    % ------------------------------------------------------------------
    % (C) Likelihood gradient & GN-diag Hessian wrt X (batched over neurons)
    %     J_n = d f_n / dX using shared K and pairwise diffs
    % ------------------------------------------------------------------
    [g_like, H_like] = like_grad_hdiag_X_batched(X, alpha_all, mu_all, theta.rho, theta.delta, BATCH_N);

    % ------------------------------------------------------------------
    % (D) OU time prior pieces (no explicit inverse)
    % ------------------------------------------------------------------
    Kt = ou_from_t(tvec, theta.r, theta.ell);
    Lt = chol_fac(Kt);                 % Lt*Lt' = Kt
    % g_prior = Kt^{-1} * X  via solves
    g_prior = K_solve(Lt, X);
    % Diagonal of Kt^{-1} ~ needed for GN diag; approximate cheaply
    % NOTE: exact diag(K^{-1}) is expensive; use Hutchinson trick with few probes
    diag_iKt = approx_diag_Kinv(Lt, T, 8);   % 8 Rademacher probes
    H_prior_d = repmat(diag_iKt, 1, m);
    ...
    % ------------------------------------------------------------------
    % (E) Quick theta refinement every few iterations
    % ------------------------------------------------------------------
    if mod(it, REFINE_EVERY) == 0
        theta = refine_theta_safe(Y, X, tvec, theta);
    end

    % Early stop
    if mean(abs(step(:))) < EPS_STEP_STOP
        break;
    end
end
...
%% ---------- helpers (fast) ----------
function [f,alpha,mu] = laplace_one_neuron(y, Lk, maxit, pcg_tol, pcg_maxit)
% Newton on f with Poisson likelihood and GP prior K, but never form K^{-1}
% Uses operator H * s = K^{-1} s + diag(mu)*s
T = numel(y);
f = log1p(y);   % init
for it=1:maxit
    mu = exp(f);
    g  = y - mu - K_solve(Lk, f);          % gradient wrt f
    % operator for H
    Hfun = @(z) K_solve(Lk, z) + mu .* z;  % (K^{-1} + diag(mu)) * z
    % diagonal preconditioner (cheap)
    Mdiag = 1 ./ (approx_diag_Kinv(Lk, T, 1) + mu);  % 1 probe is fine here
    Mfun  = @(z) Mdiag .* z;

    % PCG solve: H * step = g
    [step, ~] = pcg(Hfun, g, pcg_tol, pcg_maxit, Mfun);
    if any(~isfinite(step)), step = zeros(T,1); end

    f = f + step;
    if max(abs(step)) < 1e-6, break; end
end
mu    = exp(f);
alpha = y - mu;          % at optimum: K^{-1} f = y - mu  => alpha
end

function [g_like, H_like] = like_grad_hdiag_X_batched(X, alpha_all, mu_all, rho, delta, B)
% Compute gradient and GN-diagonal wrt X by batching neurons to limit memory.
[T,m] = size(X);
N     = size(alpha_all,2);
g_like = zeros(T,m);
H_like = zeros(T,m);

delta2 = max(delta,1e-6)^2;

% Precompute kernel and pairwise diffs once (shared across batches)
D2 = pdist2(X,X,'euclidean').^2;              % [T x T]
K  = rho * exp(-0.5 * D2 / delta2);           % [T x T]

for d = 1:m
    difd = X(:,d) - X(:,d)';                   % [T x T]
    Kd   = -(K .* (difd / delta2));            % ∂k/∂x_t(d) (T x T), with the minus
    % Process neurons in batches
    for s = 1:B:N
        e = min(N, s+B-1);
        A   = alpha_all(:,s:e);                % [T x B]
        MU  = mu_all(:,s:e);                   % [T x B]

        % J_d for this batch: [T x T] * [T x B] = [T x B]
        Jd_batch = Kd * A;                     % each column = J_n(:,d)

        % gradient accumulates sum_n J_n
        g_like(:,d) = g_like(:,d) + sum(Jd_batch, 2);

        % GN diag accumulates sum_n mu_n .* J_n.^2 (elementwise)
        H_like(:,d) = H_like(:,d) + sum( MU .* (Jd_batch.^2), 2 );
    end
end
end

function L = chol_fac(A)
% Robust jittered Cholesky
jitter = 1e-9;
for k=1:8
    [L,p] = chol((A+A')/2 + jitter*eye(size(A)),'lower');
    if p==0, return; end
    jitter = jitter*10;
end
error('Cholesky failed after jittering.');
end

function y = K_solve(L, b)
% Solve K*y = b given chol K = L*L'
y = L'\(L\b);
end

function d = approx_diag_Kinv(L, T, nProbe)
% Hutchinson estimator of diag(K^{-1}) using solves with K (via L)
% nProbe = 1–8 is usually enough; unbiased, fast.
d = zeros(T,1);
for k=1:nProbe
    v    = 2*(rand(T,1) > 0.5) - 1;      % Rademacher
    w    = K_solve(L, v);                % K^{-1} v
    d    = d + w.^2;                     % E[ (K^{-1} v).^2 ] = diag(K^{-2}) if v~N(0,I)
end
% The above estimates diag(K^{-2}); map to diag(K^{-1}) with a cheap fix:
% use diagonal of (L*L')^{-1} ≈ sum of column-squares of L^{-1} (one solve per e_i)
% For speed, fallback to a crude but stable approx:
d = max(d, 1e-12).^(1/2);                % keep positive; rough but effective precond
end

function theta = refine_theta_safe(Y, X, tvec, theta0)
% Guarded, infrequent refinement
hasOPT = license('test','Optimization_Toolbox')==1 && exist('fmincon','file')==2;
if ~hasOPT
    theta = theta0;
    return;
end
theta = refine_theta(Y, X, tvec, theta0);
end
...
%     'SpecifyObjectiveGradient', false, ...   % <-- was 'off'; must be logical
%     'FiniteDifferenceType','forward', ...
%     'OptimalityTolerance',1e-3, ...
%     'StepTolerance',1e-3};
% 
% try
%     % Newer MATLAB (R2018b+): supports MaxFunctionEvaluations
%     opts = optimoptions('fmincon', baseOpts{:}, 'MaxFunctionEvaluations', 40);
% catch
%     % Older MATLAB: fall back to MaxFunEvals
%     opts = optimoptions('fmincon', baseOpts{:}, 'MaxFunEvals', 40);
% end
% 
...
% ---- finite-diff gradient in log-space (forward diff) ----
eps_fd = 1e-2;              % step in log-space for FD
f0     = obj(th0);
g      = zeros(4,1);
for i = 1:4
    th_pert      = th0;
    th_pert(i)   = th0(i) + eps_fd;
    f_pert       = obj(th_pert);
    g(i)         = (f_pert - f0) / eps_fd;   % gradient of neg log-like
end
% We want to *maximize* log-like => minimize negative => descend along g
% So update: th_new = th0 - eta * g
eta  = 0.5;                 % step size in log-space (tune if needed)
th1  = th0 - eta * g;
