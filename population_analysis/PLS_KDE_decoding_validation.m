...
addParameter(p, 'good_bin_option', 1, @isnumeric);
...
good_bin_option = p.Results.good_bin_option;
...
for i = 1:1000
    s = RandStream('mrg32k3a','Seed',i); % Set random seed for resampling
    idx = randsample(s,length(labels), length(labels));
    [x, y, ~, AUC_shuf(i)] = perfcurve(labels(idx), log_odds(:), 1, 'XVals', fpr);
%     [x,y,~,AUC_shuf_cell(i)] = perfcurve(labels, log_odds(:),1, 'XVals', fpr);
    [C,ia,ic] = unique(x);
    y = y(ia);
    x = x(ia);
...
if shuffle_option ==1
    %%%% Cell ID shuffle
    TPR_shuf_cell = zeros(1000, length(fpr));
    AUC_shuf_cell = zeros(1,1000);
    for i = 1:1000
        s = RandStream('mrg32k3a','Seed',i); % Set random seed for resampling
        %     idx = randsample(s,length(labels), length(labels), true);
        [x, y, ~, AUC_shuf_cell(i)] = perfcurve(labels, log_odds_shuffled(i,:), 1, 'XVals', fpr);
        %     [x,y,~,AUC_shuf_cell(i)] = perfcurve(labels, log_odds(:),1, 'XVals', fpr);
        [C,ia,ic] = unique(x);
        y = y(ia);
        x = x(ia);
        ...
        TPR_interp = interp1(x, y, fpr, 'linear', 'extrap');  % force to match fpr
        TPR_shuf_cell(i,:) = TPR_interp;

    end
end

% -------------------- STEP 7: Plot ROC track label shuffle --------------------
if plot_option
    title_text = sprintf('%s %s %s ROC validation %.0f ms bins', ...
        options.SUBJECT, options.SESSION, event_type, time_bin_size*1000);
    fig = figure('Name', title_text, 'Position', [200 100 600 580]); hold on;
...
%     xline(0.5, '--k'); xlabel('False Positive Rate'); ylabel('True Positive Rate');
    ...
    if shuffle_option == 1
        CI_real = prctile(TPR_real, [0.5 99.5]);
        CI_shuf = prctile(TPR_shuf_cell, [0.5 99.5]);
        hold on
        plot([0 1],[0 1],'--k')
        PLOT(1) = fill([fpr fliplr(fpr)], [CI_real(2,:) fliplr(CI_real(1,:))], ...
            [231,41,138]/256, 'FaceAlpha', 0.3, 'EdgeColor', 'none');
...
if shuffle_option == 1
    KDE_RUN_validation.TPR_shuffle_cell = TPR_shuf_cell;
    KDE_RUN_validation.AUC_shuffle_cell = AUC_shuf_cell;
end
