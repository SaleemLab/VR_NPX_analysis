% Get coordinates for a Neuropixels 3A, 1.0 or 2.0 metadata file.
%
% Inputs:
%    path: full path to the folder including the .meta file 
%
%    output selection:  0 for text coordinate file; 
%                       1 for Kilosort or Kilosort2 channel map file (write file);
%                       2 for strings to paste into JRClust .prm file
%
%    shankSep: default 250 (um)
%
% History -     Original: Jennifer Colonell, Janelia Research Campus
%
%               SGS: adapted from original to clear up issues related to
%               filepaths and to allow some inputs 
%
function newName = SGLXMetaToCoords_ChannelMap_SGS(path,outType,shankSep)
if ~exist('outType','var') || isempty(outType)
    outType = 1;
end
% Shank separation for multishank
if ~exist('shankSep','var') || isempty(shankSep)
    shankSep = 250;
end
% Get the .meta path
[partialPath,fname,~] = fileparts(path);
metaName = dir(fullfile(partialPath, '*.meta'));
metaName = metaName.name;
% Parse in file to get the metadata structure
meta = ReadMeta(metaName, partialPath);
...
switch outType
    case 0      %tab delimited, chan, x, y, shank
        newName = fullfile(partialPath,[fname,'-siteCoords.txt']);
        fid = fopen( newName, 'w');
    ...
    case 1     %KS2 *.mat
        newName = fullfile(partialPath,[fname,'_kilosortChanMap.mat']);
        ...
        shankInd = shankind;                    % capitalise 'I' for compatability for call 
        name = fname;
        save( newName, 'chanMap', 'chanMap0ind', 'connected', 'name', 'xcoords', 'ycoords', 'kcoords','shankInd');
      
    case 2  %strings to copy into JRC prm file
       newName = fullfile(partialPath,[fname,'_forJRCprm.txt']);
       nchan = numel(chans);    
       fid = fopen( newName, 'w' );     
