...
%
%               FR version for getting channel maps for LFP analyses, only
%               works for Neuropixel 24 type atm

function [Struct] = SGLXMetaToCoords_ChannelMap_FR(path)
shankSep = 250;
% Get the .meta path
...
if pType <= 1
    ...
    % Get Complete Channel Map %
    ChanMap_um = horzcat(xcoords+(shankind.*shankSep),ycoords);    
end

Struct.xcoords = xcoords;
Struct.ycoords = ycoords;
Struct.shankind = shankind;
Struct.elecInd = elecInd;
Struct.shankSep = shankSep;
Struct.ChanMap_um = ChanMap_um;
end
