function spatial_modulation_GLM_analysis(clusters,cell_index,Behaviour,Task_info)
...
if isempty(cell_index)
   cell_index = clusters.cluster_id;
end
...
% get all shuffled spiketimes
all_spikeTimes_shuffled = nan(1000,length(clusters.spike_times));

parfor nshuffle = 1:1000
    all_spikeTimes_shuffled_temp = clusters.spike_times;

    for iLap = 1:length(Task_info.start_time_all)
        s = RandStream('mrg32k3a','Seed',nshuffle+10000*iLap); % Set random seed for resampling

        spike_times_lap_index = clusters.spike_times <= Task_info.end_time_all(iLap)...
            & clusters.spike_times >= Task_info.start_time_all(iLap);

        s_temp = RandStream('mrg32k3a','Seed',1000+nshuffle*10000+iLap);

        if rand(s_temp,1) > 0.5 % spike jitter + or -
            all_spikeTimes_shuffled_temp(spike_times_lap_index) = clusters.spike_times(spike_times_lap_index)+rand(s,1);
        else
            all_spikeTimes_shuffled_temp(spike_times_lap_index) = clusters.spike_times(spike_times_lap_index)-rand(s,1);
        end
    end

    all_spikeTimes_shuffled(nshuffle,:) = all_spikeTimes_shuffled_temp;
end
...
for ncell = 1:length(cell_index)
    ...
    SpikeTimes = clusters.spike_times(clusters.spike_id == cell_index(ncell));
