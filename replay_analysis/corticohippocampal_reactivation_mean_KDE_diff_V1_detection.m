...
fig.Name = 'KDE bias difference in HPC with different ripple powers';
...
for npower = 1:nBins
    ...
    parfor iBoot = 1:nBoot
        ...
        boot_bias_shifted = mean_bias_V1; % event id randomised
        ...
        for i = 1:nThresh
            th = thresholds(i);
            t1 = boot_V1 >= th; %%%%% Using V1 to predict HPC bias
            t2 = boot_V1 <= -th;
...
fig.Name = 'KDE bias difference in HPC low vs high ripples';
% tiledlayout(nBins, 3, 'TileSpacing', 'compact');
% colour_lines = [158,202,225;33,113,181]/256;% two blue
clear Fill
nexttile
...
% Save results
save('ripple_power_KDE_bias_difference_based_on_V1_bias.mat', 'ripple_power_KDE_bias_difference');
...
fig = figure;
fig.Position = [640 100 1100 650*2]
fig.Name = 'KDE bias difference in HPC with different ripple powers (PRE ripples)';
tiledlayout(nBins, 3, 'TileSpacing', 'compact');
...
fig = figure;
fig.Position = [640 100 2*1100/3 650/2]
fig.Name = 'KDE bias difference in HPC low vs high ripples (PRE ripples)';
% tiledlayout(nBins, 3, 'TileSpacing', 'compact');
% colour_lines = [158,202,225;33,113,181]/256;% two blue
clear Fill
nexttile
...
% Save results
save('ripple_power_KDE_bias_difference_based_on_PRE_V1_bias.mat', 'ripple_power_KDE_bias_difference');
save_all_figures(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE bias difference based on V1 bias'),[])
...
fig = figure;
fig.Position = [640 100 1100 650*2];
fig.Name = 'KDE bias difference in HPC with different spindle powers';
tiledlayout(nBins, 3, 'TileSpacing', 'compact');
...
fig = figure;
fig.Position = [640 100 2*1100/3 650/2];
fig.Name = 'KDE bias difference in HPC low vs high spindle power';
...
fig = figure;
fig.Position = [640 100 1100 650*2];
fig.Name = 'KDE bias difference in HPC with different spindle powers (PRE ripple)';
tiledlayout(nBins, 3, 'TileSpacing', 'compact');
...
fig = figure;
fig.Position = [640 100 2*1100/3 650/2];
fig.Name = 'KDE bias difference in HPC low vs high spindle power (PRE ripple)';
    ...
    parfor iBoot = 1:nBoot
        ...
        for i = 1:nThresh
            ...
            if npower == 1 % if peak not sync
                ...
                t1s = bb_shift >= th;
                t2s = bb_shift <= -th;
                t1s = t1s & (event_phase(2,:) >= -pi/2 & event_phase(2,:) <= pi/2) & (event_phase(1,:) >= -pi & event_phase(1,:) < -pi/2 | event_phase(1,:) > pi/2 & event_phase(1,:) <= pi);
                t2s = t2s & (event_phase(1,:) >= -pi/2 & event_phase(1,:) <= pi/2) & (event_phase(2,:) >= -pi & event_phase(2,:) < -pi/2 | event_phase(2,:) > pi/2 & event_phase(2,:) <= pi);
            ...
            elseif npower == 2 % if peak sync
                ...
                t1s = bb_shift >= th;
                t2s = bb_shift <= -th;
                t1s = t1s & (event_phase(2,:) >= -pi/2 & event_phase(2,:) <= pi/2) & (event_phase(1,:) >= -pi/2 & event_phase(1,:) <= pi/2);
                t2s = t2s & (event_phase(2,:) >= -pi/2 & event_phase(2,:) <= pi/2) & (event_phase(1,:) >= -pi/2 & event_phase(1,:) <= pi/2);
