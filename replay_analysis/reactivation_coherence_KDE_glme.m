...
%%%%%%%%% Load MUA
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','ripple_PSTH_POST.mat'),'ripple_PSTH');

ripple_MUA_V1_L = cell(1,2);ripple_MUA_V1_R = cell(1,2);
ripple_MUA_HPC = cell(1,2);
for nprobe = 1:2
   
    for nsession = 1:length(ripple_PSTH(1).HPC_MUA)
        ripple_MUA_V1_L{nprobe} = [squeeze(ripple_PSTH(nprobe).V1_MUA{nsession}(1,:,:)); ripple_MUA_V1_L{nprobe}];
        ripple_MUA_V1_R{nprobe} = [squeeze(ripple_PSTH(nprobe).V1_MUA{nsession}(2,:,:)); ripple_MUA_V1_R{nprobe}];
        ripple_MUA_HPC{nprobe} = [squeeze(ripple_PSTH(nprobe).HPC_MUA{nsession}(:,:)); ripple_MUA_HPC{nprobe}];
    end
end
ripple_MUA_V1_L = [ripple_MUA_V1_L{1};ripple_MUA_V1_L{2}];
ripple_MUA_V1_R = [ripple_MUA_V1_R{1};ripple_MUA_V1_R{2}];
ripple_MUA_HPC= [ripple_MUA_HPC{1};ripple_MUA_HPC{2}];

psthBinSize = 0.01;
windows = [-2 2];
x_bins = windows(1)+psthBinSize/2:psthBinSize:windows(end)-psthBinSize/2;
...
ripple_MUA_V1_L = ripple_MUA_V1_L(event_ids_first,:);ripple_MUA_V1_R = ripple_MUA_V1_R(event_ids_first,:);
ripple_MUA_HPC = ripple_MUA_HPC(event_ids_first,:);
...
ripple_info.ripple_power = max([ripple_info.ripple_power(event_ids_first) ripple_info.ripple_power(event_ids_second)],2);
...
%%% SO phase and spindle power before and after ripple
...
pre_index = [48 49];post_index = [53 55];% -50ms and 50ms
% pre_index = [43 44];post_index = [58 59];% -150ms and 150ms
pre_index = [46 46];post_index = [56 56];% -100 and 100
...
ripple_info.spindle_amplitude_PRE = squeeze(mean(ripple_info.spindle_amplitude_temporal(pre_index,:,:),1));
ripple_info.spindle_amplitude_POST = squeeze(mean(ripple_info.spindle_amplitude_temporal(post_index,:,:),1));
...
ripple_info.SO_phase_PRE = squeeze(circ_mean(ripple_info.SO_phase_temporal(pre_index,:,:),[],1));
ripple_info.SO_phase_PRE(isnan(ripple_info.spindle_amplitude_PRE))=nan;

ripple_info.SO_phase_POST = squeeze(circ_mean(ripple_info.SO_phase_temporal(post_index,:,:),[],1));
ripple_info.SO_phase_POST(isnan(ripple_info.spindle_amplitude_POST))=nan;
...
bias = KDE_reactivation_ripples_PSTH.HPC_logodds_ripples' + KDE_reactivation_ripples_PSTH.nan_mask';
bias_V1 = KDE_reactivation_ripples_PSTH.V1_logodds_ripples' + KDE_reactivation_ripples_PSTH.nan_mask';
z_bias1 = bias(isfinite(bias));
bias(bias>=inf) = prctile(z_bias1,99.5);
bias(bias<=-inf) = prctile(z_bias1,0.5);

z_bias1 = z_bias(isfinite(bias_V1));
bias_V1(bias_V1>=inf) = prctile(z_bias1,99.5);
bias_V1(bias_V1<=-inf) = prctile(z_bias1,0.5);
...
bias = bias(:,event_ids_first);
bias_V1 = bias_V1(:,event_ids_first);
...
%     [~, indices] = ismember(selected_events, selected_events1);
    ...
    tbl = table(...
        normalize([ripple_info.ripple_power(t1); ripple_info.ripple_power(t2)],'zscore'),...
        [ripple_info.SO_phase(t1,2); ripple_info.SO_phase(t2,1)],...
        [ripple_info.SO_phase(t1,1); ripple_info.SO_phase(t2,2)],...
        normalize([ripple_info.spindle_amplitude(t1,2); ripple_info.spindle_amplitude(t2,1)],'zscore'),...
        normalize([ripple_info.spindle_amplitude(t1,1); ripple_info.spindle_amplitude(t2,2)],'zscore'),...
        [ripple_info.spindle_presence(t1,2); ripple_info.spindle_presence(t2,1)],...
        [ripple_info.spindle_presence(t1,1); ripple_info.spindle_presence(t2,2)],...
        normalize([ripple_info.spindle_amplitude_PRE(t1,2); ripple_info.spindle_amplitude_PRE(t2,1)], 'zscore'),...
        normalize([ripple_info.spindle_amplitude_PRE(t1,1); ripple_info.spindle_amplitude_PRE(t2,2)], 'zscore'),...
        normalize([ripple_info.spindle_amplitude_POST(t1,2); ripple_info.spindle_amplitude_POST(t2,1)], 'zscore'),...
        normalize([ripple_info.spindle_amplitude_POST(t1,1); ripple_info.spindle_amplitude_POST(t2,2)], 'zscore'),...
        [ripple_info.SO_phase_PRE(t1,2); ripple_info.SO_phase_PRE(t2,1)],...
        [ripple_info.SO_phase_PRE(t1,1); ripple_info.SO_phase_PRE(t2,2)],...
        [ripple_info.SO_phase_POST(t1,2); ripple_info.SO_phase_POST(t2,1)],...
        [ripple_info.SO_phase_POST(t1,1); ripple_info.SO_phase_POST(t2,2)],...
        normalize(mean_bias_V1_PRE(selected_events)','zscore'),...
        normalize(mean_bias_V1(selected_events)','zscore'),...
        normalize(mean_bias(selected_events)','zscore'),...
        categorical([ones(length(t1),1); 2*ones(length(t2),1)]),...
        categorical(subject_id(selected_events)),...
        categorical(session_count(selected_events)),...
        'VariableNames', {...
        'RipplePower','SOPhase_Match','SOPhase_NonMatch','SpindlePower_Match','SpindlePower_NonMatch',...
        'Spindle_Match','Spindle_NonMatch',...
        'SpindlePowerPRE_Match', 'SpindlePowerPRE_NonMatch', ...
        'SpindlePowerPOST_Match', 'SpindlePowerPOST_NonMatch', ...
        'SOPhasePRE_Match', 'SOPhasePRE_NonMatch', ...
        'SOPhasePOST_Match', 'SOPhasePOST_NonMatch', ...
        'V1_logodds_PRE','V1_logodds','HPC_logodds','V1_trackID','AnimalID','SessionID'});
    ...
    %     writetable(tbl, 'V1_HPC_reactivation_coherence_HPC1.csv');
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_HPC_-200_200ms.mat'),'tbl');
    ...
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_POST_0_100ms.mat'),'tbl');
    ...
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_POST1.mat'),'tbl');
        ...
        % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme_HPC1.mat'),'tbl');
%     save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme_PRE1.mat'),'tbl');
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme_POST1.mat'),'tbl');
%     load(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_HPC1.mat'),'tbl');
    ...
    tbl = table([ripple_info.ripple_power(t1); ripple_info.ripple_power(t2)], ...
        ...
        [ripple_info.spindle_presence(t1,2); ripple_info.spindle_presence(t2,1)],...
        [ripple_info.spindle_presence(t1,1); ripple_info.spindle_presence(t2,2)],...
        [ripple_info.spindle_amplitude_PRE(t1,2); ripple_info.spindle_amplitude_PRE(t2,1)],...
        [ripple_info.spindle_amplitude_PRE(t1,1); ripple_info.spindle_amplitude_PRE(t2,2)],...
        [ripple_info.spindle_amplitude_POST(t1,2); ripple_info.spindle_amplitude_POST(t2,1)],...
        [ripple_info.spindle_amplitude_POST(t1,1); ripple_info.spindle_amplitude_POST(t2,2)],...
        [ripple_info.SO_phase_PRE(t1,2); ripple_info.SO_phase_PRE(t2,1)],...
        [ripple_info.SO_phase_PRE(t1,1); ripple_info.SO_phase_PRE(t2,2)],...
        [ripple_info.SO_phase_POST(t1,2); ripple_info.SO_phase_POST(t2,1)],...
        [ripple_info.SO_phase_POST(t1,1); ripple_info.SO_phase_POST(t2,2)],...
        mean_bias_V1_PRE(selected_events)',...
        mean_bias_V1(selected_events)',...
        mean_bias(selected_events)',...
        categorical([ones(length(t1),1); 2*ones(length(t2),1)]),...
        categorical(subject_id(selected_events)),...
        categorical(session_count(selected_events)),...
        'VariableNames',{'RipplePower','SOPhase_Match','SOPhase_NonMatch','SpindlePower_Match','SpindlePower_NonMatch',...
        'Spindle_Match','Spindle_NonMatch',...
        'SpindlePowerPRE_Match', 'SpindlePowerPRE_NonMatch', ...
        'SpindlePowerPOST_Match', 'SpindlePowerPOST_NonMatch', ...
        'SOPhasePRE_Match', 'SOPhasePRE_NonMatch', ...
...
%     save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_HPC1_raw.mat'),'tbl');
    ...
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_HPC3_raw.mat'),'tbl');
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_PRE3_raw.mat'),'tbl');
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_POST3_raw.mat'),'tbl');

    % writetable(tbl, 'V1_HPC_reactivation_coherence_lme_PRE1_raw.csv');
    % writetable(tbl, 'V1_HPC_reactivation_coherence_lme_POST1_raw.csv');
    % writetable(tbl, 'V1_HPC_reactivation_coherence_lme_HPC1_raw.csv');
    % 
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme_HPC1_raw.mat'),'tbl');
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme_PRE1_raw.mat'),'tbl');
    % save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme_POST1_raw.mat'),'tbl');
end
...
[A,indices]=ismember(tbl1.V1_logodds,tbl.V1_logodds);
indices = indices(A);
% [~,indices1]=ismember(tbl1.HPC_logodds,tbl.HPC_logodds);
% tbl1.V1_logodds
tbl.SpindlePower_Match(indices)= tbl1.SpindlePower_Match(A);
tbl.SpindlePower_NonMatch(indices)= tbl1.SpindlePower_NonMatch(A);
tbl.Spindle_Match(indices)= tbl1.Spindle_Match(A);
tbl.Spindle_NonMatch(indices)= tbl1.Spindle_NonMatch(A);
% writetable(tbl, 'V1_HPC_reactivation_coherence_lme1_raw.csv');
% writetable(tbl, fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme1_PRE_raw_100ms.csv'));
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme1_raw.mat'),'tbl');

% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme3_raw.mat'),'tbl');
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme3_PRE_raw.mat'),'tbl');

% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_100_200ms_raw.mat'),'tbl');
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_100_200ms_PRE_raw.mat'),'tbl');
tbl1 = tbl;

% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_POST_100_200ms.mat'));
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_PRE_100_200ms.mat'));

% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme_POST1.mat'));
...
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme1.mat'),'tbl');

% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme3.mat'),'tbl');
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme3_PRE.mat'),'tbl');
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme1_PRE.mat'),'tbl');

% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_100_200ms.mat'),'tbl');
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_100_200ms_PRE.mat'),'tbl');


%% Ripple power
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme1.mat'),'tbl');
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme_HPC1.mat'),'tbl');
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','raw_log_odds_table','V1_HPC_reactivation_coherence_lme1.mat'),'tbl');
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','V1_HPC_reactivation_coherence_lme1.mat'),'tbl');
...
tbl.RipplePower = zscore(tbl.RipplePower);
...
tbl.SpindlePower_NonMatch = zscore(tbl.SpindlePower_NonMatch);
