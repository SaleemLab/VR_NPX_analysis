...
for i = 1:10
    ...
    tbl.Match_High = tbl.SpindlePower_Match > median(tbl.SpindlePower_Match,'omitnan');
    tbl.NonMatch_High = tbl.SpindlePower_NonMatch > median(tbl.SpindlePower_NonMatch,'omitnan');
    ...
    % lme = fitlme(tbl, 'HPC_logodds ~SpindlePower_NonMatch*SpindlePower_Match + (1 | AnimalID)');
...
for i = 1:10
    ...
    bar(1+ group_offset,b_mean(i,7),bar_width, 'FaceColor', colour_lines(2,:), 'EdgeColor', 'none', 'FaceAlpha', 0.4);
    ...
    errorbar(1+ group_offset, b_mean(i,7), uci_boot(i,7), uci_boot(i,7), 'k', 'LineWidth', 1.5, 'CapSize', 10);
    ...
    text(1+ group_offset,0.03,sprintf('%.3e',spindle_power_modulation_lme(i).p(7)))
...
for i = 1:10
    ...
    % lme = fitlme(tbl, 'HPC_logodds ~ SpindlePower_Match * V1_trackID  + (1 | AnimalID)');
