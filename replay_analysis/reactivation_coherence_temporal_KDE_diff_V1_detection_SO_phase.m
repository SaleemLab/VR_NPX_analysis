...
% cortex_ref_shank(:,1) = 4;
% cortex_ref_shank(:,2) = 5;
%%%%%%%%%%%%%%%%%% Ripple info
...
% figure;polarhistogram(ripple_info.SO_phase)
...
%% Temporal log odds AUC with different spindle phase (V1→HPC)
...
spindle_thresholds = {'peak','trough'};
nBins = 2;
...
% load
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by spindle phase (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');

% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_spindle_phase.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_spindle_phase.mat'))
for npower = 1:nBins
    nexttile; hold on;
    m  = AUC.mean(:,npower);
    ci = squeeze(AUC.ci(~isnan(m),npower,:));
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title(sprintf('spindle phase bin %s', spindle_thresholds{npower}));
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.25])
...
fig = figure('Name','Temporal HPC log-odds AUC by spindle peak vs spindle trough (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
for npower = 1:nBins
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title('Spindle peak vs spindle trough');
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.25])
...
%% Temporal log odds AUC with spindle peak unilateral vs bilateral (V1→HPC)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
% SO_thresholds = prctile(ripple_info.SO_phase, 0:99.9/4:99.9);
spindle_thresholds = {'peak unilateral','peak bilateral'};
nBins = 2;
...
% load
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by spindle peak synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');

save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_spindle_peak_synchony.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_spindle_peak_synchony.mat'))
for npower = 1:nBins
    nexttile; hold on;
    m  = AUC.mean(:,npower);
    ci = squeeze(AUC.ci(~isnan(m),npower,:));
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC spindle peak synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
for npower = 1:nBins
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title('spindle peak unilateral vs spindle peak bilateral');
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.25])
...
%% Temporal log odds AUC with spindle trough unilateral vs bilateral (V1→HPC)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
% SO_thresholds = prctile(ripple_info.SO_phase, 0:99.9/4:99.9);
spindle_thresholds = {'trough unilateral','trough bilateral'};
nBins = 2;
...
% load
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by spindle trough synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');

save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_spindle_trough_synchony.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_spindle_trough_synchony.mat'))
for npower = 1:nBins
    nexttile; hold on;
    m  = AUC.mean(:,npower);
    ci = squeeze(AUC.ci(~isnan(m),npower,:));
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC spindle trough synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
for npower = 1:nBins
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title('spindle trough unilateral vs SO trough bilateral');
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.25])
...
%% Temporal log odds AUC with different SO powers (V1→HPC)
...
% load
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by SO power (0.1s win 0.02s step)','Position',[640 100 400 900]);
tiledlayout(nBins,1,'TileSpacing','compact');

save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_power.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_power.mat'))
for npower = 1:nBins
    nexttile; hold on;
    m  = AUC.mean(:,npower);
    ci = squeeze(AUC.ci(~isnan(m),npower,:));
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title(sprintf('SO power bin %d (%.2f–%.2f)', npower, ...
...
%% Temporal log odds AUC with different SO phase (V1→HPC)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
% SO_thresholds = prctile(ripple_info.SO_phase, 0:99.9/4:99.9);
...
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_2Hz_SO_phase.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_2Hz_SO_phase.mat'))
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_phase.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_phase.mat'))
% load
% -------- Plot --------
% fig = figure('Name','Temporal HPC log-odds AUC by 2Hz SO phase (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
fig = figure('Name','Temporal HPC log-odds AUC by SO phase (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
for npower = 1:nBins
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title(sprintf('SO phase bin %s', spindle_thresholds{npower}));
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.25])
...
% -------- Plot --------
% fig = figure('Name','Temporal HPC log-odds AUC SO peak vs SO trough 2Hz (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
...
%% Temporal log odds AUC with SO peak unilateral vs bilateral (V1→HPC)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
% SO_thresholds = prctile(ripple_info.SO_phase, 0:99.9/4:99.9);
...
% load
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by SO peak synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
% fig = figure('Name','Temporal HPC log-odds AUC by SO peak synchrony 2Hz (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');

% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_2Hz_SO_peak_synchony.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_2Hz_SO_peak_synchony.mat'))

save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_peak_synchony.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_peak_synchony.mat'))
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC SO peak synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
% fig = figure('Name','Temporal HPC log-odds AUC SO peak synchrony 2Hz (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
%% Temporal log odds AUC with SO trough unilateral vs bilateral (V1→HPC)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
% SO_thresholds = prctile(ripple_info.SO_phase, 0:99.9/4:99.9);
...
% load
% -------- Plot --------
% fig = figure('Name','Temporal HPC log-odds AUC by SO trough synchrony 2Hz (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
% tiledlayout(nBins,1,'TileSpacing','compact');
fig = figure('Name','Temporal HPC log-odds AUC by SO trough synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');

% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_2Hz_SO_trough_synchony.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_2Hz_SO_trough_synchony.mat'))
% 
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_trough_synchony.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_trough_synchony.mat'))
for npower = 1:nBins
    nexttile; hold on;
    m  = AUC.mean(:,npower);
    ci = squeeze(AUC.ci(~isnan(m),npower,:));
...
% -------- Plot --------
% fig = figure('Name','Temporal HPC log-odds AUC SO trough synchrony 2Hz (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
fig = figure('Name','Temporal HPC log-odds AUC SO trough synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
for npower = 1:nBins
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title('SO trough unilateral vs SO trough bilateral');
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.25])
...
%%
%%
%% Temporal log odds AUC with SO trough + spindle trough or spindle peak (V1→HPC)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
% SO_thresholds = prctile(ripple_info.SO_phase, 0:99.9/4:99.9);
spindle_thresholds = {'SO trough and spindle peak','SO trough and spindle trough'};
nBins = 2;
    ...
    for npower = 1:nBins
        ...
        parfor iBoot = 1:nBoot
            ...
            for i = 1:nThresh
                ...
                if npower == 1 % if SO and spindle trough not sync
...
% load
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_spindle_trough_synchony.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_spindle_trough_synchony.mat'))

% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by SO + spindle trough synchrony (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC SO + spindle trough synchrony combined (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
for npower = 1:nBins
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title('SO trough + spindle trough vs SO trough + spindle peak');
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.25])
...
%% Temporal log odds AUC with SO trough + spindle power (V1→HPC)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
spindle_thresholds = prctile(ripple_info.spindle_amplitude, 0:99.9/2:99.9);
...
for npower = [1 nBins]
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title('SO trough with low spindle vs SO trough with high spindle');
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.35])
...
%% Temporal log odds AUC with SO peak + spindle power (V1→HPC) (control)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
spindle_thresholds = prctile(ripple_info.spindle_amplitude, 0:99.9/2:99.9);
    ...
    for npower = 1:nBins
        ...
        parfor iBoot = 1:nBoot
            ...
            for i = 1:nThresh
                ...
                % SO peak with different spindle powers
                t1 = t1 & (ripple_info.spindle_amplitude(true_idx(idx),2) > spindle_thresholds(npower,2) & ...
                    ripple_info.spindle_amplitude(true_idx(idx),2) <= spindle_thresholds(npower+1,2))' & ...
                    (event_phase(2,:) > -pi/2 & event_phase(2,:) < pi/2 );

                t2 = t2 & (ripple_info.spindle_amplitude(true_idx(idx),1) > spindle_thresholds(npower,1) & ...
                    ripple_info.spindle_amplitude(true_idx(idx),1) <= spindle_thresholds(npower+1,1))' & ...
                    (event_phase(1,:) > -pi/2 & event_phase(1,:) < pi/2 );
                ...
                t1s = t1s & (ripple_info.spindle_amplitude(true_idx,2) > spindle_thresholds(npower,2) & ...
                    ripple_info.spindle_amplitude(true_idx,2) <= spindle_thresholds(npower+1,2))' & ...
                    (event_phase_shifted(2,:) > -pi/2 & event_phase_shifted(2,:) < pi/2);

                t2s = t2s & (ripple_info.spindle_amplitude(true_idx,1) > spindle_thresholds(npower,1) & ...
                    ripple_info.spindle_amplitude(true_idx,1) <= spindle_thresholds(npower+1,1))' & ...
                    (event_phase_shifted(1,:) > -pi/2 & event_phase_shifted(1,:) < pi/2);
...
% load
% -------- Plot --------
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_peak_spindle_power.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_peak_spindle_power.mat'))

% spindle_thresholds = {'SO peak and spindle 0-25','SO peak and spindle 25-50','SO peak and spindle 50-75','SO peak and spindle 75-100'};
spindle_thresholds = {'SO peak and spindle low','SO peak and spindle high'};

fig = figure('Name','Temporal HPC log-odds AUC by SO peak with different spindle power (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC SO peak with low vs high spindle power (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
%% Temporal log odds AUC with SO peak + SO power (V1→HPC) (control)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
SO_thresholds = prctile(ripple_info.SO_amplitude, 0:99.9/2:99.9);


% SO_thresholds1 = prctile(ripple_info.SO_amplitude(ripple_info.SO_amplitude(:,1)>0,1), 0:99.9/2:99.9);
% SO_thresholds2 = prctile(ripple_info.SO_amplitude(ripple_info.SO_amplitude(:,2)>0,2),  0:99.9/2:99.9);
% SO_thresholds = [SO_thresholds1;SO_thresholds2]';
nBins =size(SO_thresholds,1)-1;
    ...
    for npower = 1:nBins
        ...
        parfor iBoot = 1:nBoot
            ...
            for i = 1:nThresh
                ...
                t1s = t1s & (ripple_info.SO_amplitude(true_idx,2) > SO_thresholds(npower,2) & ...
                    ripple_info.SO_amplitude(true_idx,2) <= SO_thresholds(npower+1,2))' & ...
                    (event_phase_shifted(2,:) > -pi/2 & event_phase_shifted(2,:) < pi/2);

                t2s = t2s & (ripple_info.SO_amplitude(true_idx,1) > SO_thresholds(npower,1) & ...
                    ripple_info.SO_amplitude(true_idx,1) <= SO_thresholds(npower+1,1))' & ...
                    (event_phase_shifted(1,:) > -pi/2 & event_phase_shifted(1,:) < pi/2);
...
% load
% -------- Plot --------
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_peak_SO_power.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_peak_SO_power.mat'))

% spindle_thresholds = {'SO peak and power 0-25','SO peak and power 25-50','SO peak and power 50-75','SO peak and power 75-100'};
spindle_thresholds = {'SO peak and SO low','SO peak and SO high'};

fig = figure('Name','Temporal HPC log-odds AUC by SO peak with different SO power (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC SO peak with low vs high SO power (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
%% Temporal log odds AUC with SO trough + SO power (V1→HPC) (control)
%%%%%%%%%%%%%%%%%%%%%%%% spindle phase binning
% Spindle power binning across both probes
% all_spindle_power = mean(ripple_info.spindle_amplitude, 1);  % avg of probe 1 and 2
SO_thresholds = prctile(ripple_info.SO_amplitude, 0:99.9/2:99.9);
...
% load
% -------- Plot --------
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_trough_SO_power.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_SO_trough_SO_power.mat'))
...
spindle_thresholds = {'SO trough and SO low','SO trough and SO high'};

fig = figure('Name','Temporal HPC log-odds AUC by SO trough with different SO power (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC SO trough with low vs high SO power (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
for npower = [1 nBins]
    ...
    xlabel('Time (s relative to ripple)');
    ylabel('HPC bias AUC');
    title('SO trough with low vs high SO power');
    set(gca,'TickDir','out','Box','off','FontSize',12);
    xlim([-0.5 0.5]);
    ylim([-0.1 0.35])
...
%%%%%%%%%%%%%
%%%%%%%%%%%%%
%% Temporal log odds AUC of ripples that occur during UP state or DOWN state
    ...
    for npower = 1:nBins
        ...
        parfor iBoot = 1:nBoot
            ...
            for i = 1:nThresh
                ...
                if npower == 1 % UP and not UP
                    t1 = t1' + (ripple_info.normalised_UP_duration(true_idx(idx),2) > 0 & ...
                        isnan(ripple_info.normalised_UP_duration(true_idx(idx),1))) > 1';
                    t2 = t2' + (ripple_info.normalised_UP_duration(true_idx(idx),1) > 0 & ...
                        isnan(ripple_info.normalised_UP_duration(true_idx(idx),2))) > 1';
                    ...
                    t1s = t1s' + (ripple_info.normalised_UP_duration(true_idx,2) > 0 & ...
                        isnan(ripple_info.normalised_UP_duration(true_idx,1))) > 1';
                    t2s = t2s' + (ripple_info.normalised_UP_duration(true_idx,1) > 0 & ...
                        isnan(ripple_info.normalised_UP_duration(true_idx,2))) > 1';
...
% -------- save --------
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_UP.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_UP.mat'))

% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by UP states (0.1s win 0.02s step)','Position',[640 100 400 900/4*2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by Unilateral UP vs Bilateral UP (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%
%% Temporal log odds AUC of ripples that occur during Unilateral vs Bilateral UP
...
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
%% Temporal log odds AUC of ripples that occur during first half and second half of UP state 
% duration_thresholds = prctile(ripple_info.normalised_UP_duration, 0:33:100);
duration_thresholds = [0, 1/3,2/3, 1;0, 1/3,2/3, 1]';
    ...
    for npower = 1:nBins
        ...
        parfor iBoot = 1:nBoot
            ...
            for i = 1:nThresh
                ...
                t1 = t1' + (ripple_info.normalised_UP_duration(true_idx(idx),2) > duration_thresholds(npower,2) & ...
                    ripple_info.normalised_UP_duration(true_idx(idx),2) <= duration_thresholds(npower+1,2)) > 1';
                t2 = t2' + (ripple_info.normalised_UP_duration(true_idx(idx),1) > duration_thresholds(npower,1) & ...
                ...
                t1s = t1s' + (ripple_info.normalised_UP_duration(true_idx,2) > duration_thresholds(npower,2) & ...
                    ripple_info.normalised_UP_duration(true_idx,2) <= duration_thresholds(npower+1,2)) > 1';
                t2s = t2s' + (ripple_info.normalised_UP_duration(true_idx,1) > duration_thresholds(npower,1) & ...
...
% -------- save --------
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_normalised_UP_duration.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_normalised_UP_duration.mat'))

% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by normalised UP duration (0.1s win 0.02s step)','Position',[640 100 400 900/4*3]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC 1-3 vs 2-3 UP state (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC 2-3 vs 3-3 UP state (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
        ...
        parfor iBoot = 1:nBoot
            ...
            for i = 1:nThresh
                ...
                if npower == 1
                    % Early
                    % Track 1 → use right probe (2), Track 2 → left probe (1)
                    t1 = t1' & ripple_info.early_UP_index(true_idx(idx),2)==1;
                    t2 = t2' & ripple_info.early_UP_index(true_idx(idx),1)==1;
                else
                    % Late
                    % Track 1 → use right probe (2), Track 2 → left probe (1)
                    t1 = t1' & ripple_info.late_UP_index(true_idx(idx),2)==1;
                    t2 = t2' & ripple_info.late_UP_index(true_idx(idx),1)==1;
                end
                ...
                if npower == 1
                    % Early
                    % Track 1 → use right probe (2), Track 2 → left probe (1)
                    t1s = t1s' & ripple_info.early_UP_index(true_idx,2)==1;
                    t2s = t2s' & ripple_info.early_UP_index(true_idx,1)==1;
                else
                    % Late
                    % Track 1 → use right probe (2), Track 2 → left probe (1)
                    t1s = t1s' & ripple_info.late_UP_index(true_idx,2)==1;
                    t2s = t2s' & ripple_info.late_UP_index(true_idx,1)==1;
                end
...
% -------- save --------
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_DU_UD_250ms.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_temporal_bias_DU_UD_250ms.mat'))

% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by early vs late UP state (0.1s win 0.02s step)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% -------- Plot --------
fig = figure('Name','Temporal HPC log-odds AUC by DU vs UD (0.1s win 0.02s step)','Position',[640 100 400 900/4]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
figure('Name','Distribution of Bilateral V1 SO phases')
...
%% Coherence at different SO phase and UP/DOWN SO phase
% session_count = session_count(event_ids_first);
nBins = 4;
phase_bin_edges = -pi:pi/2:pi;
phase_bin_centres = phase_bin_edges(1)+pi/4:pi/2:phase_bin_edges(end)-pi/4;


SO_bin_edges = 0:1/4:1;
SO_bin_centres = SO_bin_edges(1)+1/4/2:1/4:SO_bin_edges(end)-1/4/2;
...
% Storage
AUC.mean = nan(nBins, nBins);
AUC.ci = nan(nBins, nBins, 2);
AUC.shifted_mean = nan(nBins, nBins);
AUC.shifted_ci = nan(nBins, nBins, 2);
AUC.total_events = nan(nBins, nBins);

% for nsession = 1:max(session_count)
for nphase = 1:nBins % SO phase
    for mphase = 1:nBins % Normalised UP/DOWN SO phase
        % All events included, spindle bin applied later conditional on track side
        event_index = true(1, length(z_bias));
        ...
        parfor iBoot = 1:nBoot
            ...
            for i = 1:nThresh
                ...
                t2 = (t2' & ripple_info.SO_phase(true_idx(idx),1)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx(idx),1) <phase_bin_edges(nphase+1) ...
                    & ripple_info.normalised_SO_duration(true_idx(idx),1)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(true_idx(idx),1) <SO_bin_edges(mphase+1) ...
                    & ripple_info.SO_event_duration(true_idx(idx),1) < event_duration_threshold);

                t1 = (t1' & ripple_info.SO_phase(true_idx(idx),2)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx(idx),2) <phase_bin_edges(nphase+1) ...
                    & ripple_info.normalised_SO_duration(true_idx(idx),2)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(true_idx(idx),2) <SO_bin_edges(mphase+1) ...
                    & ripple_info.SO_event_duration(true_idx(idx),2) < event_duration_threshold);
                ...
                t2s = (t2s' & ripple_info.SO_phase(true_idx,1)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx,1) <phase_bin_edges(nphase+1) ...
                    & ripple_info.normalised_SO_duration(true_idx,1)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(true_idx,1) <SO_bin_edges(mphase+1) ...
                    & ripple_info.SO_event_duration(true_idx,1) < event_duration_threshold);

                t1s = (t1s' & ripple_info.SO_phase(true_idx,2)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx,2) <phase_bin_edges(nphase+1) ...
                    & ripple_info.normalised_SO_duration(true_idx,2)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(true_idx,2) <SO_bin_edges(mphase+1) ...
                    & ripple_info.SO_event_duration(true_idx,2) < event_duration_threshold);
        ...
        % t2 = (t2' & ripple_info.SO_phase(:,1)> phase_bin_edges(nphase) & ripple_info.SO_phase(:,1) <phase_bin_edges(nphase+1) ...
        %     & ripple_info.normalised_SO_duration(:,1)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(:,1) <SO_bin_edges(mphase+1) ...
        %     & ripple_info.SO_event_duration(:,1) < event_duration_threshold);
        % 
        % t1 = (t1' & ripple_info.SO_phase(:,2)> phase_bin_edges(nphase) & ripple_info.SO_phase(:,2) <phase_bin_edges(nphase+1) ...
        %     & ripple_info.normalised_SO_duration(:,2)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(:,2) <SO_bin_edges(mphase+1) ...
        %     & ripple_info.SO_event_duration(:,2) < event_duration_threshold);
        
        no_event = sum((ripple_info.SO_phase(:,1)> phase_bin_edges(nphase) & ripple_info.SO_phase(:,1) <phase_bin_edges(nphase+1) ...
            & ripple_info.normalised_SO_duration(:,1)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(:,1) <SO_bin_edges(mphase+1) ...
            & ripple_info.SO_event_duration(:,1) < event_duration_threshold)) + ...
            sum((ripple_info.SO_phase(:,2)> phase_bin_edges(nphase) & ripple_info.SO_phase(:,2) <phase_bin_edges(nphase+1) ...
            & ripple_info.normalised_SO_duration(:,2)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(:,2) <SO_bin_edges(mphase+1) ...
            & ripple_info.SO_event_duration(:,2) < event_duration_threshold));

        % no_reactivation_event = sum(t1)+sum(t2);
        ...
        % Store summaries
        AUC.mean(nphase, mphase) = mean(auc_boot, 'omitnan');
        AUC.ci(nphase, mphase, :) = prctile(auc_boot, [2.5 97.5]);
        AUC.shifted_mean(nphase, mphase) = mean(auc_shift_boot, 'omitnan');
        AUC.shifted_ci(nphase, mphase, :) = prctile(auc_shift_boot, [2.5 97.5]);
        AUC.shifted_ci(nphase, mphase, :) = prctile(auc_shift_boot, [2.5 97.5]);
        AUC.total_events(nphase, mphase) = no_event;
        % AUC.reactivation_events(nphase, mphase, :) = prctile(auc_shift_boot, [2.5 97.5]);
    end
end
% end

% -------- save --------
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_3s.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_1_5s.mat'))
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_10s.mat'))
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN.mat'))
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_2Hz.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_2Hz.mat'))

% -------- Plot --------
fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence (3s)','Position',[640 100 400 900/2]);
% fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence 2Hz (3s)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');

fig.Position = [550 160 800 620];

subplot(2,2,1)
imagesc(SO_bin_centres,phase_bin_centres,AUC.mean)
colorbar;colormap(flipud(gray))
yticks(-pi:pi/2:pi)
yticklabels({'-pi','-pi/2','0','pi/2','pi'})
% xline(-pi/2,'r-','LineWidth',2);xline(pi/2,'r-','LineWidth',2);
yline(-pi/2,'r-','LineWidth',2);yline(pi/2,'r-','LineWidth',2);
ylabel('V1 SO phase (ripple)')
xlabel('Normalised SO duration')
set(gca,'TickDir','out','Box','off','FontSize',12);
title('Distribution of Reactivation coherence')
clim([-0.05 0.3])

subplot(2,2,2)
imagesc(SO_bin_centres,phase_bin_centres,AUC.total_events)
colorbar;colormap(flipud(gray))
yticks(-pi:pi/2:pi)
yticklabels({'-pi','-pi/2','0','pi/2','pi'})
% xline(-pi/2,'r-','LineWidth',2);xline(pi/2,'r-','LineWidth',2);
...
% subplot(2,2,3)
% imagesc(SO_bin_centres,phase_bin_centres,squeeze(AUC.ci(:,:,1)))
% colorbar;colormap(flipud(gray))
% yticks(-pi:pi/2:pi)
% yticklabels({'-pi','-pi/2','0','pi/2','pi'})
% % xline(-pi/2,'r-','LineWidth',2);xline(pi/2,'r-','LineWidth',2);
...
% xlabel('Normalised SO duration')
% set(gca,'TickDir','out','Box','off','FontSize',12);
% title('Distribution of Reactivation coherence lower CI')
% clim([-0.1 0.5])
% 
% 
% subplot(2,2,4)
% imagesc(SO_bin_centres,phase_bin_centres,squeeze(AUC.ci(:,:,2)))
% colorbar;colormap(flipud(gray))
% yticks(-pi:pi/2:pi)
% yticklabels({'-pi','-pi/2','0','pi/2','pi'})
% % xline(-pi/2,'r-','LineWidth',2);xline(pi/2,'r-','LineWidth',2);
...
% set(gca,'TickDir','out','Box','off','FontSize',12);
% title('Distribution of Reactivation coherence')
% title('Distribution of Reactivation coherence Higher CI')
% clim([-0.1 0.5])
...
% subject_session = ripple_all
session_count = subject_id;
...
% Storage
AUC.mean = nan(max(session_count),nBins, nBins);
AUC.ci = nan(max(session_count),nBins, nBins, 2);
AUC.shifted_mean = nan(max(session_count),nBins, nBins);
AUC.shifted_ci = nan(max(session_count),nBins, nBins, 2);
AUC.total_events = nan(max(session_count),nBins, nBins);
...
for nsession = 1:max(session_count)
    for nphase = 1:nBins % SO phase
        for mphase = 1:nBins % Normalised UP/DOWN SO phase
            % All events included, spindle bin applied later conditional on track side
            event_index = session_count==nsession;
            
            % event_duration_threshold = mean(prctile(ripple_info.SO_event_duration(event_index,:),40));
            ...
            true_idx = find(event_index);
            no_event = sum((ripple_info.SO_phase(true_idx,1)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx,1) <phase_bin_edges(nphase+1) ...
                & ripple_info.normalised_SO_duration(true_idx,1)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(true_idx,1) <SO_bin_edges(mphase+1) ...
                & ripple_info.SO_event_duration(true_idx,1) < event_duration_threshold)) + ...
                sum((ripple_info.SO_phase(true_idx,2)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx,2) <phase_bin_edges(nphase+1) ...
                & ripple_info.normalised_SO_duration(true_idx,2)> SO_bin_edges(mphase) & ripple_info.normalised_SO_duration(true_idx,2) <SO_bin_edges(mphase+1) ...
                & ripple_info.SO_event_duration(true_idx,2) < event_duration_threshold));
            ...
            % Store summaries
            AUC.mean(nsession,nphase, mphase) = mean(auc_boot, 'omitnan');
            AUC.ci(nsession,nphase, mphase, :) = prctile(auc_boot, [2.5 97.5]);
            AUC.shifted_mean(nsession,nphase, mphase) = mean(auc_shift_boot, 'omitnan');
            AUC.shifted_ci(nsession,nphase, mphase, :) = prctile(auc_shift_boot, [2.5 97.5]);
            AUC.shifted_ci(nsession,nphase, mphase, :) = prctile(auc_shift_boot, [2.5 97.5]);
            AUC.total_events(nsession,nphase, mphase) = no_event;
            % AUC.reactivation_events(nphase, mphase, :) = prctile(auc_shift_boot, [2.5 97.5]);
        end
    end
end
...
% -------- save --------
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_session.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_session.mat'))
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_session.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_session.mat'))
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject.mat'))
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_2Hz.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_2Hz.mat'))
% % 
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_PRE.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_PRE.mat'))


% -------- Plot --------
fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence subject','Position',[640 100 1250 700]);
% fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence 2Hz subject','Position',[640 100 1250 700]);
tiledlayout(nBins,1,'TileSpacing','compact');

for nsession = 1:6
    subplot(2,3,nsession)
    % subplot(5,5,nsession)
    % squeeze(AUC.mean(nsession,:,:))
    imagesc(SO_bin_centres,phase_bin_centres,squeeze(AUC.mean(nsession,:,:)))
    colorbar;colormap(flipud(gray))
    yticks(-pi:pi/2:pi)
    yticklabels({'-pi','-pi/2','0','pi/2','pi'})
    % xline(-pi/2,'r-','LineWidth',2);xline(pi/2,'r-','LineWidth',2);
    ...
    xlabel('Normalised SO duration')
    set(gca,'TickDir','out','Box','off','FontSize',12);
    % title(ripples_all(1).subject(nsession,:))
    title('Distribution of reactivation coherence')
    clim([-0.1 max(max(squeeze(AUC.mean(nsession,:,:))))])
end
% % 
% figure
% imagesc(SO_bin_centres,phase_bin_centres,squeeze(mean(AUC.mean(:,:,:),1)))
% colorbar;colormap(flipud(gray))
% clim([-0.1 max(max(squeeze(mean(AUC.mean(:,:,:),1))))])

fig = figure('Name','SO phase vs Normalised SO duration ripple distribution subject','Position',[640 100 1250 700]);
% fig = figure('Name','SO phase vs Normalised SO duration ripple distribution 2Hz subject','Position',[640 100 1250 700]);
tiledlayout(nBins,1,'TileSpacing','compact');

for nsession = 1:6
    subplot(2,3,nsession)
    % squeeze(AUC.total_events(nsession,:,:))
    imagesc(SO_bin_centres,phase_bin_centres,squeeze(AUC.total_events(nsession,:,:)))
    colorbar;colormap(flipud(gray))
    yticks(-pi:pi/2:pi)
    yticklabels({'-pi','-pi/2','0','pi/2','pi'})
    % xline(-pi/2,'r-','LineWidth',2);xline(pi/2,'r-','LineWidth',2);
    ...
    set(gca,'TickDir','out','Box','off','FontSize',12);
    title('Distribution of ripples')
    clim([0 max(max(AUC.total_events(nsession,:,:)))])
end
...
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_2Hz.mat'))
% -------- Plot --------
fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence average (3s)','Position',[640 100 400 900/2]);
% fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence 2Hz average (3s)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
subplot(2,2,2)
imagesc(SO_bin_centres,phase_bin_centres,squeeze(sum(AUC.total_events(:,:,:),1)))
colorbar;colormap(flipud(gray))
yticks(-pi:pi/2:pi)
yticklabels({'-pi','-pi/2','0','pi/2','pi'})
% xline(-pi/2,'r-','LineWidth',2);xline(pi/2,'r-','LineWidth',2);
...
%% Per session version
%% Coherence at different SO phase and UP SO phase
session_count = [ripples_all(1).session_count(ripples_all(1).SWS_index==1); ripples_all(2).session_count(ripples_all(2).SWS_index==1)];
subject_id = str2double(cellstr(ripples_all(1).subject(session_count,end-1:end)));
[~, ~, subject_id] = unique(subject_id);
        ...
        for mphase = 1:nBins % Normalised UP/DOWN SO phase
            ...
            parfor iBoot = 1:nBoot
                ...
                for i = 1:nThresh
                    ...
                    t2 = (t2' & ripple_info.SO_phase(true_idx(idx),1)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx(idx),1) <phase_bin_edges(nphase+1) ...
                        & ripple_info.normalised_UP_duration(true_idx(idx),1)> SO_bin_edges(mphase) & ripple_info.normalised_UP_duration(true_idx(idx),1) <SO_bin_edges(mphase+1) ...
                        & ripple_info.UP_duration(true_idx(idx),1) < event_duration_threshold);

                    t1 = (t1' & ripple_info.SO_phase(true_idx(idx),2)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx(idx),2) <phase_bin_edges(nphase+1) ...
                        & ripple_info.normalised_UP_duration(true_idx(idx),2)> SO_bin_edges(mphase) & ripple_info.normalised_UP_duration(true_idx(idx),2) <SO_bin_edges(mphase+1) ...
                        & ripple_info.UP_duration(true_idx(idx),2) < event_duration_threshold);
                    ...
                    t2s = (t2s' & ripple_info.SO_phase(true_idx,1)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx,1) <phase_bin_edges(nphase+1) ...
                        & ripple_info.normalised_UP_duration(true_idx,1)> SO_bin_edges(mphase) & ripple_info.normalised_UP_duration(true_idx,1) <SO_bin_edges(mphase+1) ...
                        & ripple_info.UP_duration(true_idx,1) < event_duration_threshold);

                    t1s = (t1s' & ripple_info.SO_phase(true_idx,2)> phase_bin_edges(nphase) & ripple_info.SO_phase(true_idx,2) <phase_bin_edges(nphase+1) ...
                        & ripple_info.normalised_UP_duration(true_idx,2)> SO_bin_edges(mphase) & ripple_info.normalised_UP_duration(true_idx,2) <SO_bin_edges(mphase+1) ...
                        & ripple_info.UP_duration(true_idx,2) < event_duration_threshold);
...
% -------- save --------
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_session.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_session.mat'))
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_session.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_session.mat'))
save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_subject.mat'),'AUC')
load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_subject.mat'))
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_subject_2Hz.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_subject_2Hz.mat'))
% % 
% save(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_PRE.mat'),'AUC')
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_PRE.mat'))
...
% -------- Plot --------
fig = figure('Name','SO phase vs Normalised UP duration reactivation coherence subject','Position',[640 100 1250 700]);
% fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence 2Hz subject','Position',[640 100 1250 700]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
fig = figure('Name','SO phase vs Normalised UP duration ripple distribution subject','Position',[640 100 1250 700]);
% fig = figure('Name','SO phase vs Normalised SO duration ripple distribution 2Hz subject','Position',[640 100 1250 700]);
tiledlayout(nBins,1,'TileSpacing','compact');
...
% load(fullfile(analysis_folder,'V1-HPC sleep reactivation','KDE_SO_vs_UP_DOWN_subject_2Hz.mat'))
% -------- Plot --------
fig = figure('Name','SO phase vs Normalised UP duration reactivation coherence average (3s)','Position',[640 100 400 900/2]);
% fig = figure('Name','SO phase vs Normalised SO duration reactivation coherence 2Hz average (3s)','Position',[640 100 400 900/2]);
tiledlayout(nBins,1,'TileSpacing','compact');
