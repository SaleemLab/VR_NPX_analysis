...
for npower = 1:length(power_thresholds)-1
    ...
    idx = randi(s, length(mean_bias), length(mean_bias), 1);

    mean_bias_V1_shuffled = mean_bias_V1(idx);

    nexttile
    scatter(mean_bias,mean_bias_V1);
...
% Storage structure
spindle_presence_KDE_bias_difference = struct;
condition = [0,1];

sampled_events = sum(ripple_info.spindle_presence == 1);
...
fig.Name = 'KDE bias difference in V1 with and without spindle';
tiledlayout(nBins, 3, 'TileSpacing', 'compact');


for npower = 1:nBins
    % Ripple power bin index
    spindle_index = ripple_info.spindle_presence == condition(npower);

    event_index = (singlet_index + spindle_index) > 1;
    mean_bias = mean(z_bias(bins_to_use, event_index), 'omitnan');
    mean_bias_shifted = mean(z_bias(bins_to_use_shifted, event_index), 'omitnan');
    mean_bias_V1 = mean(z_bias_V1(bins_to_use, event_index), 'omitnan');
    ...
    % Store results
    spindle_presence_KDE_bias_difference(npower).power_range = [power_thresholds(npower), power_thresholds(npower+1)];
    spindle_presence_KDE_bias_difference(npower).bias_diff_mean = bias_mean;
    spindle_presence_KDE_bias_difference(npower).bias_diff_CI = [bias_CI_lo; bias_CI_hi];
    spindle_presence_KDE_bias_difference(npower).prop_mean = prop_mean;
    spindle_presence_KDE_bias_difference(npower).prop_CI = [prop_CI_lo; prop_CI_hi];
    spindle_presence_KDE_bias_difference(npower).thresholds = thresholds;
    ...
    xlabel('HPC bias threshold');
    ylabel('V1 bias diff (T1 - T2)');
    title(sprintf('Spindle = %i', condition(npower)));
%     grid on;
    set(gca,"TickDir","out",'box', 'off','Color','none','FontSize',12)
...
fig = figure;
fig.Position = [640 100 2*1100/3 650/2]
fig.Name = 'KDE bias difference in V1 with vs without spindle';
% tiledlayout(nBins, 3, 'TileSpacing', 'compact');
colour_lines = [158,202,225;33,113,181]/256;% two blue
nexttile
for npower = [1 2]
    bias_mean = spindle_presence_KDE_bias_difference(npower).bias_diff_mean;
    bias_CI_lo = spindle_presence_KDE_bias_difference(npower).bias_diff_CI(1,:);
    bias_CI_hi = spindle_presence_KDE_bias_difference(npower).bias_diff_CI(2,:)
...
yline(0,'--r')
legend(Fill(1:2) ,{'without spindle','with spindle'},'box','off')
...
for npower = [1 2]
    ...
    bias_CI_lo = spindle_presence_KDE_bias_difference(npower).bias_diff_CI(1,:);
    bias_CI_hi = spindle_presence_KDE_bias_difference(npower).bias_diff_CI(2,:)
    prop_mean = spindle_presence_KDE_bias_difference(npower).prop_mean;
...
save('spindle_presence_KDE_bias_difference.mat', 'spindle_presence_KDE_bias_difference');
save_all_figures(fullfile(analysis_folder,'V1-HPC sleep reactivation'),[])
...
% Storage structure
UP_DOWN_presence_KDE_bias_difference = struct;
index = {(ripple_info.UP_index == 1),(ripple_info.DOWN_index == 1)};
title_names = {'UP','DOWN'};
...
fig = figure;
fig.Position = [640 100 1100 650]
fig.Name = 'KDE bias difference in V1 with ripples during UP and DOWN ';
tiledlayout(nBins, 3, 'TileSpacing', 'compact');
...
for npower = 1:nBins
    ...
    % Store results
    UP_DOWN_presence_KDE_bias_difference(npower).power_range = [power_thresholds(npower), power_thresholds(npower+1)];
    UP_DOWN_presence_KDE_bias_difference(npower).bias_diff_mean = bias_mean;
    UP_DOWN_presence_KDE_bias_difference(npower).bias_diff_CI = [bias_CI_lo; bias_CI_hi];
    UP_DOWN_presence_KDE_bias_difference(npower).prop_mean = prop_mean;
    UP_DOWN_presence_KDE_bias_difference(npower).prop_CI = [prop_CI_lo; prop_CI_hi];
    UP_DOWN_presence_KDE_bias_difference(npower).thresholds = thresholds;
...
for npower = [1 2]
    ...
    bias_CI_lo = UP_DOWN_presence_KDE_bias_difference(npower).bias_diff_CI(1,:);
    bias_CI_hi = UP_DOWN_presence_KDE_bias_difference(npower).bias_diff_CI(2,:)
...
yline(0,'--r')
legend(Fill(1:2) ,{'UP','DOWN'},'box','off')
...
for npower = [1 2]
    ...
    bias_CI_lo = UP_DOWN_presence_KDE_bias_difference(npower).bias_diff_CI(1,:);
    bias_CI_hi = UP_DOWN_presence_KDE_bias_difference(npower).bias_diff_CI(2,:)
    prop_mean = UP_DOWN_presence_KDE_bias_difference(npower).prop_mean;
...
is_peak_phase_1 = (ripple_info.SO_phase(:,1) >= pi/2 & ripple_info.SO_phase(:,1) <= pi) | ...
                  (ripple_info.SO_phase(:,1) >= -pi & ripple_info.SO_phase(:,1) <= -pi/2);
...
% Use real ripple data
% z_bias = KDE_reactivation_content.HPC_z_ripples';
% z_bias_V1 = KDE_reactivation_content.V1_z_ripples';

% Get session IDs
session_ids = subject_id;  % should be same length as # of ripples
...
% Colors for plotting
colour_lines = [158,202,225;33,113,181]/256;

% Loop over sessions
for isess = 1:length(unique_sessions)
    sess_id = unique_sessions(isess);

    % Indices for current session
    sess_idx = session_ids == sess_id;

    % Subset data for session
    sess_ripple_power = ripple_info.ripple_power(sess_idx);
    sess_z_bias = z_bias(:, sess_idx);
    sess_z_bias_V1 = z_bias_V1(:, sess_idx);
    sess_singlet_index = singlet_index(sess_idx);  % ensure singlet_index is defined

    power_thresholds = prctile(sess_ripple_power, 0:99.9/1:99.9);
    nBins = length(power_thresholds) - 1;
    ...
    % Optional: Create figure for this session
    fig = figure;
    fig.Position = [640 100 1100 650/2];
    fig.Name = ['Session ' num2str(sess_id) ' - KDE Bias Diff'];
    tiledlayout(nBins, 3, 'TileSpacing', 'compact');

    for npower = 1:nBins
        power_index = sess_ripple_power > power_thresholds(npower) & ...
                      sess_ripple_power <= power_thresholds(npower + 1);

        event_index = (sess_singlet_index + power_index) > 1;
        mean_bias = mean(sess_z_bias(bins_to_use, event_index), 'omitnan');
        mean_bias_V1 = mean(sess_z_bias_V1(bins_to_use, event_index), 'omitnan');
        total_events = length(mean_bias);
        ...
        ripple_power_KDE_bias_difference(npower).power_range = ...
            [power_thresholds(npower), power_thresholds(npower+1)];
        ripple_power_KDE_bias_difference(npower).bias_diff_mean = bias_mean;
        ripple_power_KDE_bias_difference(npower).bias_diff_CI = [bias_CI_lo; bias_CI_hi];
        ripple_power_KDE_bias_difference(npower).prop_mean = prop_mean;
        ripple_power_KDE_bias_difference(npower).prop_CI = [prop_CI_lo; prop_CI_hi];
        ...
        x2 = [thresholds, fliplr(thresholds)];
        y2 = [bias_CI_lo, fliplr(bias_CI_hi)];
        fill(x2, y2, colour_lines(mod(npower-1,2)+1,:), 'EdgeColor', 'none', 'FaceAlpha', 0.4);
        plot(thresholds, bias_mean, 'Color', colour_lines(mod(npower-1,2)+1,:), 'LineWidth', 2);
        ylim([-0.3 0.4]); xlim([0 1.4]); yline(0,'--r')
        xlabel('HPC bias threshold'); ylabel('V1 bias diff (T1 - T2)');
        title(sprintf('Power bin %d: %.2f–%.2f', npower, power_thresholds(npower), power_thresholds(npower+1)));
        set(gca,"TickDir","out",'box','off','Color','none','FontSize',12)

        % ---- Plot B: Prop vs Bias Diff ----
        nexttile((npower-1)*3 + 2); hold on;
        valid_idx = isfinite(bias_mean) & isfinite(prop_mean);
        x_vals = bias_mean(valid_idx); y_vals = prop_mean(valid_idx);
        x_lo = bias_CI_lo(valid_idx); x_hi = bias_CI_hi(valid_idx);
        fill([x_lo, fliplr(x_hi)], [y_vals, fliplr(y_vals)], ...
            colour_lines(mod(npower-1,2)+1,:), 'EdgeColor','none', 'FaceAlpha',0.4);
        plot(x_vals, y_vals, '-', 'Color', colour_lines(mod(npower-1,2)+1,:), 'LineWidth', 2);
        xlim([-0.3 0.4]); xline(0,'--r')
        xlabel('V1 bias diff (T1 - T2)'); ylabel('Proportion of events detected');
        title('Event Proportion vs. Bias Difference');
        set(gca,"TickDir","out",'box','off','Color','none','FontSize',12)

        % ---- Plot C: Prop vs V1 Bias Diff w/ shaded Y CI ----
        nexttile((npower-1)*3 + 3); hold on;
        y_lo = prop_CI_lo(valid_idx); y_hi = prop_CI_hi(valid_idx);
        fill([x_vals, fliplr(x_vals)], [y_lo, fliplr(y_hi)], ...
            colour_lines(mod(npower-1,2)+1,:), 'EdgeColor','none', 'FaceAlpha',0.4);
        plot(x_vals, y_vals, '-', 'Color', colour_lines(mod(npower-1,2)+1,:), 'LineWidth', 2);
        xlim([-0.3 0.4]); xline(0,'--r')
        xlabel('V1 bias diff (T1 - T2)'); ylabel('Proportion of events detected');
    ...
    session_results(isess).session_id = sess_id;
    session_results(isess).ripple_power_KDE_bias_difference = ripple_power_KDE_bias_difference;
end



save_all_figures(fullfile(analysis_folder,'V1-HPC sleep reactivation','within session'),[])
save_all_figures(fullfile(analysis_folder,'V1-HPC sleep reactivation'),[])
...
for isess = 1:length(unique_sessions)
    ...
    sess_z_bias = z_bias(:, sess_idx);
    sess_z_bias_V1 = z_bias_V1(:, sess_idx);
    sess_singlet_index = singlet_index(sess_idx);

    power_thresholds = prctile(sess_ripple_power, 0:99.9/2:99.9);
    nBins = length(power_thresholds) - 1;
    ...
    fig = figure;
    fig.Position = [640 100 1100 650];
    fig.Name = ['Session ' num2str(sess_id) ' - KDE Bias Diff low vs high ripple powers'];
    tiledlayout(nBins, 3, 'TileSpacing', 'compact');
    ...
    for npower = 1:nBins
        ...
        mean_bias = mean(sess_z_bias(bins_to_use, event_index), 'omitnan');
        mean_bias_shifted = mean(sess_z_bias(bins_to_use_shifted, event_index), 'omitnan');
        mean_bias_V1 = mean(sess_z_bias_V1(bins_to_use, event_index), 'omitnan');

        % Remove NaN values from all 3 vectors
        valid_mask = isfinite(mean_bias) & isfinite(mean_bias_shifted) & isfinite(mean_bias_V1);
        mean_bias = mean_bias(valid_mask);
        mean_bias_shifted = mean_bias_shifted(valid_mask);
        mean_bias_V1 = mean_bias_V1(valid_mask);

        total_events = numel(mean_bias);
        if total_events < 10
            warning("Session %s, Power bin %d: Not enough valid events (%d). Skipping...", ...
                    string(sess_id), npower, total_events);
            continue;
        end
        ...
        parfor iBoot = 1:nBoot
            ...
            diff_real = NaN(1, nThresh); prop_real = NaN(1, nThresh);
            diff_shifted = NaN(1, nThresh); prop_shifted = NaN(1, nThresh);

            for i = 1:nThresh
                th = thresholds(i);

                t1 = boot_bias >= th; t2 = boot_bias <= -th;
                t1_V1 = boot_V1(t1); t2_V1 = boot_V1(t2);
                if ~isempty(t1_V1) && ~isempty(t2_V1)
                    diff_real(i) = mean(t1_V1, 'omitnan') - mean(t2_V1, 'omitnan');
                end
                prop_real(i) = (sum(t1) + sum(t2)) / total_events;

                % t1s = boot_bias_shifted >= th; t2s = boot_bias_shifted <= -th;


                s = RandStream('philox4x32_10', 'Seed', iBoot+2000);
                temp = [find(t1) find(t2)];
                idx = randi(s, length(temp), length(temp), 1);

                t1s_V1 = boot_V1(temp(idx(1:round(end/2)))); t2s_V1 = boot_V1(temp(idx(round(end/2)+1:end)));

                % t1s_V1 = boot_V1(t1s); t2s_V1 = boot_V1(t2s);
                if ~isempty(t1s_V1) && ~isempty(t2s_V1)
                    diff_shifted(i) = mean(t1s_V1, 'omitnan') - mean(t2s_V1, 'omitnan');
                end
                prop_shifted(i) = (length(temp)) / total_events;
            end

            bias_diff_boot(iBoot, :) = diff_real;
            prop_events_boot(iBoot, :) = prop_real;
            bias_diff_shifted_boot(iBoot, :) = diff_shifted;
            prop_events_shifted_boot(iBoot, :) = prop_shifted;
        end
        ...
        bias_shift_mean = mean(bias_diff_shifted_boot, 1, 'omitnan');
        bias_shift_CI_lo = prctile(bias_diff_shifted_boot, 2.5, 1);
        bias_shift_CI_hi = prctile(bias_diff_shifted_boot, 97.5, 1);

        prop_shift_mean = mean(prop_events_shifted_boot, 1, 'omitnan');
        prop_shift_CI_lo = prctile(prop_events_shifted_boot, 2.5, 1);
        prop_shift_CI_hi = prctile(prop_events_shifted_boot, 97.5, 1);
        ...
        ripple_power_KDE_bias_difference(npower).prop_mean = prop_mean;
        ripple_power_KDE_bias_difference(npower).prop_CI = [prop_CI_lo; prop_CI_hi];
        ripple_power_KDE_bias_difference(npower).bias_diff_shifted_mean = bias_shift_mean;
        ripple_power_KDE_bias_difference(npower).bias_diff_shifted_CI = [bias_shift_CI_lo; bias_shift_CI_hi];
        ripple_power_KDE_bias_difference(npower).prop_shifted_mean = prop_shift_mean;
        ripple_power_KDE_bias_difference(npower).prop_shifted_CI = [prop_shift_CI_lo; prop_shift_CI_hi];
        ripple_power_KDE_bias_difference(npower).thresholds = thresholds;

        % -------- Plot A: Bias difference vs threshold --------
        nexttile((npower-1)*3 + 1); hold on;
        fill([thresholds, fliplr(thresholds)], ...
             [bias_CI_lo, fliplr(bias_CI_hi)], ...
             colour_lines(mod(npower-1,2)+1,:), 'EdgeColor','none', 'FaceAlpha',0.4);
        plot(thresholds, bias_mean, 'Color', colour_lines(mod(npower-1,2)+1,:), 'LineWidth',2);
        fill([thresholds, fliplr(thresholds)], ...
             [bias_shift_CI_lo, fliplr(bias_shift_CI_hi)], ...
             [0 0 0], 'EdgeColor','none', 'FaceAlpha', 0.2);
        plot(thresholds, bias_shift_mean, 'k--', 'LineWidth',1.5);
        ylim([-0.3 0.4]); xlim([0 1.4]); yline(0,'--r')
        xlabel('HPC bias threshold'); ylabel('V1 bias diff (T1 - T2)');
        title(sprintf('Power bin %d: %.2f–%.2f', npower, power_thresholds(npower), power_thresholds(npower+1)));
        set(gca,"TickDir","out",'box','off','Color','none','FontSize',12)
        ...
        nexttile((npower-1)*3 + 2); hold on;
        valid_idx = isfinite(bias_mean) & isfinite(prop_mean);
        x = bias_mean(valid_idx); y = prop_mean(valid_idx);
        x_lo = bias_CI_lo(valid_idx); x_hi = bias_CI_hi(valid_idx);
        fill([x_lo, fliplr(x_hi)], [y, fliplr(y)], ...
             colour_lines(mod(npower-1,2)+1,:), 'EdgeColor','none', 'FaceAlpha',0.4);
        plot(x, y, '-', 'Color', colour_lines(mod(npower-1,2)+1,:), 'LineWidth',2);
        x_shift = bias_shift_mean(valid_idx); y_shift = prop_shift_mean(valid_idx);
        x_lo_s = bias_shift_CI_lo(valid_idx); x_hi_s = bias_shift_CI_hi(valid_idx);
        fill([x_lo_s, fliplr(x_hi_s)], [y_shift, fliplr(y_shift)], [0 0 0], 'EdgeColor','none', 'FaceAlpha',0.2);
        plot(x_shift, y_shift, 'k--', 'LineWidth', 1.5);
        xlim([-0.3 0.4]); xline(0,'--r')
        xlabel('V1 bias diff (T1 - T2)'); ylabel('Proportion of events detected');
        title('Event Proportion vs. Bias Difference');
        set(gca,"TickDir","out",'box','off','Color','none','FontSize',12)

        % -------- Plot C: Prop vs Bias Diff (Y-CI) --------
        nexttile((npower-1)*3 + 3); hold on;
        y_lo = prop_CI_lo(valid_idx); y_hi = prop_CI_hi(valid_idx);
        fill([x, fliplr(x)], [y_lo, fliplr(y_hi)], ...
             colour_lines(mod(npower-1,2)+1,:), 'EdgeColor','none', 'FaceAlpha',0.4);
        plot(x, y, '-', 'Color', colour_lines(mod(npower-1,2)+1,:), 'LineWidth',2);
        y_lo_s = prop_shift_CI_lo(valid_idx); y_hi_s = prop_shift_CI_hi(valid_idx);
        fill([x_shift, fliplr(x_shift)], [y_lo_s, fliplr(y_hi_s)], ...
             [0 0 0], 'EdgeColor','none', 'FaceAlpha',0.2);
        plot(x_shift, y_shift, 'k--', 'LineWidth', 1.5);
        xlim([-0.3 0.4]); xline(0,'--r')
        xlabel('V1 bias diff (T1 - T2)'); ylabel('Proportion of events detected');
...
for isess = 1:length(session_results)
    result = session_results(isess).ripple_power_KDE_bias_difference;

    fig = figure;
    fig.Position = [640 100 2*1100/3 650/2];
    fig.Name = sprintf('Session %s - KDE bias diff: Low vs High Ripple Power', string(session_results(isess).session_id));
    tiledlayout(1,2, 'TileSpacing', 'compact');

    colour_lines = [158,202,225; 33,113,181]/256; % light blue, dark blue

    % ---------------- Plot A: Bias difference vs. threshold ----------------
    nexttile
    hold on;
    Fill = gobjects(1,2); % preallocate for legend
    for npower = [1 2]
        data = result(npower);
        thresholds = data.thresholds;
        bias_mean = data.bias_diff_mean;
        bias_CI_lo = data.bias_diff_CI(1,:);
        bias_CI_hi = data.bias_diff_CI(2,:);
    ...
    xlabel('HPC Bias Threshold');
    ylabel('V1 Bias Diff (T1 - T2)');
    ylim([-0.3 0.4]); yline(0, '--r')
    set(gca, "TickDir", "out", 'box', 'off', 'Color', 'none', 'FontSize', 12)
    legend(Fill(1:2), {'Low Ripple Power', 'High Ripple Power'}, 'box', 'off')

    % ---------------- Plot B: Proportion vs. Bias Difference ----------------
    nexttile
    hold on;
    for npower = [1 2]
        data = result(npower);
...
for isess = 1:length(unique_sessions)
    ...
    for npower = 1:nBins
        ...
        t1 = mean_bias >= prctile(mean_bias,80); t2 = mean_bias <= prctile(mean_bias,20);
        t1_V1 = boot_V1(t1); t2_V1 = boot_V1(t2);
        if ~isempty(t1_V1) && ~isempty(t2_V1)
            bias_diff_V1(isess,npower) = mean(t1_V1, 'omitnan') - mean(t2_V1, 'omitnan');
        end
    end
...
bias_diff_V1(11:14,:)=nan
[p,h,stats] = ranksum(bias_diff_V1(:,1),bias_diff_V1(:,2));

scatter(bias_diff_V1(:,1),bias_diff_V1(:,2))