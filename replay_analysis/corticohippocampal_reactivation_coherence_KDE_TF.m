...
for n = 1:2
    HF_amplitude{n}=[];
    HF_amplitude_percentile{n}=[];
  
    SO_amplitude{n}=[];
    SO_amplitude_percentile{n}=[];
    ...
    V1_HPC_SO_synchrony{n}=[];
    V1_HPC_SO_synchrony_percentile{n}=[];

    V1_HPC_SO_amplitude_corr{n}=[];
    V1_HPC_SO_amplitude_corr_percentile{n}=[];

    spindle_amplitude{n}=[];
    spindle_amplitude_percentile{n}=[];

    V1_HPC_spindle_synchrony{n} = [];
    V1_HPC_spindle_synchrony_percentile{n} = [];
    ...
    for nsession = 1:length(sessions_to_process)
        ...
        % V1 HF power
        temp1 = [squeeze(ripples_TF_stats.V1_amp{nsession}(1,end,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_amp{nsession}(2,end,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_amp{nsession}(2,end,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_amp{nsession}(1,end,timebins(n),hemi_id == 2))'];
     
        HF_amplitude{n} =  [SO_amplitude{n} temp1 temp2];
        HF_amplitude_percentile{n} = [SO_amplitude_percentile{n} tiedrank(temp1.').' / size(temp1, 2) tiedrank(temp2.').' / size(temp2, 2);];

        % V1 SO power
        temp1 = [squeeze(ripples_TF_stats.V1_amp{nsession}(1,1,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_amp{nsession}(2,1,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_amp{nsession}(2,1,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_amp{nsession}(1,1,timebins(n),hemi_id == 2))'];
     
        SO_amplitude{n} =  [SO_amplitude{n} temp1 temp2];
        SO_amplitude_percentile{n} = [SO_amplitude_percentile{n} tiedrank(temp1.').' / size(temp1, 2) tiedrank(temp2.').' / size(temp2, 2);];

        % V1 spindle power (dimension 3 is spindle)
        temp1 = [squeeze(ripples_TF_stats.V1_amp{nsession}(1,3,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_amp{nsession}(2,3,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_amp{nsession}(2,3,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_amp{nsession}(1,3,timebins(n),hemi_id == 2))'];

        spindle_amplitude{n} =  [spindle_amplitude{n} temp1 temp2];
        spindle_amplitude_percentile{n} = [spindle_amplitude_percentile{n} tiedrank(temp1.').' / size(temp1, 2) tiedrank(temp2.').' / size(temp2, 2);];

        % V1 SO phase
        temp1 = [squeeze(ripples_TF_stats.V1_phase_median{nsession}(1,1,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_phase_median{nsession}(2,1,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_phase_median{nsession}(2,1,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_phase_median{nsession}(1,1,timebins(n),hemi_id == 2))'];
        SO_phase{n} =  [SO_phase{n} temp1 temp2];

        % V1 spindle phase (dimension 3 is spindle)
        temp1 = [squeeze(ripples_TF_stats.V1_phase_median{nsession}(1,3,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_phase_median{nsession}(2,3,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_phase_median{nsession}(2,3,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_phase_median{nsession}(1,3,timebins(n),hemi_id == 2))'];
        spindle_phase{n} =  [spindle_phase{n} temp1 temp2];
        % spindle_amplitude_percentile{n} = [spindle_amplitude_percentile{n} tiedrank(temp1.').' / size(temp1, 2) tiedrank(temp2.').' / size(temp2, 2);];
        ...
        % V1 HPC SO PLV
        temp1 = [squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(1,1,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(2,1,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(2,1,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(1,1,timebins(n),hemi_id == 2))'];

        V1_HPC_SO_synchrony{n} =  [V1_HPC_SO_synchrony{n} temp1 temp2];
        V1_HPC_SO_synchrony_percentile{n} = [V1_HPC_SO_synchrony_percentile{n} tiedrank(temp1.').' / size(temp1, 2) tiedrank(temp2.').' / size(temp2, 2);];

        % V1 HPC SO amp corr
        temp1 = [squeeze(ripples_TF_stats.V1_HPC_amp_corr{nsession}(1,1,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_HPC_amp_corr{nsession}(2,1,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_HPC_amp_corr{nsession}(2,1,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_HPC_amp_corr{nsession}(1,1,timebins(n),hemi_id == 2))'];

        V1_HPC_SO_amplitude_corr{n} =  [V1_HPC_SO_amplitude_corr{n} temp1 temp2];
        V1_HPC_SO_amplitude_corr_percentile{n} = [V1_HPC_SO_amplitude_corr_percentile{n} tiedrank(temp1.').' / size(temp1, 2) tiedrank(temp2.').' / size(temp2, 2);];


        % V1 HPC SO PLV
        temp1 = [squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(1,3,timebins(n),hemi_id == 1))'; squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(2,3,timebins(n),hemi_id == 1))'];
        temp2 = [squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(2,3,timebins(n),hemi_id == 2))'; squeeze(ripples_TF_stats.V1_HPC_PLV{nsession}(1,3,timebins(n),hemi_id == 2))'];

        V1_HPC_spindle_synchrony{n} =  [V1_HPC_spindle_synchrony{n} temp1 temp2];
        V1_HPC_spindle_synchrony_percentile{n} = [V1_HPC_spindle_synchrony_percentile{n} tiedrank(temp1.').' / size(temp1, 2) tiedrank(temp2.').' / size(temp2, 2);];


        % z_bias_V1 = ([PSTH_MUA(1).R_V1_ripples-PSTH_MUA(1).L_V1_ripples;...
        %     [PSTH_MUA(2).R_V1_ripples-PSTH_MUA(2).L_V1_ripples]])';

        % PSTH_MUA(1).L_V1_ripples 
        % PSTH_MUA(1).R_V1_ripples
        % 
        % PSTH_MUA(2).L_V1_ripples
        % PSTH_MUA(2).R_V1_ripples
    end
end
...
for n = 1:2
    ...
    is_peak_phase_1 = ripple_info.SO_phase{n}(1,:) >= 0 & ripple_info.SO_phase{n}(1,:) <= pi;

    is_peak_phase_2 = ripple_info.SO_phase{n}(2,:) >= 0 & ripple_info.SO_phase{n}(2,:) <= pi;
    ...
    % Define logical arrays for whether phase is in the peak range
    is_trough_phase_1 = ripple_info.SO_phase{n}(1,:) >= -pi & ripple_info.SO_phase{n}(1,:) < 0;

    is_trough_phase_2 = ripple_info.SO_phase{n}(2,:) >= -pi & ripple_info.SO_phase{n}(2,:) < 0;
...
for n = 1:2
    ...
    SO_index = ripple_info.V1_HPC_SO_synchrony_percentile{n}()<0.25;
