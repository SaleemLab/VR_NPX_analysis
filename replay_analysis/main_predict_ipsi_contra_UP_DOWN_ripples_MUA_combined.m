...
varnames = {
    ...
    'time_from_first_ripples'
    'time_to_last_ripples'
    'time_to_first_ripples'
...
%                 ripples_index = find(ismember(ripples_original_index,ripples_index));
                ...
                data_struct.time_from_first_ripples{nprobe}(nevent) = ...
                    UP_ints{nprobe}(up_idx,2) - ripples_peaktimes(ripples_index(1));

                data_struct.time_to_last_ripples{nprobe}(nevent) = ...
                    ripples_peaktimes(ripples_index(end)) - UP_ints{nprobe}(up_idx,1);
                data_struct.time_to_first_ripples{nprobe}(nevent) = ...
                    ripples_peaktimes(ripples_index(1)) - UP_ints{nprobe}(up_idx,1);
...
fig = figure('Color','w');
fig.Position = [350 59 750 500];
fig.Name = 'DOWN transition Bilateral lag and delta power distribution';
nexttile
nexttile
nexttile
hold on;
histogram(abs(lags(lag_index)'), 0:0.005:0.15, 'Normalization','probability');
xlabel('DOWN ipsi-contra lags'); ylabel('Proportion');
set(gca,'TickDir','out','Box','off','Color','none','FontSize',12)
xline(prctile(abs(lags(lag_index)),0:20:99.5),'r','LineWidth',1)

nexttile
hold on
histogram(UP_DOWN_info.SWpeakmag_UD, 0.7:0.1:4, 'Normalization','probability');
xlabel('DOWN SO peak'); ylabel('Proportion');
xticks(0:1:5)
xline(prctile(UP_DOWN_info.SWpeakmag_UD,0:20:99.5),'r','LineWidth',1)
set(gca,'TickDir','out','Box','off','Color','none','FontSize',12)

save_all_figures(fullfile(analysis_folder,'V1-HPC bilateral interaction\UP_DOWN_ripples_PSTH'),[])
...
output = plot_UP_survival_probability( ...
    ...
    'title_name', 'Last ripple coherence and last ripple to UP-DOWN transition','event_option','binary','subject_id',subject_id(index)...
    );
...
% output = plot_UP_survival_probability( ...
%     {log_odds_prod(HPC_reactivation_index)}, ...
...
%     'event_option',[],'title_name', 'Last ripple coherence (high bias events) and last ripple to UP-DOWN transition','event_option',[],'subject_id',subject_id(HPC_reactivation_index)...
%     );
...
log_odds_prod = sign(UP_DOWN_info.late_UP_V1_log_odds .* UP_DOWN_info.last_ripples_log_odds_UP);log_odds_prod(log_odds_prod<0)=0;
% log_odds_prod = sign(UP_DOWN_info.last_ripples_V1_log_odds_UP .* UP_DOWN_info.last_ripples_log_odds_UP);log_odds_prod(log_odds_prod<0)=0;

% output = plot_UP_survival_probability( ...
%     {log_odds_prod(HPC_reactivation_index)}, ...
%     {UP_DOWN_info.time_from_last_ripples_UP(HPC_reactivation_index)}, ...
%     {UP_DOWN_info.ripple_counts_UP(HPC_reactivation_index)},'count_option',[],'bilateral_merging',1, ...
%     'event_option',[],'title_name', 'Last ripple sign coherence (high bias events) and last ripple to UP-DOWN transition','subject_id',subject_id(HPC_reactivation_index)...
%     );
...
output = plot_UP_survival_probability( ...
    ...
    {UP_DOWN_info.time_from_last_ripples_UP}, ...
    {UP_DOWN_info.ripple_counts_UP},'count_option',[],'bilateral_merging',1, ...
    'event_option','binary','title_name', 'Last ripple sign coherence and last ripple to UP-DOWN transition','subject_id',subject_id...
    );
...
output = plot_UP_survival_probability( ...
    ...
    {UP_DOWN_info.time_from_last_ripples_UP}, ...
    {UP_DOWN_info.ripple_counts_UP},'count_option',[],'bilateral_merging',1, ...
    'event_option',[],'title_name', 'Last ripple V1 log odds PRE and last ripple to UP-DOWN transition','subject_id',subject_id...
    );
...
event_index = UP_DOWN_info.ripple_counts_UP>1;

histogram(sign(UP_DOWN_info.last_ripples_PRE_V1_log_odds_UP(event_index)).*(UP_DOWN_info.last_ripples_V1_log_odds_UP(event_index) - UP_DOWN_info.last_ripples_PRE_V1_log_odds_UP(event_index)))
hold on;
histogram(sign(UP_DOWN_info.first_ripples_PRE_V1_log_odds_UP(event_index)).*(UP_DOWN_info.first_ripples_V1_log_odds_UP(event_index) - UP_DOWN_info.first_ripples_PRE_V1_log_odds_UP(event_index)))


output = plot_UP_survival_probability( ...
    {sign(UP_DOWN_info.last_ripples_PRE_V1_log_odds_UP).*(UP_DOWN_info.last_ripples_V1_log_odds_UP - UP_DOWN_info.last_ripples_PRE_V1_log_odds_UP)}, ...
    {UP_DOWN_info.time_from_last_ripples_UP}, ...
    {UP_DOWN_info.ripple_counts_UP},'count_option',[],'bilateral_merging',1, ...
...
probability_psth_whole = probability;
load(fullfile(analysis_folder,'slow_waves_all_POST.mat'))
tbl = readtable('UP_DOWN_info_GAM.csv');
...
UD_lags = nan(length(UP_DOWN_info.ripple_counts_UP),1);UD_lags_signed = UD_lags;
...
DU_lags = nan(length(UP_DOWN_info.ripple_counts_UP),1);DU_lags_signed = DU_lags;
...
tbl = table(...
    ...
    UP_DOWN_info.ripple_counts_UP(:), ...               % Added ripple_counts_UP
    ...
    'RippleCounts', ...         % Renamed to 'RippleCounts'
    'AnimalID', 'SessionID'});


tbl.secondUPcumIpsiV1 = UP_DOWN_info.ipsi_second_half_V1_MUA_cumulative_UP(:);
tbl.firstUPcumIpsiV1=UP_DOWN_info.ipsi_first_half_V1_MUA_cumulative_UP(:);
tbl.UPcumDiffIpsiV1=UP_DOWN_info.ipsi_second_half_V1_MUA_cumulative_UP(:)-UP_DOWN_info.ipsi_first_half_V1_MUA_cumulative_UP(:);

tbl.secondUPcumHPC=UP_DOWN_info.second_half_HPC_MUA_cumulative_UP(:);
tbl.firstUPcumHPC=UP_DOWN_info.first_half_HPC_MUA_cumulative_UP(:);
tbl.UPcumDiffHPC=UP_DOWN_info.second_half_HPC_MUA_cumulative_UP(:)-UP_DOWN_info.first_half_HPC_MUA_cumulative_UP(:);
tbl.normalisedRippleUPcumHPC= UP_DOWN_info.normalised_ripple_HPC_MUA_cumulative_UP(:);
...
fig = figure;
fig.Name = "Time from last ripple to DOWN distribution";
histogram(tbl.TimefromLastRipple,0:0.01:1,'EdgeAlpha',0,'Normalization','probability')
hold on;xline(prctile(tbl.TimefromLastRipple,[20 40 60 80]))
xlabel('Time from last ripple to DOWN (s)')
...
%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%
tbl_temp = tbl(UP_DOWN_info.ripple_counts_UP>0,:);
lme2 = fitlme(tbl_temp, 'earlyUP ~  lateUPHPC + (1|subject_id) + (1|session_id)');
lme2.Coefficients

lme2 = fitlme(tbl_temp, 'nextUP ~  lateUPHPC + (1|subject_id) + (1|session_id)');
lme2.Coefficients

lme2 = fitlme(tbl_temp, 'lateUP ~  lateUPHPC + (1|subject_id) + (1|session_id)');
lme2.Coefficients

lme2 = fitlme(tbl_temp, ' lateUPHPC~ earlyUP + lateUP + nextUP + (1|subject_id) + (1|session_id)');
lme2.Coefficients
...
beta_V1=nan(1,length(t_win));
beta_HC=nan(1,length(t_win));
clear pval_HC pval_V1
ripple_nextUP_lme = struct(); ripple_lateUP_lme = struct();ripple_last_lme = struct();ripple_earlyUP_lme = struct();
...
for tidx = 1:length(t_win)-1
    ...
    % lme2 = fitlme(tbl_temp, 'lastRippleV1 ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
    ...
    lme2 = fitlme(tbl_temp, 'lateUP ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
    ripple_lateUP_lme.b(tidx,:) = lme2.Coefficients.Estimate(2:end);
    ripple_lateUP_lme.b_CI(:,:,tidx) = [lme2.Coefficients.Lower(2:end) lme2.Coefficients.Upper(2:end)];
    ripple_lateUP_lme.pval(tidx,:) = [lme2.Coefficients.pValue(2:end)];
    ripple_lateUP_lme.variable = [lme2.Coefficients.Name(2:end)];

    lme2 = fitlme(tbl_temp, 'earlyUP ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
    % lme2 = fitlme(tbl_temp, 'lastRippleV1 ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
    ripple_earlyUP_lme.b(tidx,:) = lme2.Coefficients.Estimate(2:end);
    ripple_earlyUP_lme.b_CI(:,:,tidx) = [lme2.Coefficients.Lower(2:end) lme2.Coefficients.Upper(2:end)];
    ripple_nextUP_ripple_earlyUP_lmelme.pval(tidx,:) = [lme2.Coefficients.pValue(2:end)];
    ripple_earlyUP_lme.variable = [lme2.Coefficients.Name(2:end)];
    ...
    ripple_last_lme.variable = [lme2.Coefficients.Name(2:end)];


    lme2 = fitlme(tbl_temp, 'lastRippleHPC ~ lastRippleV1PRE + (1|subject_id) + (1|session_id)');
    ripple_lastPRE_lme.b(tidx,:) = lme2.Coefficients.Estimate(2:end);
    ripple_lastPRE_lme.b_CI(:,:,tidx) = [lme2.Coefficients.Lower(2:end) lme2.Coefficients.Upper(2:end)];
    ripple_lastPRE_lme.pval(tidx,:) = [lme2.Coefficients.pValue(2:end)];
    ripple_lastPRE_lme.variable = [lme2.Coefficients.Name(2:end)];
end
% 
% save(fullfile(analysis_folder,'V1-HPC bilateral interaction','UP-DOWN log odds','last_ripple_time_to_end_lme.mat'),...
...
tbl_temp = tbl(UP_DOWN_info.time_from_last_ripples_UP<0.1,:);
lme2 = fitlme(tbl_temp, 'nextUP ~ lateUP + lateUPHPC + (1|subject_id) + (1|session_id)');
% lme2 = fitlme(tbl_temp, 'lastRippleV1 ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
lateUP_lme_with_ripple.b = lme2.Coefficients.Estimate(2:end);
lateUP_lme_with_ripple.b_CI = [lme2.Coefficients.Lower(2:end) lme2.Coefficients.Upper(2:end)];
lateUP_lme_with_ripple.pval = [lme2.Coefficients.pValue(2:end)];
lateUP_lme_with_ripple.variable = [lme2.Coefficients.Name(2:end)];

tbl_temp = tbl(UP_DOWN_info.time_from_last_ripples_UP>0.1,:);
lme2 = fitlme(tbl_temp, 'nextUP ~ lateUP + lateUPHPC + (1|subject_id) + (1|session_id)');
% lme2 = fitlme(tbl_temp, 'lastRippleV1 ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
lateUP_lme_without_ripple.b = lme2.Coefficients.Estimate(2:end);
lateUP_lme_without_ripple.b_CI = [lme2.Coefficients.Lower(2:end) lme2.Coefficients.Upper(2:end)];
lateUP_lme_without_ripple.pval= [lme2.Coefficients.pValue(2:end)];
lateUP_lme_without_ripple.variable = [lme2.Coefficients.Name(2:end)];

save(fullfile(analysis_folder,'V1-HPC bilateral interaction','UP-DOWN log odds','lateUP_lme_without_ripple.mat'),'lateUP_lme_without_ripple')
save(fullfile(analysis_folder,'V1-HPC bilateral interaction','UP-DOWN log odds','lateUP_lme_with_ripple.mat'),'lateUP_lme_with_ripple')
...
fig = figure('Name','last ripple from UP and to DOWN');
histogram2(UP_DOWN_info.time_to_last_ripples_UP,UP_DOWN_info.time_from_last_ripples_UP);
...
fig = figure('Name','Last ripple effect on current and next UP (time to end)','Position',[850 150 1300 820]);
% fig = figure('Name','Last ripple effect on current and next UP (time from start)','Position',[850 150 1300 820]);
x = t_win(1:end-1)';

nexttile
HC_idx = contains(ripple_nextUP_lme.variable,'lastRippleHPC');
y_CI = squeeze(ripple_nextUP_lme.b_CI(HC_idx,:,:))';
y_mean = ripple_nextUP_lme.b(:,HC_idx);
hold on;
% --- Plot Ipsi (Shading first, then Line) ---
fill([x; flipud(x)], [y_CI(:,1); flipud(y_CI(:,2))], ...
    'b', 'EdgeColor', 'none', 'FaceAlpha', 0.15);
h1 = plot(x, y_mean, 'Color', 'b', 'LineWidth', 2);
yline(0,'k')
title('last ripple -> Next UP')
xlabel('Time from last ripple to DOWN transition')
ylim([-0.05 0.1])
set(gca,'TickDir','out','Box','off','Color','none','FontSize',12); grid off

nexttile
HC_idx = contains(ripple_last_lme.variable,'lastRippleHPC');
y_CI = squeeze(ripple_last_lme.b_CI(HC_idx,:,:))';
...
h1 = plot(x, y_mean, 'Color', 'b', 'LineWidth', 2);
yline(0,'k')
title('last ripple -> last ripple V1')
xlabel('Time from last ripple to DOWN transition')
set(gca,'TickDir','out','Box','off','Color','none','FontSize',12); grid off
ylim([-0.05 0.1])

nexttile
HC_idx = contains(ripple_lastPRE_lme.variable,'lastRippleV1PRE');
y_CI = squeeze(ripple_lastPRE_lme.b_CI(HC_idx,:,:))';
y_mean = ripple_lastPRE_lme.b(:,HC_idx);
hold on;
% --- Plot Ipsi (Shading first, then Line) ---
fill([x; flipud(x)], [y_CI(:,1); flipud(y_CI(:,2))], ...
    'b', 'EdgeColor', 'none', 'FaceAlpha', 0.15);
h1 = plot(x, y_mean, 'Color', 'b', 'LineWidth', 2);
yline(0,'k')
title('last ripple PRE V1 -> last ripple')
xlabel('Time from last ripple to DOWN transition')
set(gca,'TickDir','out','Box','off','Color','none','FontSize',12); grid off
ylim([-0.05 0.1])
...
nexttile
HC_idx = contains(ripple_earlyUP_lme.variable,'lastRippleHPC');
y_CI = squeeze(ripple_earlyUP_lme.b_CI(HC_idx,:,:))';
y_mean = ripple_earlyUP_lme.b(:,HC_idx);
hold on;
% --- Plot Ipsi (Shading first, then Line) ---
fill([x; flipud(x)], [y_CI(:,1); flipud(y_CI(:,2))], ...
    'b', 'EdgeColor', 'none', 'FaceAlpha', 0.15);
...
nexttile
HC_idx = contains(ripple_lateUP_lme.variable,'lastRippleHPC');
y_CI = squeeze(ripple_lateUP_lme.b_CI(HC_idx,:,:))';
y_mean = ripple_lateUP_lme.b(:,HC_idx);
hold on;
% --- Plot Ipsi (Shading first, then Line) ---
fill([x; flipud(x)], [y_CI(:,1); flipud(y_CI(:,2))], ...
    'b', 'EdgeColor', 'none', 'FaceAlpha', 0.15);
...
tbl = table(normalize(UP_DOWN_info.time_from_last_ripples_UP)',V1_direction',normalize(UP_DOWN_info.last_ripples_power_UP)',normalize(UP_DOWN_info.next_early_UP_V1_log_odds)',normalize(UP_DOWN_info.late_UP_V1_log_odds)',normalize(UP_DOWN_info.last_ripples_V1_log_odds_UP)',...
    normalize(UP_DOWN_info.last_ripples_PRE_V1_log_odds_UP)',normalize(UP_DOWN_info.early_UP_V1_log_odds)',normalize(UP_DOWN_info.previous_late_UP_V1_log_odds)',...
    normalize(UP_DOWN_info.late_UP_log_odds)',normalize(UP_DOWN_info.last_ripples_log_odds_UP)',normalize(UP_DOWN_info.first_ripples_log_odds_UP)',normalize(UP_DOWN_info.previous_late_UP_log_odds)',...
    subject_id,UP_session_count,'VariableNames',{'TimefromLastRipple','V1Track','ripplePower','nextUP','lateUP','lastRippleV1', ...
    'lastRippleV1PRE','earlyUP','previousUP'...
    'lateUPHPC','lastRippleHPC','firstRippleHPC','previousUPHPC',...
    'subject_id','session_id'});
% 
% lme2 = fitlme(tbl, 'lastRippleV1 ~  ripplePower*firstRippleHPC+ (1|subject_id) + (1|session_id)');
% lme2.Coefficients

% UP_DOWN_info
%%%%%%%%%%
% HC ripple contribution to ripple V1
%%%%%%%%%%%
V1_HC_ripple_lme = predict_V1_from_pre_V1_and_HC(tbl,[]);
save_all_figures(fullfile(analysis_folder,'V1-HPC bilateral interaction','UP-DOWN log odds'),[])
save(fullfile(analysis_folder,'V1-HPC bilateral interaction','UP-DOWN log odds','V1_HC_ripple_lme'),'V1_HC_ripple_lme')
...
tbl = table(normalize(UP_DOWN_info.time_from_last_ripples_UP)',V1_direction',normalize(UP_DOWN_info.last_ripples_power_UP)',normalize(UP_DOWN_info.next_early_UP_V1_log_odds)',normalize(UP_DOWN_info.late_UP_V1_log_odds)',normalize(UP_DOWN_info.last_ripples_V1_log_odds_UP)',...
    normalize(UP_DOWN_info.last_ripples_PRE_V1_log_odds_UP)',normalize(UP_DOWN_info.early_UP_V1_log_odds)',normalize(UP_DOWN_info.previous_late_UP_V1_log_odds)',...
    normalize(UP_DOWN_info.late_UP_log_odds)',normalize(UP_DOWN_info.last_ripples_log_odds_UP)',normalize(UP_DOWN_info.previous_late_UP_log_odds)',...
    subject_id,UP_session_count,'VariableNames',{'TimefromLastRipple','V1Track','ripplePower','nextUP','lateUP','lastRippleV1', ...
    'lastRippleV1PRE','earlyUP','previousUP'...
    'lateUPHPC','lastRippleHPC','previousUPHPC',...
    'subject_id','session_id'});

lme1 = fitlme(tbl, 'lastRippleV1 ~ lastRippleV1PRE * lastRippleHPC + (1|subject_id) + (1|session_id)');
lme1.Coefficients
lme1.Rsquared.Adjusted

lme1 = fitlme(tbl, 'lastRippleV1 ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
lme1.Coefficients
lme1.Rsquared.Adjusted


lme1 = fitlme(tbl, 'lastRippleV1PRE ~ lastRippleHPC + (1|subject_id) + (1|session_id)');
lme1.Coefficients
lme1.Rsquared.Adjusted

%%%%%%%%%%
% HC ripple contribution to next UP
%%%%%%%%%%%
V1_HC_ripple_lme = predict_V1_from_pre_V1_and_HC(tbl,[]);
save_all_figures(fullfile(analysis_folder,'V1-HPC bilateral interaction','UP-DOWN log odds'),[])
save(fullfile(analysis_folder,'V1-HPC bilateral interaction','UP-DOWN log odds','V1_HC_ripple_lme'),'V1_HC_ripple_lme')
