...
%% Receptive Field From Sparse Noise

SUBJECT = ['M23031';'M23032';'M23034';'M23037';'M23038'];
SESSION = {['20230714'];
    ['20230719';'20230720';'20230721';'20230722'];
    ['20230804';'20230805';'20230806';'20230807'];
    ['20230810';'20230811';'20230812';'20230813'];
    ['20230816';'20230817']};
stimulus_name = 'SparseNoise_fullscreen';
for iMouse = 2:5
    load(fullfile(ROOTPATH,'DATA','SUBJECTS',SUBJECT(iMouse,:),'analysis','experiment_info.mat'))
    for iSession = 1:size(SESSION{iMouse},1)
        load(fullfile(ROOTPATH,'DATA','SUBJECTS',SUBJECT(iMouse,:),'analysis',SESSION{iMouse}(iSession,:),stimulus_name,'session_info.mat'))
        load(fullfile(ROOTPATH,'DATA','SUBJECTS',SUBJECT(iMouse,:),'analysis',SESSION{iMouse}(iSession,:),stimulus_name,'extracted_clusters_ks3.mat'))
        load(fullfile(ROOTPATH,'DATA','SUBJECTS',SUBJECT(iMouse,:),'analysis',SESSION{iMouse}(iSession,:),stimulus_name,'extracted_task_info.mat'))
        receptive_field = cell(length(session_info.probe),1);
        for nprobe = 1:length(session_info.probe)
            ...
            no_units = length(clusters_ks3(nprobe).cluster_id);
            cluster_id = clusters_ks3(nprobe).cluster_id;
            resps = zeros(no_units,7,size(Task_info.stim_matrix,1));
            parfor unit_id = 1:no_units
                spikes_this_cell = clusters_ks3(nprobe).spike_times(clusters_ks3(nprobe).spike_id==cluster_id(unit_id));
            ...
            unit_no = 0;
            initMap =cell(size(resps,1),1);
            parfor unit_id = 1:size(resps,1)
            ...
            receptive_field{nprobe} = initMap;
            
        end
    save(fullfile(ROOTPATH,'DATA','SUBJECTS',SUBJECT(iMouse,:),'analysis',SESSION{iMouse}(iSession,:),stimulus_name,'receptiveFields_ks3.mat'),'receptive_field')
    end
