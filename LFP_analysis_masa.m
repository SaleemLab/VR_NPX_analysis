...
xlim([find(sorted_config.Channel == best_ripple_channel) 80])
ylim([find(sorted_config.Channel == best_ripple_channel) 80])
...
for nchannel = channels_selected(1):channels_selected(end)
    ...
    if ~isempty(good_units_this_channel) % If any SUA
        ...
        for unit = 1:length(good_units_index)
            ...
            spike_ID = [spike_ID; good_units_this_channel(unit)*ones(length(these_spike_times{good_units_index(unit)}),1)];
        end
...
% sorted_config_spikes.SpikeGLXchan0 (maybe better to use 0 bsaed for accurate channel)
...
[ripples] = FindRipples_masa(raw_LFP(best_ripple_channel,:)',tvec','minDuration',20,'durations',[30 100],'frequency',new_SR,'noise',raw_LFP(1,:)','passband',[125 300],'show','off')
save extracted_ripples_events ripples

% need to extract best ripple channel for V1
[V1_ripples] = FindRipples_masa(raw_LFP(250,:)',tvec','minDuration',20,'durations',[30 100],'frequency',new_SR,'noise',raw_LFP(1,:)','passband',[125 300],'show','off')
