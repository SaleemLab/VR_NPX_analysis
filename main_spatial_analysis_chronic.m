...
%             | (clusters_combined.peak_percentile(:,2)>0.95&clusters_combined.odd_even_stability(:,2)>0.95));
        ...
        for track_id = 1:length(place_fields_BAYESIAN)
            for temp_track = 1:length(place_fields_BAYESIAN)
                ...
                decoded_position_V1_combined_shuffled{nsession}{track_id}{temp_track}  = [];
                decoded_error_V1_combined_shuffled{nsession}{track_id}{temp_track}  = [];
...
%                             [~,index] = max(estimated_position_lap_CV_shuffled_HPC(nprobe).track(track_id).lap(lap_id).track(temp_track).run);
                        ...
                        if temp_track == 1 %Do not need to loop twice
                            ...
                            for nshuffle = 1:100
                                ...
                                [~,index] = max([estimated_position_lap_CV_shuffled_HPC(nprobe).track{nshuffle}(track_id).lap(lap_id).track(1).run; ...
                    ...
                    if length(session_info(n).probe)>1
                        ...
                        [~,index]=max(estimated_position_lap_CV_V1_combined.track(track_id).lap(lap_id).track(temp_track).run);
                        decoded_error_V1_combined{nsession}{track_id}{temp_track} = [decoded_error_V1_combined{nsession}{track_id}{temp_track} ...
                            position_bin(index) - estimated_position_lap_CV_V1_combined.track(track_id).lap(lap_id).track(track_id).run_actual_position];

                        [~,index]=max(estimated_position_lap_CV_HPC_combined.track(track_id).lap(lap_id).track(temp_track).run);
                        decoded_error_HPC_combined{nsession}{track_id}{temp_track} = [decoded_error_HPC_combined{nsession}{track_id}{temp_track} ...
                            position_bin(index) - estimated_position_lap_CV_HPC_combined.track(track_id).lap(lap_id).track(track_id).run_actual_position];

                        for nshuffle = 1:100
                            [~,index]=max(estimated_position_lap_CV_shuffled_V1_combined.track{nshuffle}(track_id).lap(lap_id).track(temp_track).run);

                            decoded_error_V1_combined_shuffled{nsession}{track_id}{temp_track} = [decoded_error_V1_shuffled{probe_hemisphere}{nsession}{track_id}{temp_track} ...
                                position_bin(index) - estimated_position_lap_CV_shuffled_V1_combined.track(track_id).lap(lap_id).track(temp_track).run_actual_position];

                            [~,index]=max(estimated_position_lap_CV_shuffled_HPC(nprobe).track{nshuffle}(track_id).lap(lap_id).track(temp_track).run);

                            decoded_error_HPC_combined_shuffled{nsession}{track_id}{temp_track} = [decoded_error_HPC_shuffled{probe_hemisphere}{nsession}{track_id}{temp_track} ...
                                position_bin(index) - estimated_position_lap_CV_HPC(nprobe).track(track_id).lap(lap_id).track(temp_track).run_actual_position];
                            %                             [~,index] = max(estimated_position_lap_CV_shuffled_HPC(nprobe).track(track_id).lap(lap_id).track(temp_track).run);
                        end
                        ...
                        if temp_track == 1 %Do not need to loop twice
                            ...
                            for nshuffle = 1:100
                                [~,index] = max([estimated_position_lap_CV_shuffled_V1(nprobe).track{nshuffle}(track_id).lap(lap_id).track(1).run; ...
                                    estimated_position_lap_CV_shuffled_V1(nprobe).track{nshuffle}(track_id).lap(lap_id).track(2).run]);
                                decoded_position_V1_combined_shuffled{nsession}{track_id} = [decoded_position_V1_combined_shuffled{nsession}{track_id} index];

                                [~,index] = max([estimated_position_lap_CV_shuffled_HPC(nprobe).track{nshuffle}(track_id).lap(lap_id).track(1).run; ...
                                    estimated_position_lap_CV_shuffled_HPC(nprobe).track{nshuffle}(track_id).lap(lap_id).track(2).run]);
                                decoded_position_HPC_combined_shuffled{nsession}{track_id} = [decoded_position_HPC_combined_shuffled{nsession}{track_id} index];
                            end
        ...
        if length(session_info(n).probe)>1
            decoding_performance = plot_within_session_decoded_error_HPC_V1(decoded_position_lap_id,VR_speed,actual_position,estimated_position_lap_CV_V1,decoded_position_V1,decoded_position_HPC,decoded_position_HPC_combined,decoded_position_V1_combined,...
                decoded_error_V1,decoded_error_HPC,decoded_error_HPC_combined,decoded_error_V1_combined,place_fields_BAYESIAN,session_info(n),nsession)
        else
